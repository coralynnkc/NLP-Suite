...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window,
        "word2vec_main.py",
        ["os", "tkinter", "pandas", "gensim", "stanza", "itertools", "numpy", "string"],
    )
    == False
):
    sys.exit(0)
...
def run_Gensim_word2vec(
    ...
    startTime = IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis start", "Started running Gensim Word2Vec at", True
    )
    ...
    IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis end", "Finished running Gensim Word2Vec at", True, "", True, startTime
    )
