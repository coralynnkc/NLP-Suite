...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "NLP_setup_external_software_main", ["os", "tkinter", "webbrowser", "time"]
    )
    == False
):
    sys.exit(0)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, missing_software_lb, True
)
...
missing_software_display_area = tk.Entry(
    width=GUI_IO_util.missing_software_display_area_width, state="disabled", textvariable=missing_software_var
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.website_url_placement,
    y_multiplier_integer,
    missing_software_display_area,
    True,
    False,
    True,
    False,
    ...
    "The widget, always disabled, displays all the external software yet to be installed on your machine.",
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    "Open csv config file 'NLP_setup_external_software_config.csv'\nYou can manually enter any external software installation path (in case of errors with the setup algorithm)",
)

software_download_lb = tk.Label(window, text="Software DOWNLOAD from web")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, software_download_lb, True
)
software_download_var.set("")
# temporarily excluded '*'
software_download_menu = tk.OptionMenu(
    window, software_download_var, "Stanford CoreNLP", "Gephi", "Google Earth Pro", "Java (JDK)", "MALLET", "WordNet"
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    ...
    "Select the FREEWARE external software to be downloaded; the software website url will be displayed after selection. YOU CAN MAKE MULTIPLE SELECTIONS AND SAVE UPON CLOSING."
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.website_url_placement, y_multiplier_integer, software_website, True
)

software_website_url_var = tk.StringVar()
software_website_display_area = tk.Entry(
    width=GUI_IO_util.missing_software_display_area_width, state="disabled", textvariable=software_website_url_var
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    ...
    "The widget, always disabled, displays the download website of the external software selected in the 'Software download from web' dropdown menu.",
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, software_install_lb, True
)
software_install_var.set("")
# temporarily excluded '*'
software_install_menu = tk.OptionMenu(
    window, software_install_var, "Stanford CoreNLP", "Gephi", "Google Earth Pro", "Java (JDK)", "MALLET", "WordNet"
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    ...
    90,
    GUI_IO_util.labels_x_coordinate,
    "Select the external software to be installed after having downloaded it; the software installation directory will be displayed after selection."
...
software_install_area = tk.Entry(
    width=GUI_IO_util.missing_software_display_area_width, state="disabled", textvariable=software_install_dir_var
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    ...
    90,
    GUI_IO_util.watch_videos_x_coordinate,
    "The widget, always disabled, displays the installation directory of the external software selected in the 'Software installation on your machine' dropdown menu.",
)

opensoftware_dir_button = tk.Button(
    window,
    width=GUI_IO_util.open_file_directory_button_width,
    text="",
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    x_coordinate_hover_over,
    y_multiplier_integer,
    opensoftware_dir_button,
    False,
    False,
    True,
    False,
    90,
    x_coordinate_hover_over - 200,
    "Open external software directory",
)
...
def activate_software_download(*args):
    ...
    if software_download_var.get() != "":
        ...
        software_dir, software_url, missing_software, error_found = IO_libraries_util.get_external_software_dir(
            scriptName, software_name, silent=True, only_check_missing=True, install_download="download"
        )
        software_website_url_var.set(software_url)
        software_website_display_area = software_url
        download_message = "###"
        software_dir, software_url, download_message = IO_libraries_util.external_software_download(
            scriptName, software_name, existing_software_config, False
        )
...
def activate_software_install(download_install, software_dir, software_url, missing_software, silent=False):
    ...
    if software_install_var.get() != "" or "download" in download_install:
        ...
        software_dir, software_url, missing_software, error_found = IO_libraries_util.get_external_software_dir(
            scriptName, software_name, silent=False, only_check_missing=True, install_download="install"
        )
        # software_install_dir_var.set(software_dir)
        software_website_url_var.set(software_url)
        software_website_display_area = software_url
        ...
        # @@@
        # 9/1
        software_dir, existing_software_config, errorFound = IO_libraries_util.external_software_install(
            scriptName, software_name, existing_software_config, silent=False, errorFound=error_found
        )
        if software_dir == None or software_dir == "":
            software_install_dir_var.set("Not installed")
            software_install_area = "Not installed"  # software_install_dir_var.get()
...
software_install_var.trace(
    "w",
    lambda x, y, z: activate_software_install(
        "", software_install_dir_var.get(), software_website_url_var.get(), "", silent=True
    ),
)
...
TIPS_lookup = {
    "Stanford CoreNLP download install run": "TIPS_NLP_Stanford CoreNLP download install run.pdf",
...
TIPS_options = (
    "Stanford CoreNLP download install run",
    "Topic modeling MALLET installation",
...
software_dir, software_url, missing_external_software, error_found = IO_libraries_util.get_external_software_dir(
    scriptName, software_download_var.get(), silent=True, only_check_missing=True
)
if missing_external_software == "":
    ...
    if not os.path.isfile(config_filename):
        mb.showwarning(
            title="External software installation error",
