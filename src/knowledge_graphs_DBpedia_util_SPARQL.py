...
def DBpedia_annotate(inputFile, inputDir, outputDir, configFileName, annotationTypes, colors):
    ...
    for file in files:
        ...
        outFilename = os.path.join(outputDir, "NLP_DBpedia_annotated_" + str(fileName.split(".txt")[0]) + ".html")
...
def form_query_string(phrase, ont_ls):
    ...
    for ont in ont_ls:
        query_body = (
            query_body + " { ?w1 a " + ont + " } UNION" + "{?w1 a ?type.\n ?type rdfs:subClassOf* " + ont + "} UNION"
        )
...
def check_eligible(phrase):
    if [x for x in phrase if (x not in punksAndNum)] != [] and len(phrase) > 2 and phrase.lower() != "not":
        return True
    else:
        return False
