...
def _run_analysis():
    ...
    if enter_data_ID_var != "":
        ...
        if from_dataID_setupID_objectType_var == "Complex":
            ID_setup_complex, complex_name = DB_PCACE_data_analysis_util.get_setup_complex_ID_Name_from_data_complex_ID(
                int(enter_data_ID_var)
            )
            setup_name_var.set(complex_name)
        elif from_dataID_setupID_objectType_var == "Simplex":
            ID_setup_simplex, simplex_name = (
                DB_PCACE_data_analysis_util.get_setup_simplex_ID_Name_from_simplex_value_ID(int(enter_data_ID_var))
            )
    ...
    if not os.path.exists(outputSubDir):
        outputDir = IO_files_util.make_output_subdirectory("", "", outputDir, label=tail[:-5], silent=False)
        if outputDir == "":
            return
    else:
    ...
    any_simplex_checkbox = (
        simplex_export_values_var.get() == 1
        or simplex_charts_var.get() == 1
        or simplex_timechart_var.get() == 1
        or simplex_GIS_var.get() == 1
    )
    any_other_operation = (
        identifiers == 1
        or extended_headers == 1
        or document_sources_var == 1
        or comments_var == 1
        or parents_children_var.get() == 1
        or any_simplex_checkbox
        or search_simplex_value != ""
        or simplex_value != ""
    )
    if primary_complex_var != "" and not any_other_operation:
        ...
        if html_filepath:
            ...
            mb.showwarning(title="Story form", message=f"The story form has been saved to:\n{filepath}")
            if openOutputFiles:
    ...
    if search_simplex_value != "":
        ...
        if search_simplex_result != "":
            ...
            story_text, filepath = DB_PCACE_data_analysis_util.story_form_from_dropdown(
                search_simplex_result, outputDir
            )
            if filepath:
                filesToOpen.append(filepath)
            ...
            if proceed:
                ...
                if filepath:
                    filesToOpen.append(filepath)
                html_filepath = DB_PCACE_data_analysis_util.search_and_export_stories_html(
                    search_simplex_value, outputDir
                )
                if html_filepath:
                    filesToOpen.append(html_filepath)
                    if openOutputFiles:
    ...
    if setup_complex != "":
        ...
        # Checkbox 4: export comments for the selected complex
            ...
            comment_files = DB_PCACE_data_analysis_util.get_comment_info(
                "", setup_complex, comment_type_str, inputDir, outputDir
            )
            if comment_files:
            ...
            if n_objects > 0:
                ...
                if proceed:
                    ...
                    if filepath:
                        filesToOpen.append(filepath)
                    html_filepath = DB_PCACE_data_analysis_util.export_all_stories_html_for_type(
                        setup_complex, outputDir
                    )
                    if html_filepath:
                        filesToOpen.append(html_filepath)
                        if openOutputFiles:
                ...
                df = DB_PCACE_data_analysis_util.higher_lower(
                    inputDir, outputDir, setup_complex, export_identifier=False
                )
    ...
    if setup_simplex != "":
        ...
        if simplex_export_values_var.get() == 1:
            values_csv = DB_PCACE_data_analysis_util.get_data_simplex_values_listing(inputDir, outputDir, setup_simplex)
        ...
        if simplex_charts_var.get() == 1:
            ...
            values_csv = DB_PCACE_data_analysis_util.get_data_simplex_values_listing(inputDir, outputDir, setup_simplex)
            if values_csv and os.path.isfile(values_csv):
                chart_outputFiles = charts_util.plot(
                    values_csv,
                    outputDir,
                    columns=[],
                    title=f'Frequency of "{setup_simplex}" values',
                    x_label=setup_simplex,
                    group_by=None,
                )
            ...
            if vtype == 3:  # date
                try:
                    ...
                    if timechart_csv and os.path.isfile(timechart_csv):
                        ...
                        charts_util.timechart(
                            timechart_csv, timechart_output, var_name, date_fmt, cumulative=False, yearly=True
                        )
        ...
        if simplex_GIS_var.get() == 1:
            gis_csv = DB_PCACE_data_analysis_util.prepare_gis_locations_csv(inputDir, outputDir, setup_simplex)
...
GUI_IO_util.place_help_button(
    window, GUI_IO_util.help_button_x_coordinate, y_multiplier_integer - 1, "NLP Suite Help", GUI_IO_util.msg_IO_setup
)
...
_shell_help_button = tk.Button(
    window,
    text="? HELP",
    command=lambda: GUI_IO_util.display_help_button_info(
        "NLP Suite Help",
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, notebook, False, True
)
...
view_relations_button = tk.Button(
    grammar_tab, text="View table relations", width=17, height=1, state="disabled", command=lambda: view_relations()
)
...
view_grammar_button = tk.Button(
    grammar_tab, text="View grammar (as text)", width=17, height=1, state="disabled", command=lambda: view_grammar()
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    grammar_tab,
...
def visualize_grammar_tree():
    ...
    if csv_path:
        ...
        outputFiles = charts_util.hierarchical_tree(
            csv_path, outputDir_val, parent_col="Parent", child_col="Child", color_col="Type"
        )
        if outputFiles:
            IO_files_util.OpenOutputFiles(GUI_util.window, True, outputFiles, outputDir_val)
    else:
        mb.showwarning(
            title="Warning", message="No grammar structure found.\n\nPlease, make sure the PC-ACE database is loaded."
        )


visualize_grammar_tree_button = tk.Button(
    grammar_tab,
    text="View grammar (as tree)",
    width=20,
    height=1,
    state="disabled",
...
update_grammar_button = tk.Button(
    grammar_tab, text="Update grammar", width=17, height=1, state="disabled", command=lambda: update_grammar()
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    grammar_tab,
...
update_identifier_button = tk.Button(
    grammar_tab, text="Update identifiers", width=17, height=1, state="disabled", command=lambda: update_identifiers()
)
# place widget with hover-over info
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    grammar_tab, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, object_type_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    grammar_tab,
    GUI_IO_util.labels_x_coordinate + 50,
    y_multiplier_integer,
...
setup_complex_menu, setup_simplex_menu = (
    DB_PCACE_data_analysis_util.get_setup_complex_simplex_names()
...
_required_object_base_text = (
    "You can use the dropdown menu to scroll through the list of available objects.\n"
...
def _update_required_object_dropdown(*args):
    ...
    _update_combo_hover(
        required_object,
        _required_object_y_row,
        GUI_IO_util.open_setup_x_coordinate + 150,
        GUI_IO_util.open_TIPS_x_coordinate,
        n,
        _required_object_base_text,
    )
...
def _rename_grammar_object():
    ...
    new_name = tk.simpledialog.askstring(
        "Rename " + obj_type, f"Current name: {obj_name}\n\nEnter new name:", initialvalue=obj_name
    )
    ...
    if proceed:
        success, msg = DB_PCACE_data_analysis_util.rename_grammar_object(
            obj_type, obj_name, new_name.strip(), inputDir.get()
        )
        if success:
...
def _merge_grammar_objects():
    ...
    tk.Label(
        merge_win,
        text=f"All data from '{source_name}' will be reassigned\n"
        f"to the selected target. '{source_name}' will then\n"
        f"be removed from the grammar.",
        fg="gray",
    ).pack(pady=5)
    ...
    def _do_merge():
        ...
        if proceed:
            merge_win.destroy()
            success, msg = DB_PCACE_data_analysis_util.merge_grammar_objects(
                obj_type, source_name, target_name, inputDir.get()
            )
            if success:
...
rename_button = tk.Button(
    grammar_tab, text="Rename", width=8, height=1, state="disabled", command=_rename_grammar_object
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    grammar_tab,
    GUI_IO_util.labels_x_coordinate,
...
remove_button = tk.Button(
    grammar_tab, text="Remove", width=8, height=1, state="disabled", command=_remove_grammar_object
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    grammar_tab,
    GUI_IO_util.labels_x_coordinate + 80,
    y_multiplier_integer,
    remove_button,
    True,
    False,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    grammar_tab,
    GUI_IO_util.labels_x_coordinate + 160,
    y_multiplier_integer,
    merge_button,
    True,
    False,
...
merge_info_lb = tk.Label(
    grammar_tab, text="Select Object type and name above, then click Rename / Remove / Merge", font=("", 8), fg="gray"
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    grammar_tab,
    GUI_IO_util.labels_x_coordinate + 240,
    y_multiplier_integer,
    merge_info_lb,
    False,
    True,
...
# place widget with hover-over info
lookup_y = GUI_IO_util.placeWidget(
    lookup_tab,
...
lookup_y = GUI_IO_util.placeWidget(
    lookup_tab, GUI_IO_util.open_reminders_x_coordinate, lookup_y, enter_data_ID_lb, True
)
...
lookup_y = GUI_IO_util.placeWidget(
    lookup_tab,
    GUI_IO_util.open_reminders_x_coordinate + 100,
    lookup_y,
    enter_data_ID,
    True,
    False,
    ...
    90,
    GUI_IO_util.open_reminders_x_coordinate + 100,
    "Enter the numeric data ID value",
)
...
lookup_y = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.open_setup_x_coordinate,
    "Extracted setup name",
)


complex_objects_lb = tk.Label(query_tab, text="Complex ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    query_tab, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, complex_objects_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    query_tab,
    GUI_IO_util.labels_x_coordinate + 90,
    y_multiplier_integer,
    setup_complex,
    True,
    False,
...
identifiers_checkbox = tk.Checkbutton(
    complex_options_frame, text="Identifiers", variable=identifiers_var, onvalue=1, offvalue=0, state="disabled"
)
extended_headers_checkbox = tk.Checkbutton(
    complex_options_frame,
    text="Extended headers",
    variable=extended_headers_var,
    onvalue=1,
    offvalue=0,
    state="disabled",
)
parents_children_checkbox = tk.Checkbutton(
    complex_options_frame,
    text="Parents & children",
    variable=parents_children_var,
    onvalue=1,
    offvalue=0,
    state="disabled",
)
document_sources_checkbox = tk.Checkbutton(
    complex_options_frame,
    text="Document sources",
    variable=document_sources_var,
    onvalue=1,
    offvalue=0,
    state="disabled",
)
comments_checkbox = tk.Checkbutton(
    complex_options_frame, text="Comments", variable=comments_var, onvalue=1, offvalue=0, state="disabled"
)
...
complex_options_frame.place(
    x=GUI_IO_util.labels_x_coordinate + 90, y=GUI_IO_util.basic_y_coordinate + 40 * (_setup_complex_y_row + 1)
)

complex_identifiers_lb = tk.Label(query_tab, text="Complex identifier")
y_multiplier_integer = GUI_IO_util.placeWidget(
    query_tab, GUI_IO_util.open_setup_x_coordinate, y_multiplier_integer, complex_identifiers_lb, True
)
...
def _update_combo_hover(combo, y_row, x_coord, x_hover, count, base_text):
    ...
    combo.bind(
        ...
        lambda e, t=tip: (
            ...
            GUI_IO_util.display_widget_info(
                query_tab, e, x_coord, GUI_IO_util.basic_y_coordinate + GUI_IO_util.y_step * y_row, x_hover, t
            ),
        ),
    )
    combo.bind(
        "<Leave>",
        lambda e: (
            e.widget.config(background=combo.cget("background"), foreground=combo.cget("foreground")),
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    query_tab, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, simplex_objects_lb, True
)
...
simplex_export_values_checkbox = tk.Checkbutton(
    simplex_options_frame,
    text="Export values",
    variable=simplex_export_values_var,
    onvalue=1,
    offvalue=0,
    state="disabled",
)
simplex_charts_checkbox = tk.Checkbutton(
    simplex_options_frame, text="Charts", variable=simplex_charts_var, onvalue=1, offvalue=0, state="disabled"
)
simplex_timechart_checkbox = tk.Checkbutton(
    simplex_options_frame, text="Time chart", variable=simplex_timechart_var, onvalue=1, offvalue=0, state="disabled"
)
simplex_GIS_checkbox = tk.Checkbutton(
    simplex_options_frame, text="GIS map", variable=simplex_GIS_var, onvalue=1, offvalue=0, state="disabled"
)
...
simplex_options_frame.place(
    x=GUI_IO_util.labels_x_coordinate + 90, y=GUI_IO_util.basic_y_coordinate + 40 * (_setup_simplex_y_row + 1)
)

simplex_values_var = tk.StringVar()
simplex_values_lb = tk.Label(query_tab, text="Simplex values")
y_multiplier_integer = GUI_IO_util.placeWidget(
    query_tab, GUI_IO_util.open_setup_x_coordinate, y_multiplier_integer, simplex_values_lb, True
)
...
def _simplex_values_enter(event):
    ...
    values_csv = DB_PCACE_data_analysis_util.get_data_simplex_values_listing(inputDir_val, outputDir_val, simplex_name)
    if values_csv and os.path.isfile(values_csv):
...
lookup_y = GUI_IO_util.placeWidget(
    ...
    False,
    90,
    GUI_IO_util.labels_x_indented_coordinate + 300,
...
value_parent_object_checkbox = tk.Checkbutton(
    lookup_tab,
    text="Get simplex/complex objects of selected data type (& value)",
    variable=value_parent_object_var,
    onvalue=1,
    offvalue=0,
)
# place widget with hover-over info
...
search_simplex_results = ttk.Combobox(
    lookup_tab, textvariable=search_simplex_results_var, width=GUI_IO_util.widget_width_short
)
...
def run_simplex_search(*args):
    ...
    if results:
        ...
        search_simplex_results_var.set("")
        mb.showwarning(title="Search results", message=f'No hierarchical objects found containing "{search_term}".')
    # Update hover-over with item count
    n = len(results) if results else 0
    _update_combo_hover(
        search_simplex_results,
        _search_results_y_row,
        GUI_IO_util.open_setup_x_coordinate + 150,
        GUI_IO_util.open_reminders_x_coordinate,
        n,
        "Auto-populated when you type a search term and press Enter in the Search simplex value field.\n"
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    query_tab, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, select_parents_lb, True
)

select_parents = ttk.Combobox(
    query_tab, width=GUI_IO_util.widget_width_short, textvariable=complex_parents_var, state="disabled"
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    query_tab, GUI_IO_util.open_setup_x_coordinate, y_multiplier_integer, select_children_lb, True
)
...
def _update_last_updated_hovers(in_dir, out_dir):
    ...
    gis_base = (
        "GIS MAP: geocode location values and display on Google Earth Pro, Google Maps, and Folium.\n"
...
def changed_filename(*args):
    ...
    if GUI_util.input_main_dir_path.get() != "" and GUI_util.input_main_dir_path.get() != inputDirSV:
        ...
        if (len(table_list) == 0) or (
            (len(table_list) > 18)
            and ("data_Document.xlsx" not in str(table_list) and "data_Complex.xlsx" not in str(table_list))
        ):
            GUI_util.run_button.configure(state="disabled")
            table_menu_values = []
            error = True
        ...
        if len(setup_complex_menu) > 0:
            ...
            if not database_already_loaded:
                try:
                    complex_identifiers_menu = DB_PCACE_data_analysis_util.build_macro_event_dropdown_menu(
                        inputDir.get()
                    )
                    complex_identifiers["values"] = complex_identifiers_menu
                    ...
                    hierarchical_complex_menu = DB_PCACE_data_analysis_util.build_hierarchical_complex_dropdown_menu(
                        inputDir.get()
                    )
                    complex_identifiers["values"] = complex_identifiers_menu
                except Exception as e:
...
def activate_parents_children(*args):
    ...
    if setup_complex_var.get() != "":
        ...
        children_complex_list_all, children_complex_list_required = (
            DB_PCACE_data_analysis_util.get_setup_complex_children(setup_complex_var.get())
        )
        ...
        #                                            False, '', True, '', False)
        #
        simplex_children_all_list, simplex_children_required_list = (
            DB_PCACE_data_analysis_util.get_setup_complex_simplex_children(setup_complex_var.get())
        )
...
def view_grammar():
    ...
    DB_PCACE_data_analysis_util.view_grammar(
        os.path.join(inputDir.get(), "setup_Complex.xlsx"),
        "GrammarRule_Text",
        os.path.join(inputDir.get(), "PC-ACE grammar for database " + tail + ".txt"),
    )
...
TIPS_options = (
    "PC-ACE tables analyzer via Pandas",
    "PC-ACE - Export ACCESS tables to Excel",
    "SVO automatic extraction and visualization",
    "Google Earth Pro",
    "Google API Key",
    "Geocoding",
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Select Object type (Complex/Simplex) and an object name, then press Enter to toggle its REQUIRED value."
        + GUI_IO_util.msg_Esc,
    )
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        "Enter: export the story form for the selected object.\n"
        "RUN: export the story form, or perform a checkbox operation." + GUI_IO_util.msg_Esc,
    )
    # Row: Simplex
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        ...
        "Spell-check and lemmatization have been moved to the Data Validation GUI." + GUI_IO_util.msg_Esc,
    )
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        "RESULTS: select a match and press Enter to export its story form." + GUI_IO_util.msg_Esc,
    )
...
def run_validation():
    ...
    def _spell_check_bar_chart(dupes_csv, n_total, outputDir):
        ...
        try:
            ...
            bars = ax.bar(["Flagged for review", "No issues"], [n_flagged, n_correct], color=["#e74c3c", "#2ecc71"])
            for bar in bars:
                ax.text(
        ...
        if simplex_name and simplex_name != "ALL text simplexes":
            ...
            if vtype != 1:
                ...
                try:
                    ...
                    mb.showerror(title="Spell-check error", message=f'Spell-check failed for "{simplex_name}":\n\n{e}')
        else:
            ...
            try:
                ...
                mb.showerror(title="Spell-check error", message=f"Spell-check failed:\n\n{e}")
    ...
    if lemmatize_var.get() == 1:
        ...
        if not noun_simplexes and not verb_simplexes:
            ...
            for sx_name in noun_simplexes:
                try:
                    lemma_csv = DB_PCACE_data_analysis_util.lemmatize_simplex_values(
                        inputDir, outputDir, simplex_name=sx_name, language=lang_code, pos_filter=["NOUN", "PROPN"]
                    )
                    if lemma_csv and os.path.isfile(lemma_csv):
                        all_lemma_files.append(lemma_csv)
                except Exception as e:
                    mb.showerror(
                        title="Lemmatization error", message=f'Lemmatization failed for "{sx_name}" (noun):\n\n{e}'
                    )
            ...
            for sx_name in verb_simplexes:
                try:
                    lemma_csv = DB_PCACE_data_analysis_util.lemmatize_simplex_values(
                        inputDir, outputDir, simplex_name=sx_name, language=lang_code, pos_filter=["VERB", "AUX"]
                    )
                    if lemma_csv and os.path.isfile(lemma_csv):
                        all_lemma_files.append(lemma_csv)
                except Exception as e:
                    mb.showerror(
                        title="Lemmatization error", message=f'Lemmatization failed for "{sx_name}" (verb):\n\n{e}'
                    )
    ...
    if run_cross_db or run_side_by_side or has_agg_simplexes:
        ...
        if not db_dirs:
            ...
            if run_cross_db:
                if len(db_dirs) < 2:
                    mb.showwarning(
                        title="Cross-DB comparison",
                        message="Cross-DB comparison requires at least 2 databases.\n\n"
                        "Use the + button to add more database directories.",
                    )
                else:
                    try:
                        cross_files = DB_PCACE_data_analysis_util.compare_aggregate_codes_across_dbs(db_dirs, outputDir)
                        ...
                        mb.showerror(title="Cross-DB comparison error", message=f"Cross-DB comparison failed:\n\n{e}")

            if run_side_by_side or has_agg_simplexes:
                orig_name = agg_orig_var.get()
                if not orig_name:
                    mb.showwarning(
                        title="Side-by-side mapping",
                    ...
                    for db_dir in db_dirs:
                        try:
                            side_file = DB_PCACE_data_analysis_util.build_aggregate_side_by_side(
                                db_dir, outputDir, original_simplex=orig_name, simplex_names=list(_agg_simplex_list)
                            )
                            if side_file:
...
def get_csv_file(validation_tab, title, fileType, annotate):
    initialFolder = (
        os.path.dirname(os.path.abspath(csv_file_var.get()))
        if csv_file_var.get()
...
valid_y = GUI_IO_util.placeWidget(validation_tab, GUI_IO_util.labels_x_coordinate, valid_y, csv_file_button, True)
...
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
...
valid_y = GUI_IO_util.placeWidget(validation_tab, GUI_IO_util.entry_box_x_coordinate, valid_y, csv_file_entry, True)
...
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
    GUI_IO_util.entry_box_x_coordinate + 580,
    valid_y,
    clear_csv_button,
    True,
    False,
...
def _apply_changes():
    ...
    if not inputDir_val:
        mb.showwarning(title="Apply changes", message="Please select a PC-ACE database directory first.")
        return
    if not _ensure_database_loaded(inputDir_val):
        mb.showwarning(
            title="Apply changes", message="Could not load the PC-ACE database. Please check the input directory."
        )
        return
    import pandas as pd
...
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
    GUI_IO_util.entry_box_x_coordinate + 640,
    valid_y,
...
valid_y = GUI_IO_util.placeWidget(validation_tab, GUI_IO_util.labels_x_coordinate, valid_y, spell_check_lb, True)

spell_check_var = tk.IntVar()
spell_check_checkbox = tk.Checkbutton(
    validation_tab, text="Run spell-check", variable=spell_check_var, onvalue=1, offvalue=0
)
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
    GUI_IO_util.open_TIPS_x_coordinate,
...
spell_check_simplex_menu = ttk.Combobox(
    validation_tab, textvariable=spell_check_simplex_var, width=30, state="readonly"
)
spell_check_simplex_menu["values"] = ["ALL text simplexes"]
spell_check_simplex_menu.bind("<<ComboboxSelected>>", _combobox_release_focus)
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
    GUI_IO_util.open_TIPS_x_coordinate + 200,
    valid_y,
...
valid_y = GUI_IO_util.placeWidget(validation_tab, GUI_IO_util.labels_x_coordinate, valid_y, lemmatize_lb, True)

lemmatize_var = tk.IntVar()
lemmatize_checkbox = tk.Checkbutton(
    validation_tab, text="Run lemmatization", variable=lemmatize_var, onvalue=1, offvalue=0
)
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
    GUI_IO_util.open_TIPS_x_coordinate,
...
lemmatize_lang_menu = ttk.Combobox(
    validation_tab, textvariable=lemmatize_lang_var, width=20, values=_stanza_languages, state="disabled"
)
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
    GUI_IO_util.open_TIPS_x_coordinate + 200,
...
valid_y = GUI_IO_util.placeWidget(
    validation_tab, GUI_IO_util.labels_x_indented_coordinate, valid_y, lemmatize_nouns_lb, True
)
...
def _update_noun_hover():
    ...
    add_noun_button.bind(
        ...
        lambda e, t=btn_tip: (
            ...
            GUI_IO_util.display_widget_info(
                validation_tab,
                e,
                GUI_IO_util.open_TIPS_x_coordinate + 280,
                y_pos - 20,
                GUI_IO_util.open_TIPS_x_coordinate + 280,
                t,
            ),
    ...
    lemmatize_nouns_menu.bind(
        "<Enter>",
        lambda e, t=combo_tip: GUI_IO_util.display_widget_info(
            validation_tab, e, GUI_IO_util.open_TIPS_x_coordinate, y_pos - 20, GUI_IO_util.open_TIPS_x_coordinate, t
        ),
    )
...
add_noun_button = tk.Button(
    validation_tab, text="+", width=GUI_IO_util.add_button_width, height=1, state="disabled", command=_add_noun_simplex
)
_noun_btn_y = valid_y
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
    GUI_IO_util.open_TIPS_x_coordinate + 230,
    valid_y,
    add_noun_button,
    True,
    False,
...
reset_noun_button = tk.Button(
    validation_tab,
    text="Reset",
    width=GUI_IO_util.reset_button_width,
    height=1,
    state="disabled",
    command=_reset_noun_simplexes,
)
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
    GUI_IO_util.open_TIPS_x_coordinate + 270,
    valid_y,
    reset_noun_button,
    True,
    False,
    ...
    False,
    90,
    GUI_IO_util.open_TIPS_x_coordinate + 320,
...
valid_y = GUI_IO_util.placeWidget(
    validation_tab, GUI_IO_util.open_TIPS_x_coordinate + 380, valid_y, lemmatize_verbs_lb, True
)
...
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
    GUI_IO_util.open_TIPS_x_coordinate + 450,
    valid_y,
    ...
    False,
    90,
    GUI_IO_util.open_TIPS_x_coordinate + 470,
    ...
    add_verb_button.bind(
        ...
        lambda e, t=btn_tip: (
            ...
            GUI_IO_util.display_widget_info(
                validation_tab,
                e,
                GUI_IO_util.open_TIPS_x_coordinate + 700,
                y_pos - 20,
                GUI_IO_util.open_TIPS_x_coordinate + 700,
                t,
            ),
...
add_verb_button = tk.Button(
    validation_tab, text="+", width=GUI_IO_util.add_button_width, height=1, state="disabled", command=_add_verb_simplex
)
_verb_btn_y = valid_y
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
...
reset_verb_button = tk.Button(
    validation_tab,
    text="Reset",
    width=GUI_IO_util.reset_button_width,
    height=1,
    state="disabled",
    command=_reset_verb_simplexes,
)
...
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
    GUI_IO_util.close_button_x_coordinate + 35,
    valid_y,
    reset_verb_button,
    False,
    False,
...
valid_y = GUI_IO_util.placeWidget(validation_tab, GUI_IO_util.labels_x_coordinate, valid_y, agg_lb, True)
...
_init_input = (
    GUI_util.input_main_dir_path.get() if hasattr(GUI_util.input_main_dir_path, "get") else GUI_util.input_main_dir_path
)
if (
    _init_input
    and os.path.isdir(str(_init_input))
    and os.path.isfile(os.path.join(str(_init_input), "data_Complex.xlsx"))
):
...
agg_mode_menu = ttk.Combobox(
    validation_tab,
    textvariable=agg_mode_var,
    width=25,
    state="readonly",
...
def _refresh_agg_simplex_display():
    ...
    agg_simplex_selected_label.bind(
        "<Enter>",
        lambda e, t=tip: GUI_IO_util.display_widget_info(
            validation_tab,
            e,
            GUI_IO_util.labels_x_indented_coordinate,
...
_entry_width = 165
agg_simplex_selected_label = tk.Entry(
    validation_tab, textvariable=agg_simplex_selected_var, width=_entry_width, state="readonly"
)
valid_y = GUI_IO_util.placeWidget(
    validation_tab,
    GUI_IO_util.labels_x_indented_coordinate,
...
def _apply_aggregate_corrections():
    ...
    if not inputDir_val or not os.path.isdir(inputDir_val):
        mb.showwarning(title="Apply corrections", message="Please select a PC-ACE database directory first.")
        return
    if not _ensure_database_loaded(inputDir_val):
        mb.showwarning(title="Apply corrections", message="Could not load the PC-ACE database.")
        return
    ...
    try:
        n_applied = DB_PCACE_data_analysis_util.apply_aggregate_corrections(
            edited_path, inputDir_val, original_csv_path=_original_side_by_side_csv[0]
        )
        if n_applied > 0:
            mb.showinfo(
                title="Corrections applied",
            ...
            mb.showinfo(
                title="No changes", message="No changes detected between the edited CSV and the original values."
            )
        else:
            mb.showerror(title="Error", message="An error occurred. Check the console output for details.")
    except Exception as e:
