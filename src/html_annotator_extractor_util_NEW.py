...
if (
    IO_libraries_util.install_all_Python_packages(
        "html_annotator_extractor_util", ["os", "tkinter", "re", "csv", "ntpath"]
    )
    == False
):
    sys.exit(0)
...
def gatherAnnotations(inputFile, tags, mustInclude="<p>", cleanMultiples=True):
    ...
    text = open(inputFile, encoding="utf-8", errors="ignore").read()
...
def buildcsv(inputHTMLFile, inputHTMLFolder, outputDir, openOutputFiles, chartPackage, dataTransformation):
    ...
    startTime = IO_user_interface_util.timed_alert(
        ...
        2000,
        "Analysis start",
        "Started running html annotator extractor at",
        True,
        "You can follow html annotation extractor in command line.",
    )
    ...
    IO_user_interface_util.timed_alert(
        ...
        2000,
        "Analysis end",
        "Finished running html annotator extractor at",
        True,
        "",
        True,
        startTime,
