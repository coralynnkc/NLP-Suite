...
if not IO_libraries_util.install_all_Python_packages(
    GUI_util.window, "sentiment_analysis_main.py", ["os", "tkinter", "subprocess"]
):
    sys.exit(0)
...
def run():
    ...
    if BERT_var == 1:
        ...
        outputFiles = BERT_util.sentiment_main(
            inputFilename, inputDir, outputDir, config_filename, mode, chartPackage, dataTransformation, model_path
        )
        if outputFiles != None:
    ...
    if (BERT_var or spaCy_var or CoreNLP_var or Stanza_var) and shape_of_stories_var:
        ...
        config_input_output_alphabetic_options_temp, missing_IO, config_file_exists = config_util.read_config_file(
            config_filename_temp, config_input_output_numeric_options_temp
        )
        ...
        config_util.write_IO_config_file(
            GUI_util.window,
            config_filename_temp,
            config_input_output_numeric_options,
            config_input_output_alphabetic_options_temp,
            True,
        )

        reminders_util.checkReminder(
            scriptName, reminders_util.title_options_shape_of_stories, reminders_util.message_shape_of_stories, True
        )
    ...
    if anew_var == 1 and (mean_var or median_var):
        if language == "English":
            ...
            outputFiles = sentiment_analysis_ANEW_util.main(
                inputFilename, inputDir, outputDir, mode, chartPackage, dataTransformation
            )
            ...
            IO_user_interface_util.timed_alert(
                ...
                2000,
                "Analysis end",
                "Finished running ANEW Sentiment Analysis at",
                True,
                "",
                True,
                startTime,
    ...
    if SA_algorithm_var == "*" or nrc_var == 1:
        if language == "English":
            ...
            outputFiles = sentiment_analysis_NRC_util.main(
            ...
            IO_user_interface_util.timed_alert(
                ...
                2000,
                "Analysis end",
                "Finished running NRC Emotion Analysis at",
                True,
                "",
                True,
                startTime,
    ...
    if character_arcs_var == 1:
        if language == "English":
            ...
            outputFiles = character_emotion_arcs_util.main(
                inputFilename, inputDir, outputDir, chartPackage, dataTransformation
            )
            if outputFiles is not None:
                if isinstance(outputFiles, str):
            ...
            IO_user_interface_util.timed_alert(
                ...
                2000,
                "Analysis end",
                "Finished running Character Emotion Arcs at",
                True,
                "",
                True,
                startTime,
    ...
    if SA_algorithm_var == "*" or hedonometer_var == 1 and (mean_var or median_var):
        ...
        if language == "English":
            startTime = IO_user_interface_util.timed_alert(
                ...
                2000,
                "Analysis start",
                "Started running HEDONOMETER Sentiment Analysis at",
                True,
                "",
                True,
                "",
            ...
            outputFiles = sentiment_analysis_hedonometer_util.main(
                inputFilename, inputDir, outputDir, mode, chartPackage, dataTransformation
            )
            ...
            IO_user_interface_util.timed_alert(
                ...
                2000,
                "Analysis end",
                "Finished running HEDONOMETER Sentiment Analysis at",
                True,
                "",
                True,
                startTime,
    ...
    if SA_algorithm_var == "*" or SentiWordNet_var == 1 and (mean_var or median_var):
        if language == "English":
            ...
            startTime = IO_user_interface_util.timed_alert(
                ...
                2000,
                "Analysis start",
                "Started running SentiWordNet Sentiment Analysis at",
                True,
                "",
                True,
                "",
            ...
            outputFiles = sentiment_analysis_SentiWordNet_util.main(
                inputFilename, inputDir, outputDir, config_filename, mode, chartPackage, dataTransformation
            )
            ...
            IO_user_interface_util.timed_alert(
                ...
                2000,
                "Analysis end",
                "Finished running SentiWordNet Sentiment Analysis at",
                True,
                "",
                True,
                startTime,
    ...
    if SA_algorithm_var == "*" or vader_var == 1 and (mean_var or median_var):
        if language == "English":
            ...
            startTime = IO_user_interface_util.timed_alert(
                ...
                2000,
                "Analysis start",
                "Started running VADER Sentiment Analysis at",
                True,
                "",
                True,
                "",
            ...
            outputFiles = sentiment_analysis_VADER_util.main(
                inputFilename, inputDir, outputDir, mode, chartPackage, dataTransformation
            )
            ...
            IO_user_interface_util.timed_alert(
                ...
                2000,
                "Analysis end",
                "Finished running VADER Sentiment Analysis at",
                True,
                "",
                True,
                startTime,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, mean_checkbox, True
)

median_checkbox = tk.Checkbutton(window, text="Calculate sentence median", variable=median_var, onvalue=1, offvalue=0)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.sentiment_analysis_median_checkbox_pos, y_multiplier_integer, median_checkbox
)
...
def display_reminder(*args):
    if "Neural network" in SA_algorithm_var.get() or "Dictionar" in SA_algorithm_var.get():
        mb.showwarning(
            ...
            + " is only a label. Please, select one of the indented sentiment analysis options and try again.",
        )
        return
    if SA_algorithm_var.get() == "":
        mb.showwarning(
            "Warning",
            "The selected option is only a spacer. Please, select one of the indented sentiment analysis options and try again.",
        )
        return
    if "Stanford CoreNLP" in SA_algorithm_var.get():
        ...
        reminders_util.checkReminder(
            scriptName, reminders_util.title_options_SA_VADER, reminders_util.message_SA_VADER, True
        )
        if mean_var.get() or median_var.get() == True:
            reminders_util.checkReminder(
                scriptName, reminders_util.title_options_VADER_MeanMedian, reminders_util.message_VADER_MeanMedian, True
            )
    elif "SentiWordNet" in SA_algorithm_var.get():
        reminders_util.checkReminder(
            scriptName, reminders_util.title_options_SA_SentiWordNet, reminders_util.message_SA_SentiWordNet, True
        )
    elif "spaCy" in SA_algorithm_var.get():
        mb.showinfo(
            "spaCy (TextBlob) Sentiment",
...
SA_algorithms = [
    ...
    "",
    "Character-level:",
    "   Character Emotion Arcs (NER + NRC)",
]

SA_algorithm_var.set("*")
SA_algorithm_lb = tk.Label(window, text="Select sentiment analysis algorithm")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, SA_algorithm_lb, True
)

SA_algorithm_menu = tk.OptionMenu(window, SA_algorithm_var, *SA_algorithms)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.sentiment_analysis_SA_algorithm_menu_pos, y_multiplier_integer, SA_algorithm_menu
)

y_multiplier_integerSV = y_multiplier_integer - 1

sentence_index_var.set(0)
sentence_index_checkbox = tk.Checkbutton(
    window,
    state="disabled",
    text="Do sentiments fluctuate across a document (Sentiment scores by sentence index)",
    variable=sentence_index_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, sentence_index_checkbox
)

shape_of_stories_var.set(0)
shape_of_stories_checkbox = tk.Checkbutton(
    window,
    text="Do sentiments fluctuate across documents (Open 'Shape of stories' GUI)",
    variable=shape_of_stories_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, shape_of_stories_checkbox
)

character_arcs_checkbox_var = tk.IntVar()
character_arcs_checkbox_var.set(0)
character_arcs_checkbox = tk.Checkbutton(
    window,
    text="Do sentiments fluctuate across actors and/or locations? (Sentiment scores by NER values)",
    variable=character_arcs_checkbox_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, character_arcs_checkbox
)

ALL_options_button = tk.Button(
    window,
    text="Sentiments/emotions (ALL options GUI)",
    command=lambda: run_script_util.run_script("sentiments_emotions_ALL_main.py"),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, ALL_options_button
)
...
def activate_SOS(*args):
    # the Shape of Stories is only available when processing a directory and using CoreNLP
    if input_main_dir_path.get() == "" or (
        "BERT" not in SA_algorithm_var.get().strip()
        and "spaCy" not in SA_algorithm_var.get().strip()
        and SA_algorithm_var.get().strip() != "Stanza"
        and SA_algorithm_var.get().strip() != "Stanford CoreNLP"
    ):
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox to display a line plot of sentiment scores by sentence index across a specific document.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
...
readMe_message = (
    "The Python 3 Dictionary-based Analyses scripts calculate the mean/median values for various aspects of the language used in a text: sentiment, arousal, dominance.\n\nIn INPUT the scripts expect either a single text file or a set of text files stored in a directory. THE hedonometer, ANEW, AND VADER SCRIPTS ALSO EXPECT TO FIND DICTIONARY FILES IN A "
    "lib"
    " SUBFOLDER OF THE FOLDER WHERE THE PYTHON SCRIPTS ARE STORED.\n\nIn OUTPUT, the scripts create csv files containing the calculated mean/median values for each sentence."
)
