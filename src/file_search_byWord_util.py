...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "file_search_byWord_util.py", ["os", "tkinter", "stanza", "re"]
    )
    == False
):
    sys.exit(0)
...
def search_sentences_documents(
    ...
    for file in files:
        ...
        if search_by_dictionary or search_by_search_keywords:
            ...
            if file[-4:] != ".txt":
                continue
        with open(file, encoding="utf-8", errors="ignore") as f_doc:
            docText = f_doc.read()
        import NGrams_CoOccurrences_util
    ...
    IO_error = IO_csv_util.list_to_csv(
        GUI_util.window, distinct_keywords_not_found_list, outputFilename_csv_distinct_word_NOT_found
    )
    if not IO_error:
    ...
    if not search_within_sentence:
        ...
        IO_error = IO_csv_util.list_to_csv(
            GUI_util.window, all_search_keywords_NOT_found, outputFilename_csv_word_NOT_found
        )
        if not IO_error:
    ...
    # search in sentence
        ...
        all_search_keywords_NOT_found.insert(0, header)
        IO_error = IO_csv_util.list_to_csv(
            GUI_util.window, all_search_keywords_NOT_found[2], outputFilename_csv_word_NOT_found
        )
        if not IO_error:
            filesToOpen.append(outputFilename_csv_word_NOT_found)
        ...
        with open(outputFilename_csv_word, "w", newline="") as f_csv:
            ...
            if extract_sentences:
                ...
                with open(
                    ...
                    outputFile_extract_w_searchword.write(all_found_sentences_allDocs)
                with open(
                    outputFilename_extract_wo_searchword, "w", encoding="utf-8", errors="ignore"
                ) as outputFile_extract_wo_searchword:
                    outputFile_extract_wo_searchword.write(all_adjacent_sentences_allDocs)
    ...
    if create_subcorpus_var and len(corpus_to_copy) > 0:
        ...
        mb.showwarning(
            title="Warning",
            message='The search function has created a subcorpus of the files containing the search word(s) "'
    ...
    if not search_within_sentence:
        outputFiles = charts_util.plot(
            ...
            outputDir,
            columns=["Frequency of occurrence"],
            title=chart_title,
            x_label=search_keywords_str,
        )
        if outputFiles != None:
        ...
        if not search_keywords_found:
            ...
            try:
                ...
                mb.showwarning(
                    ...
                    message="Could not read the file " + outputFilename_csv_word + "\n\nThe file is not utf-8",
                )
    ...
    IO_user_interface_util.timed_alert(
        ...
        2000,
        "Analysis end",
        "Finished running the Search word function at",
        True,
        "",
        True,
        startTime,
...
def visualize_wordcloud(
    ...
    if len(textToProcess) < 15:
        mb.showwarning(
            title="Warning",
            message='The text required to produce a wordcloud is too short.\n"'
            + textToProcess
            + "\n\nWordcloud exits.",
        )
        return
