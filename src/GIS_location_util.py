...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "GIS_location_util", ["re", "tkinter", "csv", "pandas"]
    )
    == False
):
    sys.exit(0)
...
def extract_NER_locations(window, conllFile, encodingValue, datePresent):
    ...
    if len(locList) == 0:
        mb.showwarning(
            title="NER locations",
            message="There are no NER tags for 'LOCATION','STATE_OR_PROVINCE','CITY','COUNTRY' in your CoNLL file\n\n"
            + conllFile
...
def extract_csvFile_locations(
    ...
    try:
        ...
        mb.showerror(
            ...
            message="There was an error in the function 'Extract csv locations' reading the input csv file\n"
            + str(inputFilename)
    ...
    if len(locList) == 0:
        mb.showwarning(
            title="Locations",
            message="There are no locations in your input file\n\n"
            + inputFilename
            + "\n\nThere is no geocoding to be done.\n\nNo maps can be done.",
        )
        return
