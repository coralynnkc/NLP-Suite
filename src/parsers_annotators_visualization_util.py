...
def parsers_annotators_visualization(
    ...
    if ("Lemma" in str(annotator_params) and "Lemma" in temp_outputFilename) or "parse" in str(annotator_params):
        ...
        outputFiles = charts_util.plot(
            ...
            outputDir,
            columns=["Form", "Lemma"],
            title="Frequency Distribution of Form & Lemma Values",
            x_label="Form & Lemma values",
            file_label="form-lemma",
            plot_list=["Frequency"],
            title_label="Lemma Values",
        )
    ...
    if ("POS" in str(annotator_params) and "POS" in temp_outputFilename) or "parse" in str(annotator_params):
        outputFiles = charts_util.plot(
            ...
            outputDir,
            columns=["POS"],
            title="Frequency Distribution of Part of Speech (POS) Tags",
            x_label="POS (Part of Speech) tag values",
            file_label="POS",
            plot_list=["Frequency"],
            title_label="POS (Part of Speech) Tag Values",
        )
        if outputFiles != None:
            if isinstance(outputFiles, str):
                filesToOpen.append(outputFiles)
        ...
        outputFiles = charts_util.plot(
            ...
            outputDir,
            columns=["Form"],
            title="Frequency Distribution of Form Values",
            x_label="Form values",
            file_label="Form",
            group_by=["POS"],
            plot_list=["Frequency"],
            title_label="Form Values",
        )
        if outputFiles != None:
            if isinstance(outputFiles, str):
                filesToOpen.append(outputFiles)
    ...
    if ("NER" in str(annotator_params) and "NER" in temp_outputFilename) or "parse" in str(annotator_params):
        ...
        if IO_csv_util.get_csvfile_headers(outputFilename, False)[1] == "NER":
            ...
            outputFiles = charts_util.plot(
                ...
                outputDir,
                columns=["NER"],
                title="Frequency Distribution of NER Tags",
                x_label="NER tags",
                file_label="NER-tag",
                plot_list=["Frequency"],
                title_label="NER",
            ...
            outputFiles = charts_util.plot(
                ...
                outputDir,
                columns=["Word"],
                title="Frequency Distribution of NER " + NER_tag + " Values",
                x_label="NER " + NER_tag + " expression",
                file_label="NER-tag-value",
                title_label="NER Tag Values",
            )
            if outputFiles != None:
                if isinstance(outputFiles, str):
                    filesToOpen.append(outputFiles)
            ...
            if "Multi-Word Expression" in _mwe_cols:
                outputFiles = charts_util.plot(
                    outputFilename,
                    outputDir,
                    columns=["Multi-Word Expression"],
                    title="Frequency Distribution of Named Entities (Multi-Word Expressions)",
                    x_label="Named entities (multi-word expressions)",
                    file_label="Named-Entity",
                    group_by=["NER"],
                    plot_list=["Frequency"],
                    title_label="Named Entity",
                )
                if outputFiles != None:
                    if isinstance(outputFiles, str):
                        filesToOpen.append(outputFiles)
    ...
    if "parse" in str(annotator_params):  # and 'CoNLL' in outputFilename) or ('depparse' in str(annotator_params)):
        ...
        outputFiles = charts_util.plot(
            ...
            outputDir,
            columns=["DepRel"],
            title="Frequency Distribution of DepRel (Dependency Relations) Values",
            x_label="DepRel values",
            file_label="DepRel",
            plot_list=["Frequency"],
            title_label="DepRel",
        )
    ...
    if ("date" in str(annotator_params) and "date" in temp_outputFilename) or (
        ...
        outputFiles = charts_util.plot(
            outputFilename,
            outputDir,
            columns=["Date expression"],
            title="Frequency Distribution of Date Expressions",
            x_label="Date expression",
            file_label="date-express",
            plot_list=["Frequency"],
            title_label="Date Expressions",
        )
        if outputFiles != None:
            if isinstance(outputFiles, str):
                filesToOpen.append(outputFiles)
        ...
        outputFiles = charts_util.plot(
            outputFilename,
            outputDir,
            columns=["Normalized date"],
            title="Frequency Distribution of Normalized Dates",
            x_label="Normalized date",
            file_label="date",
            plot_list=["Frequency"],
            title_label="Normalized Dates",
        )
        if outputFiles != None:
            if isinstance(outputFiles, str):
                filesToOpen.append(outputFiles)
        ...
        outputFiles = charts_util.plot(
            ...
            outputDir,
            columns=["Date type"],
            title="Frequency Distribution of Date Types",
            x_label="Date type",
            file_label="date-types",
            plot_list=["Frequency"],
            title_label="Date Types",
        )
        if outputFiles != None:
            if isinstance(outputFiles, str):
                filesToOpen.append(outputFiles)
    ...
    if "gender" in str(annotator_params) and "gender" in temp_outputFilename:
        outputFiles = charts_util.plot(
            ...
            outputDir,
            columns=["Gender"],
            title="Frequency Distribution of Gender Values",
            x_label="Gender values",
            file_label="gender-values",
            plot_list=["Gender"],
            title_label="Gender",
        )
        if outputFiles != None:
        ...
        outputFiles = charts_util.plot(
            ...
            outputDir,
            columns=["Word"],
            title="Frequency Distribution of Gendered Words",
            x_label="Gender words",
            file_label="gender-words",
            group_by=["Gender", "Document"],
            plot_list=["Frequency"],
            title_label="Gendered Words",
        )
        if outputFiles != None:
            if isinstance(outputFiles, str):
                filesToOpen.append(outputFiles)
    ...
    if "quote" in str(annotator_params) and "quote" in temp_outputFilename:
        outputFiles = charts_util.plot(
            ...
            outputDir,
            columns=["Speakers"],
            title="Frequency Distribution of Speakers\n(CoreNLP Quote Annotator)",
            x_label="Speakers",
            file_label="quote",
            plot_list=["Frequency"],
            title_label="Quotes",
    ...
    if "sentiment" in str(annotator_params) and "sentiment" in temp_outputFilename:
        if IO_csv_util.get_csvfile_headers(outputFilename, False)[0] == "Sentiment score":
            outputFiles = charts_util.plot(
                ...
                title="Frequency Distribution of Sentiment Scores",
                x_label="Sentiment score",
                file_label="score",
                plot_list=["Sentiment score"],
                title_label="Sentiment Score Statistics",
            )
            if outputFiles != None:
                if isinstance(outputFiles, str):
                    filesToOpen.append(outputFiles)
        ...
        if IO_csv_util.get_csvfile_headers(outputFilename, False)[1] == "Sentiment label":
            outputFiles = charts_util.plot(
                ...
                title="Frequency Distribution of Sentiment Labels",
                x_label="Sentiment label",
                file_label="label",
                plot_list=["Sentiment label"],
                title_label="Sentiment Label Statistics",
            )
            if outputFiles != None:
                if isinstance(outputFiles, str):
                    filesToOpen.append(outputFiles)
    ...
    if ("SVO" in str(annotator_params) and "SVO" in temp_outputFilename) or (
        ...
        if not os.path.isdir(outputSVOUnFilterDir):
            outputSVOUnFilterDir = IO_files_util.make_output_subdirectory(
                "", "", outputDir, label="SVO_form", silent=True
            )
        ...
        outputFiles = charts_util.plot(
            ...
            columns=["Subject (S)"],
            title="Frequency Distribution of Subjects (unlemmatized, unfiltered)",
            x_label="Subjects (unlemmatized, unfiltered)",
            file_label="S-form",
            plot_list=["Frequency"],
            title_label="Subjects (unlemmatized, unfiltered)",
        )
        if outputFiles != None:
            if isinstance(outputFiles, str):
                filesToOpen.append(outputFiles)
        ...
        outputFiles = charts_util.plot(
            ...
            outputSVOUnFilterDir,
            columns=["Verb (V)"],
            title="Frequency Distribution of Verbs (unlemmatized, unfiltered)",
            x_label="Verbs (unlemmatized, unfiltered)",
            file_label="V-form",
            plot_list=["Frequency"],
            title_label="Verbs (unlemmatized, unfiltered)",
        )
        if outputFiles != None:
            if isinstance(outputFiles, str):
                filesToOpen.append(outputFiles)
        ...
        outputFiles = charts_util.plot(
            ...
            outputSVOUnFilterDir,
            columns=["Object (O)"],
            title="Frequency Distribution of Objects (unlemmatized, unfiltered)",
            x_label="Objects (unlemmatized, unfiltered)",
            file_label="O-form",
            plot_list=["Frequency"],
            title_label="Objects (unlemmatized, unfiltered)",
        )
        if outputFiles != None:
            if isinstance(outputFiles, str):
                filesToOpen.append(outputFiles)
    ...
    if "coref table" in str(annotator_params) or "parser" in str(annotator_params) or "SVO" in str(annotator_params):
        ...
        if "coref table" in str(annotator_params):
            outputFiles = charts_util.plot(
                ...
                outputDir,
                columns=["Pronoun"],
                title="Frequency Distribution of Pronouns (Antecedents)",
                x_label="Pronouns (antecedents)",
                file_label="pronouns",
                group_by=None,
            )
            if outputFiles != None:
            ...
            outputFiles = charts_util.plot(
                ...
                outputDir,
                columns=["Referent"],
                title="Frequency Distribution of Coreferences (Referents)",
                x_label="Coreferences (referents)",
                file_label="referents",
                group_by=None,
            )
            if outputFiles != None:
                if isinstance(outputFiles, str):
...
def visualize_html_file(
    inputFilename, inputDir, outputDir, configFileName, dictFilename, genderCol=["Gender"], wordCol=[]
):
