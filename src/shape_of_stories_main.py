...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window,
        "shape_of_stories_main.py",
        ["subprocess", "os", "tkinter", "matplotlib", "csv", "numpy", "sklearn", "tqdm", "codecs"],
    )
    == False
):
    sys.exit(0)
...
def run():
    ...
    if (
        ...
        and corpus_analysis == False
        and hierarchical_clustering == False
        and SVD == False
        and NMF == False
        and best_topic_estimation == False
    ):
        mb.showwarning(
            title="Option selection error",
    ...
    startTime = IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis start", "Started running Shape of Stories at", True
    )
    ...
    if hierarchical_clustering or SVD or NMF or best_topic_estimation:
        ...
        val = GUI_IO_util.slider_widget(
            GUI_util.window,
            "Please, select the value for window size. Window size is the number of sentences "
            + "that will be averaged to obtain one point of the story arc. The recommend value is "
            + str(vectz.window_size)
            + ".",
            1,
            vectz.min_doc_len - 1,
            vectz.window_size,
        ...
        val = GUI_IO_util.slider_widget(
            GUI_util.window,
            "Please, select the value for sentiment vector size. Sentiment vector size is the number of values "
            + "that each document will be represented with. The recommend value is "
            + str(vectz.ideal_sent_v_size)
            + ".",
            1,
            vectz.min_doc_len,
        ...
        val = GUI_IO_util.slider_widget(
            GUI_util.window,
            "Please, select the value for number of clusters (modes). The recommend value is "
            + str(rec_n_clusters)
            + ".",
            1,
            vectz.sentiment_vector_size,
            rec_n_clusters,
        )
        rec_n_clusters = val
    ...
    if best_topic_estimation:
        ...
        IO_user_interface_util.timed_alert(
            GUI_util.window, 2000, "Analysis end", "Finished running estimate_best_k at", True, "", True, startTime1
        )

    IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis end", "Finished running Shape of Stories at", True, "", True, startTime
    )
...
sentiment_analysis_checkbox = tk.Checkbutton(
    window,
    text="Sentiment Analysis",
    variable=sentiment_analysis_var,
    onvalue=1,
    offvalue=0,
    command=lambda: check_IO_requirements(GUI_util.inputFilename.get(), GUI_util.input_main_dir_path.get()),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, sentiment_analysis_checkbox, True
)

sentiment_analysis_lb = tk.Label(window, text="Select the Sentiment Analysis algorithm")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.shape_of_stories_sentiment_analysis_lb_pos, y_multiplier_integer, sentiment_analysis_lb, True
)

sentiment_analysis_menu_var.set("BERT (English model)")
sentiment_analysis_menu = tk.OptionMenu(
    window,
    sentiment_analysis_menu_var,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.shape_of_stories_sentiment_analysis_menu_pos,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.shape_of_stories_memory_lb_pos, y_multiplier_integer, memory_var_lb, True
)

memory_var = tk.Scale(window, from_=1, to=16, orient=tk.HORIZONTAL)
memory_var.set(6)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.shape_of_stories_memory_pos, y_multiplier_integer, memory_var
)


def activate_warning(*args):
    if (
        "CoreNLP" not in sentiment_analysis_menu_var.get()
        and "BERT (English model)" not in sentiment_analysis_menu_var.get()
        and "BERT (Multilingual model)" not in sentiment_analysis_menu_var.get()
        and "spaCy" not in sentiment_analysis_menu_var.get()
        and "Stanza" not in sentiment_analysis_menu_var.get()
    ):
        mb.showwarning(
            title="Sentiment Analysis option deprecated",
            message="The selected sentiment analysis dictionary-based option '"
            + (sentiment_analysis_menu_var.get()).lstrip()
...
corpus_analysis_checkbox = tk.Checkbutton(
    window, text="Compute & visualize corpus statistics", variable=corpus_analysis_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, corpus_analysis_checkbox
)


hierarchical_clustering_var.set(1)
hierarchical_clustering_checkbox = tk.Checkbutton(
    window, text="Hierarchical Clustering (HC)", variable=hierarchical_clustering_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, hierarchical_clustering_checkbox
)


SVD_var.set(1)
SVD_checkbox = tk.Checkbutton(
    window, text="Singular Value Decomposition (SVD)", variable=SVD_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, SVD_checkbox
)


NMF_var.set(1)
NMF_checkbox = tk.Checkbutton(
    window, text="Non-Negative Matrix Factorization (NMF)", variable=NMF_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, NMF_checkbox
)

# RF
best_topic_estimation_var.set(0)
best_topic_estimation_checkbox = tk.Checkbutton(
    window, text="Best topic estimation", variable=best_topic_estimation_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, best_topic_estimation_checkbox
)
...
def check_IO_requirements(inputFilename, inputDir):
    ...
    if inputFilename != "":
        if sentimentAnalysis == True or corpus_analysis == True:
            # txt files required
            mb.showwarning(title="Input directory error", message=txt_fileErr)
            Error = True
            return Error
        ...
        if not ("Document" in str2 and "Sentence" in str2 and "Sentiment" in str2):
            mb.showwarning(title="csv file error", message=csv_fileErr)
            Error = True
            return Error
        ...
        if nSAscoreFiles < 50:
            # too few csv files
            answer = mb.askyesno("Data warning: Data reduction algorithms", message=csv_fileWarning)
            if answer == False:
                Error = True
                return Error
    else:  # inputDir
        if inputDir != "":
            if sentimentAnalysis == True or corpus_analysis == True:
                ...
                if nSAscoretxtFiles < 50 and sentimentAnalysis == True:
                    # too few txt files
                    answer = mb.askyesno("Input directory warning", message=txt_dirWarning)
                    if answer == False:
                        Error = True
                        return Error
            ...
            if not sentimentAnalysis:
                if hierarchical_clustering or SVD or NMF or best_topic_estimation:
                    ...
                    if nSAscorecsvFiles == 0:
                        ...
                        mb.showwarning(title="Input directory error", message=csv_DirErr + alternative_msg)
                        Error = True
                    elif nSAscorecsvFiles < 50 and sentimentAnalysis == True:
                        # too few csv files
                        answer = mb.askyesno("Data reduction algorithms", message=csv_dirWarning)
                        if answer == False:
                            Error = True
                return Error
    ...
    if (
        inputFilename != ""
        and sentiment_analysis_var.get() == False
        and corpus_analysis_var.get() == False
        and (hierarchical_clustering_var.get() == True or SVD_var.get() == True or NMF_var.get() == True)
    ):
        nSAscoreFiles = IO_csv_util.GetMaxValueInCSVField(inputFilename, "Shape of Stories", "Document ID")
        if nSAscoreFiles == 0:
            Error = True
    ...
    if (
        ...
        if nSAscoreFiles == 0:
            mb.showwarning(title="Data warning: Data reduction algorithms", message=csv_DirErr)
            Error = True
            return Error
...
TIPS_options = (
    "Shape of stories",
    "Sentiment analysis",
    "Data reduction algorithms: Parameters formulae",
    "Data reduction algorithms: Hierarchical clustering (HC)",
    "Data reduction algorithms: Singular Value Decomposition (SVD)",
    "Data reduction algorithms: Non-negative Matrix Factorization (NMF)",
    "Shape of stories: Best topic estimation",
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    outputDirMsg = (
        "\n\nIn OUTPUT all results of the analyses will be saved in a double subdirectory of the output directory - Shape of Stories/Last part of input directory name/sentiment_analysis_results_last part of input directory name."
        + IO_widget
    )
    ...
    if not IO_setup_display_brief:
        y_multiplier_integer = GUI_IO_util.place_help_button(
            ...
            y_multiplier_integer,
            "NLP Suite Help",
            inputFileMsg + GUI_IO_util.msg_openFile,
        )
        y_multiplier_integer = GUI_IO_util.place_help_button(
            window,
            help_button_x_coordinate,
            y_multiplier_integer,
            "NLP Suite Help",
            inputDirTXTCSVMsg + GUI_IO_util.msg_openExplorer,
        )
        y_multiplier_integer = GUI_IO_util.place_help_button(
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox 'Sentiment Analysis' if you wish to run the Sentiment Analysis algorithm.\n\nIf you do want to run the algorithm, using the dropdown menu, please select the type of Sentiment Analysis algorithm you wish to use (Stanford CoreNLP neural network approach recommended)."
        + inputDirTXTMsg
        + outputDirMsg,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to compute & visualize corpus statistics. This will help you identify any document outlier in terms of number of words and, particularly relevant for the analysis of the shape of stories, number of sentences."
        + inputDirTXTMsg
        + outputDirMsg,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to run the Hierarchical Clustering algorithm of data reduction."
        + inputDirCSVMsg
        + outputDirMsg,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to run the SVD (Singular Value Decomposition) algorithm of data reduction."
        + inputDirCSVMsg
        + outputDirMsg,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to run the NMF (Non-Negative Matrix Factorization) algorithm of data reduction."
        + inputDirCSVMsg
        + outputDirMsg,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
