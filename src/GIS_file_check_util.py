...
def geocoded_checker(numColumns, minColumns, headers, locationColumnValue, inputFilename, encodingValue):
    ...
    try:
        ...
        mb.showerror(
            ...
            message="There was an error reading the input file\n"
            + str(inputFilename)
            + "\nwith geocoded input.\n\n"
            + str(e),
        )
...
def location_column_checker(inputFilename, locationColumnValue, encodingValue):
    ...
    if check1 == False:
        mb.showerror(
            title="Input file error",
            message="The location column you selected, "
            + locationColumnValue
...
def restrictions_checker(inputFilename, inputIsCoNLL, withHeader, headers, locationColumnValue):
    ...
    if inputIsGeocoded == True:
        ...
        if (
            geocoded_checker(numColumns, minColumns, headers, locationColumnValue, inputFilename, encodingValue)
            == False
        ):
            return False
    return True
