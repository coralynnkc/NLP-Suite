...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window,
        "CoreNLP_annotator",
        ["os", "tkinter", "time", "json", "re", "subprocess", "string", "pandas", "pycorenlp", "nltk"],
    )
    == False
):
    sys.exit(0)
...
def create_output_directory(
    ...
    if "coref" in outputDir and "coref" in str(annotator):
        ...
        if "table" in annotator:
            ...
            label = temp_dir
        outputDir = IO_files_util.make_output_subdirectory("", "", outputDir, label=label, silent=silent)
        ...
        outputDir = IO_files_util.make_output_subdirectory(
            inputFilename, inputDir, outputDir, label=annotator + "_" + NER_tag + "_CoreNLP", silent=silent
        )
    else:
        if outputDirSV != outputDir:
            # create output subdirectory
            outputDir = IO_files_util.make_output_subdirectory(
                inputFilename, inputDir, outputDir, label=annotator + "_CoreNLP", silent=silent
            )
        else:
            outputDir = IO_files_util.make_output_subdirectory(
                inputFilename, inputDir, outputDir, label=annotator + "_CoreNLP", silent=silent
    ...
    if export_json_var:
        if coref_outputDir != "":
            outputJsonDir = IO_files_util.make_output_subdirectory(
                inputFilename, inputDir, coref_outputDir, label="Json", silent=silent
            )
        else:
            outputJsonDir = IO_files_util.make_output_subdirectory(
...
def check_CoreNLP_annotator_availability(config_filename, annotator, language):
    ...
    if "lemma" in annotator:
        if language != "English":
            mb.showwarning(
                title=str(annotator).upper() + " annotator availability for " + language,
        ...
        if language != "English" and language != "Chinese":
            mb.showwarning(
                title=str(annotator).upper() + " annotator availability for " + language,
                message="The Stanford CoreNLP SENTIMENTT ANALYSIS annotator is only available for Chinese and English."
                + CoreNLP_web,
            )
            not_available = True
        ...
        if language != "English" and language != "Chinese":
            mb.showwarning(
                title=str(annotator).upper() + " annotator availability for " + language,
                message="The Stanford CoreNLP COREFERENCE RESOLUTION annotator is only available for Chinese and English."
                + CoreNLP_web,
            )
            not_available = True
        ...
        if language == "Arabic" or language == "Hungarian":
            mb.showwarning(
                title=str(annotator).upper() + " annotator availability for " + language,
                message="The Stanford CoreNLP NEURAL NETWORK PARSER is not available for Arabic and Hungarian."
                + CoreNLP_web,
            )
            not_available = True
    ...
    if not_available:
        ...
        reminder_status = reminders_util.checkReminder(
            scriptName, reminders_util.title_options_CoreNLP_website, reminders_util.message_CoreNLP_website, True
        )
        if reminder_status == "Yes" or reminder_status == "ON":  # 'Yes' the old way of saving reminders
            ...
            IO_libraries_util.open_url(
                website_name, url, ask_to_open=True, message_title=message_title, message=message
            )
...
def CoreNLP_annotate(
    ...
    **kwargs,
    ...
    CoreNLPdir, existing_software_config, errorFound = IO_libraries_util.external_software_install(
        "Stanford_CoreNLP_util", "Stanford CoreNLP", "", silent=False, errorFound=False
    )
    ...
    startTime = IO_user_interface_util.timed_alert(
        ...
        2000,
        "Analysis start",
        "Started running Stanford CoreNLP "
    ...
    if (
        ...
        reminders_util.checkReminder(
            scriptName, reminders_util.title_options_CoreNLP_SVO_timing, reminders_util.message_CoreNLP_SVO_timing, True
        )
    ...
    if "All POS" in str(annotator_params):
        reminders_util.checkReminder(
            scriptName, reminders_util.title_options_CoreNLP_POS_timing, reminders_util.message_CoreNLP_POS_timing, True
        )
    if "NER" in str(annotator_params):
        reminders_util.checkReminder(
            scriptName, reminders_util.title_options_CoreNLP_NER_timing, reminders_util.message_CoreNLP_NER_timing, True
        )
    ...
    for docName in inputDocs:
        ...
        split_file = file_splitter_ByLength_util.splitDocument_byLength(
            GUI_util.window, config_filename, docName, "", document_length
        )
        ...
        for doc_split in split_file:
            ...
            if param_string != "":
                ...
                exportJson(
                    export_json_var, tail, outputJsonDir, CoreNLP_output, language_encoding, annotator_params[0]
                )  # only one annotator
            else:
                CoreNLP_output = ""
            ...
            for run in routine_list:
                ...
                if parse_model == "NN" and model_switch == False:
                    ...
                    exportJson(
                        export_json_var, tail_split, outputJsonDir, CoreNLP_output, language_encoding, annotator_chosen
                    )
                ...
                if "parser" in annotator_chosen:
                    if "pcfg" in annotator_chosen:
                        sub_result, subtree_string, recordID = routine(
                ...
                # elif ("SVO" in str(annotator_params) or "OpenIE" in str(annotator_params)) and "coref" in docName.split("_"):
                    ...
                    sub_result = routine(
                        config_filename, split_docID, doc_split, sentenceID, CoreNLP_output, filter_subjects, **kwargs
                    )
                else:
                ...
                if output_format == "text":  # this type of output format is for 'coref' annotator only
                    ...
                    if sub_result != "":
                        with open(
                            outputFilename, "a+", encoding=language_encoding, errors="ignore"
                        ) as output_text_file:
        ...
        if isinstance(output_format[0], list):  # multiple outputs
            ...
            if annotator_chosen == "NER":
                ...
                try:
                    ...
                    outputFilename_tag = "ALL_NER"
                outputFilename = IO_files_util.generate_output_file_name(
                    inputFilename, inputDir, outputDir_chosen, ".csv", "CoreNLP_NER_" + outputFilename_tag
                )
            elif "parser" in annotator_chosen:
                if "pcfg" in annotator_chosen:
                    parser_label = "PCFG"
                    if len(subtree_string) > 0:
                        subtree_string.insert(
                            0, ["Clause tag", "String", "Indexes", "Sentence ID", "Document ID", "Document"]
                        )
                        subtree_string_fileName = outputDir_chosen + os.sep + "clausal_tags.csv"
                        IO_csv_util.list_to_csv(
                            GUI_util.window, subtree_string, subtree_string_fileName, encoding=language_encoding
                        )
                    ...
                    parser_label = "nn"
                outputFilename = IO_files_util.generate_output_file_name(
                    inputFilename, inputDir, outputDir_chosen, ".csv", "CoreNLP", parser_label, "CoNLL"
                )
                ...
                outputFilename = IO_files_util.generate_output_file_name(
                    inputFilename, inputDir, outputDir_chosen, ".csv", "CoreNLP_" + annotator_chosen
                )
            filesToOpen.append(outputFilename)
    ...
    speed_assessment.append(
        [-1, "Total Operation", total_time_elapsed, total_length, str(annotator_params), len(annotator_params)]
    )
...
def language_models(CoreNLPdir, language: str):
    if language == "English":
        ...
        if not os.path.isfile(language_file):
            ...
            for file in os.listdir(CoreNLPdir):
                if f"models-{language.lower()}.jar" in file:
                    ...
                    answer = tk.messagebox.askyesno(
                        title="Language pack",
                        message="You have selected to work with the "
                        + language.upper()
            ...
            if CoreNLP_version not in language_file:
                mb.showerror(
                    "Warning",
                    ...
                    + " for "
                    + language
                    + " in your Stanford CoreNLP directory\n\n"
                    + CoreNLPdir
                    + "\n\nis different from the Stanford CoreNLP version "
                    + str(CoreNLP_version)
...
def date_in_filename(document, **kwargs):
    ...
    if filename_embeds_date_var:
        date, date_str, int_month, int_day, int_year = IO_files_util.getDateFromFileName(
        ...
        for sentence in corenlp_output["sentences"]:
            ...
            for token in sentence["tokens"]:
                ...
                if token["lemma"].lower() in possessives or token["pos"] == "PRP$":
                    if "'s" not in output_word and output_word.lower() not in pronouns:
...
def process_json_SVO_enhanced_dependencies(
    config_filename, documentID, document, sentenceID, json, filter_subjects, **kwargs
):
    ...
    if quote_var:
        ...
        quote_df = quote_df.groupby(["Sentence ID", "Document ID"], as_index=False).agg(
            {"Speakers": lambda s: "; ".join(dict.fromkeys(str(x) for x in s)), "Number of Quotes": "max"}
        )
    ...
    if gender_var:
        SVO_df = pd.DataFrame(
            SVO_brief,
            columns=["Subject (S)", "Verb (V)", "Object (O)", "Sentence ID", "Sentence", "Document ID", "Document"],
        )
...
def exportJson(export_json_var, inputFilename, outputJsonDir, CoreNLP_output, language_encoding, annotator_params):
...
def visualize_GIS_maps(kwargs, locations, documentID, document, date_str):
    ...
    for sent in locations:
        if ("extract_date_from_text_var" in kwargs and kwargs["extract_date_from_text_var"] == True) or (
...
def check_pronouns(
    ...
    if total_count > 0:
        if option != "coref table":
            head, scriptName = os.path.split(os.path.basename(__file__))
            reminders_util.checkReminder(
                scriptName, reminders_util.title_options_CoreNLP_pronouns, reminders_util.message_CoreNLP_pronouns, True
            )
            return return_files
        else:
            ...
            with open(outputFilename, "w", newline="", encoding="utf-8", errors="ignore") as csvFile:
                ...
                writer.writerow(["Number of Pronouns", "Number of Coreferenced Pronouns", "Pronouns Coreference Rate"])
            ...
            if chartPackage != "No charts":
                ...
                outputFiles = charts_util.plot(
                    ...
                    title="Coreferenced Pronouns",
                    x_label="Coreference values",
                    count=0,
                    group_by=None,
                )
                if outputFiles != None:
