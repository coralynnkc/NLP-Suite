...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "statistics_corpus_word_frequency_util", ["os", "tkinter", "pandas", "numpy", "re"]
    )
    == False
):
    sys.exit(0)
...
def compute_word_frequency(
    inputFilename, inputDir, outputDir, chartPackage="Excel", dataTransformation="No transformation", top_n=50
):

    filesToOpen = []

    outputDir = IO_files_util.make_output_subdirectory(
        inputFilename, inputDir, outputDir, label="word_frequency", silent=True
    )
    if outputDir == "":
        return filesToOpen

    startTime = IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis start", "Started running Word Frequency Distribution at", True
    )
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        inputFilename, inputDir, outputDir, ".csv", "word_frequency_distribution", "", "", "", "", False, True
    )
    ...
    if chartPackage != "No charts":
        ...
        axes[1].scatter(df["Log Rank"].values, df["Log Frequency"].values, s=3, alpha=0.4, color="#E24B4A")
        max_rank = df["Log Rank"].max()
        max_freq = df["Log Frequency"].max()
        ideal_x = np.linspace(0, max_rank, 100)
        ideal_y = max_freq - ideal_x
        axes[1].plot(
            ideal_x, ideal_y, "--", color="#333333", alpha=0.6, linewidth=1.5, label="Zipf's ideal (slope = -1)"
        )
        ...
        axes[2].annotate(f"{n50} words = 50%", xy=(n50, 50), fontsize=8, xytext=(n50 + len(df) * 0.05, 45))
        axes[2].annotate(f"{n90} words = 90%", xy=(n90, 90), fontsize=8, xytext=(n90 + len(df) * 0.05, 85))
        ...
        summary_file = IO_files_util.generate_output_file_name(
            inputFilename, inputDir, outputDir, ".csv", "word_frequency_summary", "", "", "", "", False, True
        )
    ...
    IO_user_interface_util.timed_alert(
        ...
        2000,
        "Analysis end",
        "Finished running Word Frequency Distribution at",
        True,
        "",
        True,
        startTime,
