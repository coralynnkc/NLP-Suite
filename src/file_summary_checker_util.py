...
if (
    IO_libraries_util.install_all_Python_packages(GUI_util.window, "Summary checker", ["nltk", "os", "tkinter", "glob"])
    == False
):
    sys.exit(0)
...
def get_comp_soc_actors(id, soc_acts, c_path, nlp, checkNER):
    ...
    try:
        with open(my_path, encoding="utf-8", errors="ignore") as fh:
            fcontent = fh.read()
    except OSError:
        print("")
