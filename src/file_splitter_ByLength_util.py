...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "File splitter by TOC", ["os", "io", "re", "ntpath", "tkinter", "shutil", "stanza"]
    )
    == False
):
    sys.exit(0)
...
def split_byLength(window, input_path, filename, output_path, maxLength, inSentence=False):
    ...
    if maxLength > len_sentences:
        IO_user_interface_util.timed_alert(
            window, 2000, "File split warning", "The length of file " + filename + " is less than " + str(maxLength)
        )
        ...
        if l + len(words) < maxLength:
            ...
            if inSentence:  # the subfile's word count is less than max limit, but contain no incomplete sentences
                ...
                if (
                    ...
                    restsent = sent
                    while (
                        len(tokenize_stanza_text(stanzaPipeLine(text))) <= diff
