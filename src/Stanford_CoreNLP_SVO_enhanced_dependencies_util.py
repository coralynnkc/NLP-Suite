...
def verb_root(
    verb_list, conj_word, token, sent_data, v_obj_obl_json, v_prep_json
...
def linking_verb_LVC_extraction(token, gov_dict, sent_data, linking_verb_LVC_json):
    ...
    if (
        token["lemma"] in linking_verb_LVC_json.keys()
...
def SVO_extraction(sent_data, entitymentions):  # returns columns of the final output
    ...
    v_obj_obl_text = (
        GUI_IO_util.CoreNLP_enhanced_dependencies_libPath + os.sep + "LVC_verb_obj_obl_json.txt"
    )  # json that help with extracting certain light verb constructions as a whole verb
    linking_verb_text = (
        GUI_IO_util.CoreNLP_enhanced_dependencies_libPath + os.sep + "linking_verb_LVC_json.txt"
    )  # json that help with extracting object that follows a preposition
    ...
    for key in sent_data.keys():  # traverse each token in that sentence
        ...
        if (
            ...
            if key not in CollectedVs:
                ...
                svo_verb, negation_verb = verb_root(
                    verb_list, conj_word, token, sent_data, v_obj_obl_json, v_prep_json
                )  # processing
