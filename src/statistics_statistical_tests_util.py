...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "statistics_statistical_tests_util", ["os", "tkinter", "pandas", "numpy", "scipy"]
    )
    == False
):
    sys.exit(0)
...
def _save_results_csv(results_df, inputFilename, inputDir, outputDir, label):
    outputFilename = IO_files_util.generate_output_file_name(inputFilename, inputDir, outputDir, ".csv", "", label)
...
def _mann_kendall_test(x):
    ...
    return {"tau": tau, "p_value": p_value, "slope": slope, "intercept": intercept, "trend": trend, "z": z, "S": s}
...
def run_chi_square_test(
    inputFilename, outputDir, col1, col2, chartPackage="Excel", dataTransformation="No transformation"
):
    filesToOpen = []
    ...
    for col in [col1, col2]:
        if col not in df.columns:
            mb.showwarning(title="Column error", message='Column "' + col + '" not found in the input file.')
            return filesToOpen

    df = df[[col1, col2]].dropna()
    if len(df) < 5:
        mb.showwarning(title="Insufficient data", message="At least 5 rows are needed for the Chi-Square test.")
        return filesToOpen

    ct = pd.crosstab(df[col1], df[col2])
    if ct.shape[0] < 2 or ct.shape[1] < 2:
        mb.showwarning(
            title="Insufficient categories", message="Both columns need at least 2 unique values for a Chi-Square test."
        )
        return filesToOpen
    ...
    if min_expected < 5:
        IO_user_interface_util.timed_alert(
            ...
            4000,
            "Chi-Square warning",
            "Some expected frequencies are below 5 (min="
            + str(round(min_expected, 2))
    ...
    outputFiles = charts_util.run_all(
        ...
        chart_csv,
        outputDir,
        outputFileLabel="chi2",
        chartPackage=chartPackage,
        dataTransformation=dataTransformation,
        chart_type_list=["bar"],
        chart_title="Chi-Square Test: Observed vs Expected\n" + col1 + " x " + col2,
        column_xAxis_label_var=col1,
        column_yAxis_label_var="Count",
...
def run_crosstab(inputFilename, outputDir, col1, col2, chartPackage="Excel", dataTransformation="No transformation"):
...
def run_mann_whitney_test(
    inputFilename, outputDir, value_col, group_col, chartPackage="Excel", dataTransformation="No transformation"
):
    filesToOpen = []
    ...
    if len(labels) < 2:
        mb.showwarning(title="Group error", message='Column "' + group_col + '" must have at least 2 unique values.')
        return filesToOpen
    if len(labels) > 2:
        IO_user_interface_util.timed_alert(
            GUI_util.window,
            4000,
            "Mann-Whitney warning",
    ...
    if len(group_a) < 3 or len(group_b) < 3:
        mb.showwarning(title="Insufficient data", message="Each group needs at least 3 observations.")
        return filesToOpen
    ...
    outputFiles = charts_util.run_all(
        ...
        + ", p="
        + str(round(p, 4)),
        column_xAxis_label_var="Group",
        column_yAxis_label_var=value_col,
        hover_info_column_list=[],
    )
    _append_chart_files(filesToOpen, outputFiles)
...
def run_kruskal_wallis_test(
    ...
    for i, g in enumerate(groups):
        if len(g) < 2:
            mb.showwarning(
                title="Insufficient data", message='Group "' + str(labels[i]) + '" has fewer than 2 observations.'
            )
            return filesToOpen
    ...
    chart_data = pd.DataFrame(
        {"Group": [str(lbl) for lbl in labels], "Median": [round(np.median(g), 4) for g in groups]}
    )
    ...
    outputFiles = charts_util.run_all(
        ...
        chart_csv,
        outputDir,
        outputFileLabel="kruskal_wallis",
        chartPackage=chartPackage,
        dataTransformation=dataTransformation,
        chart_type_list=["bar"],
        ...
        + ", p="
        + str(round(p, 4)),
        column_xAxis_label_var=group_col,
        column_yAxis_label_var=value_col,
        hover_info_column_list=[],
    )
    _append_chart_files(filesToOpen, outputFiles)
...
def run_mann_kendall_trend_test(
    inputFilename, outputDir, date_col, value_col, chartPackage="Excel", dataTransformation="No transformation"
):
    filesToOpen = []
    ...
    try:
        ...
        mb.showwarning(title="Date parse error", message='Could not parse dates in column "' + date_col + '".')
        return filesToOpen
    ...
    if len(df) < 8:
        mb.showwarning(
            title="Insufficient data", message="At least 8 observations are needed for a reliable Mann-Kendall test."
        )
        return filesToOpen
    ...
    if len(df) > 5000:
        IO_user_interface_util.timed_alert(
            ...
            4000,
            "Performance warning",
            "Large dataset (" + str(len(df)) + " rows). The Mann-Kendall test may take a moment.",
            False,
        )
    ...
    summary_data = {
        ...
        "Value": [
            ...
            round(mk["p_value"], 6),
            round(mk["slope"], 6),
            round(mk["intercept"], 4),
            mk["trend"],
            mk["S"],
            round(mk["z"], 4),
            "Yes" if mk["p_value"] < 0.05 else "No",
            len(values),
        ],
    }
    summary_df = pd.DataFrame(summary_data)
    ...
    outputFiles = charts_util.run_all(
        ...
        + str(round(mk["tau"], 3))
        + ", p="
        + str(round(mk["p_value"], 4))
        + " ("
        + mk["trend"]
        + ")",
        column_xAxis_label_var=date_col,
        column_yAxis_label_var=value_col,
        hover_info_column_list=[],
    )
    _append_chart_files(filesToOpen, outputFiles)
...
def run_log_likelihood_test(
    ...
    if word_col not in df.columns:
        mb.showwarning(title="Column error", message='Column "' + word_col + '" not found in the input file.')
        return filesToOpen

    # Mode B: pivot by corpus column
    if corpus_col is not None and corpus_col in df.columns:
        if freq_col1 not in df.columns:
            mb.showwarning(title="Column error", message='Frequency column "' + freq_col1 + '" not found.')
            return filesToOpen
        pivot = df.pivot_table(index=word_col, columns=corpus_col, values=freq_col1, aggfunc="sum", fill_value=0)
        corpus_labels = list(pivot.columns)
        if len(corpus_labels) < 2:
            mb.showwarning(title="Corpus error", message='Need at least 2 corpus groups in "' + corpus_col + '".')
            return filesToOpen
        if len(corpus_labels) > 2:
            IO_user_interface_util.timed_alert(
                GUI_util.window,
                4000,
                "Log-likelihood warning",
    ...
    if total_a == 0 or total_b == 0:
        mb.showwarning(title="Empty corpus", message="One of the corpora has zero total frequency.")
        return filesToOpen
    ...
    if len(chart_df) > 0:
        ...
        outputFiles = charts_util.run_all(
            ...
            chart_csv,
            outputDir,
            outputFileLabel="log_likelihood",
            chartPackage=chartPackage,
            dataTransformation=dataTransformation,
            chart_type_list=["bar"],
            chart_title="Log-Likelihood Ratio (G2)\nCorpus Comparison: " + label_a + " vs " + label_b,
            column_xAxis_label_var="Word",
            column_yAxis_label_var="Log Ratio",
            hover_info_column_list=[],
        )
        _append_chart_files(filesToOpen, outputFiles)
...
def run_correlation_test(
    ...
    if len(df) < 5:
        mb.showwarning(title="Insufficient data", message="At least 5 observations are needed for a correlation test.")
        return filesToOpen

    if df[col_x].std() == 0 or df[col_y].std() == 0:
        mb.showwarning(
            title="Constant column", message="One of the columns has zero variance. Correlation is undefined."
        )
        return filesToOpen
    ...
    outputFiles = charts_util.run_all(
        ...
        chart_csv,
        outputDir,
        outputFileLabel="correlation",
        chartPackage=chartPackage,
        dataTransformation=dataTransformation,
        chart_type_list=["line"],
        chart_title=method_label + " Correlation\nr=" + str(round(coef, 3)) + ", p=" + str(round(p, 4)),
        column_xAxis_label_var=col_x,
        column_yAxis_label_var=col_y,
        hover_info_column_list=[],
    )
    _append_chart_files(filesToOpen, outputFiles)
...
def run_kappa_test(inputFilename, outputDir, rater_cols, chartPackage="Excel", dataTransformation="No transformation"):
    ...
    for col in rater_cols:
        if col not in df.columns:
            mb.showwarning(title="Column error", message='Column "' + str(col) + '" not found in the input file.')
            return filesToOpen
    ...
    if len(df) < 2:
        mb.showwarning(title="Insufficient data", message="At least 2 items (rows) rated by all annotators are needed.")
        return filesToOpen
    ...
    if n_raters == 2:
        ...
        summary_df = pd.DataFrame(
            {
                "Statistic": [
                    ...
                    "Kappa",
                    "Interpretation",
                    "Mean observed agreement (P-bar)",
                    "Expected agreement (Pe-bar)",
                    "N items",
                    "N annotators",
                    "N categories",
                    "Annotators",
        ...
        dist_df = pd.DataFrame(
            {"Category": [str(c) for c in categories], "Proportion": [round(float(p), 4) for p in p_j]}
        )
    ...
    chart_df = pd.DataFrame(
        {"Agreement type": ["Observed", "Expected (chance)"], "Proportion": [round(po_obs, 4), round(pe_chance, 4)]}
    )
    ...
    outputFiles = charts_util.run_all(
        ...
        chart_csv,
        outputDir,
        outputFileLabel="kappa",
        chartPackage=chartPackage,
        dataTransformation=dataTransformation,
        chart_type_list=["bar"],
        chart_title=method_label
        + ": kappa="
        + str(round(kappa, 3))
        + " ("
        + interp
        ...
        column_xAxis_label_var="Agreement type",
        column_yAxis_label_var="Proportion",
        hover_info_column_list=[],
    )
    _append_chart_files(filesToOpen, outputFiles)
...
def run_change_point_test(
    ...
    if len(df) < 10:
        mb.showwarning(
            title="Insufficient data", message="At least 10 observations are needed for change-point detection."
        )
        return filesToOpen
    ...
    summary_df = pd.DataFrame(
        {
            ...
            "Value": [
                ...
                round(mean_before, 4),
                round(mean_after, 4),
                round(mean_after - mean_before, 4),
                len(values),
            ],
        }
    )
    ...
    chart_df = pd.DataFrame({date_col: x_axis, value_col: values, "Segment mean": np.round(seg_mean, 4)})
...
def run_permutation_test(
    ...
    pooled_sd = (
        np.sqrt(((len(a) - 1) * np.var(a, ddof=1) + (len(b) - 1) * np.var(b, ddof=1)) / (len(a) + len(b) - 2))
        if (len(a) + len(b) - 2) > 0
        else 0.0
    )
    ...
    chart_df = pd.DataFrame({"Group": [str(labels[0]), str(labels[1])], "Mean": [round(mean_a, 4), round(mean_b, 4)]})
    ...
    outputFiles = charts_util.run_all(
        ...
        + str(labels[1])
        + "\nobserved diff="
        + str(round(observed_diff, 3))
        + ", p="
        + str(round(float(p_value), 4)),
        column_xAxis_label_var="Group",
        column_yAxis_label_var=value_col,
        hover_info_column_list=[],
    )
...
# ---------------------------------------------------------------------------
def run_adjusted_rand_test(
    inputFilename, outputDir, labels_col1, labels_col2, chartPackage="Excel", dataTransformation="No transformation"
):
    ...
    if len(df) < 2:
        mb.showwarning(title="Insufficient data", message="At least 2 items (rows) labeled in BOTH columns are needed.")
        return filesToOpen
    ...
    summary_df = pd.DataFrame(
        {
            ...
            "Value": [
                ...
                round(nmi, 4),
                len(df),
                int(df[labels_col1].nunique()),
                int(df[labels_col2].nunique()),
            ],
        }
    )
...
def run_bayes_factor_test(
    ...
    df = df.dropna()
    if df.empty:
        mb.showwarning(title="No numeric data", message='Column "' + value_col + '" has no numeric values to test.')
        return filesToOpen
    labels = sorted(df[group_col].unique())
    if len(labels) < 2:
        mb.showwarning(title="Group error", message='Column "' + group_col + '" must have at least 2 unique values.')
    ...
    summary_df = pd.DataFrame(
        {
            "Statistic": [
                "Bayes factor BF10 (difference vs none)",
                "Interpretation",
                "Bayes factor BF01 (none vs difference)",
                "Cohen's d (effect size)",
                "t statistic (pooled)",
                "p-value (Welch, for reference)",
                "Group A",
                "Group A n",
                "Group A mean",
                "Group B",
            ...
            "Value": [
                round(bf10, 4) if bf10 == bf10 else "n/a",
                _bf_interp(bf10),
                round(bf01, 4) if bf01 == bf01 else "n/a",
                round(cohens_d, 4),
                round(t_student, 4),
                round(p_welch, 6),
                str(labels[0]),
...
def run_silhouette_test(
    inputFilename, outputDir, label_col, feature_cols=None, chartPackage="Excel", dataTransformation="No transformation"
):
    ...
    if label_col not in df.columns:
        mb.showwarning(title="Column error", message='Cluster-label column "' + str(label_col) + '" not found.')
        return filesToOpen
    ...
    if len(keep) < 3:
        mb.showwarning(
            title="Insufficient data", message="Silhouette needs 3+ rows with a label and complete numeric features."
        )
        return filesToOpen
    ...
    per = (
        pd.DataFrame({"Cluster": labels, "_s": samp})
        .groupby("Cluster", as_index=False)
        .agg(Mean_silhouette=("_s", "mean"), N=("_s", "size"))
    ...
    try:
        outputFiles = charts_util.run_all(
            ...
            out2,
            outputDir,
            outputFileLabel="silhouette",
            chartPackage=chartPackage,
            dataTransformation=dataTransformation,
            chart_type_list=["bar"],
            chart_title="Silhouette by cluster (overall=" + str(round(overall, 3)) + ")",
            column_xAxis_label_var="Cluster",
            column_yAxis_label_var="Mean silhouette",
            hover_info_column_list=[],
        )
        _append_chart_files(filesToOpen, outputFiles)
...
def run_automatic_tests(inputFilename, outputDir, chartPackage="Excel", dataTransformation="No transformation"):
    filesToOpen = []
        ...
        for val_col in numeric_cols[:3]:
            try:
                results = run_mann_kendall_trend_test(
                    inputFilename, outputDir, date_col, val_col, chartPackage, dataTransformation
                )
    ...
    if group_col:
        freq_cols = [c for c in numeric_cols if any(k in c.lower() for k in ("freq", "count", "total"))]
        if freq_cols:
                ...
                if other_cat:
                    try:
                        results = run_chi_square_test(
                            inputFilename, outputDir, group_col, other_cat, chartPackage, dataTransformation
                        )
                        filesToOpen.extend(results)
                    except:
                        pass
