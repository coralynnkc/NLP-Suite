...
def attribute_space_crosstab(observations, classify=True, drop_unclassified=True, drop_unknown_attribute=True):
    ...
    if classify:
        ...
        pairs = [((str(a) if a is not None else "unknown").strip().lower(), c) for a, c in observations]
...
def plot_attribute_space_heatmap(
    table, output_path, values="residuals", stats=None, title="Attribute × non-geocodable space"
):
    ...
    fig, ax = plt.subplots(figsize=(max(6, data.shape[1] * 1.15), max(3, data.shape[0] * 0.9)))
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            v = data.values[i, j]
            ax.text(
                j, i, (f"{v:.1f}" if values == "residuals" else int(v)), ha="center", va="center", fontsize=8
            )
    ttl = title
    if stats and stats.get("cramers_v") is not None:
        ttl += "  (Cramer's V = {:.2f}, p = {:.3g})".format(stats["cramers_v"], stats["p_value"])
...
def narrative_transitions(sequence, classify=True, collapse_repeats=True, keep_unclassified=False):
...
def plot_transition_graph(edges, output_path, title="Movement through non-geocodable space"):
    ...
    for i, name in enumerate(nodes):
        ...
        pos[name] = (math.cos(ang) if n > 1 else 0.0, math.sin(ang) if n > 1 else 0.0)
    ...
    for a, b, c in edges:
        ...
        ax.text((x1 + x2) / 2, (y1 + y2) / 2, str(c), fontsize=8, color="#b00000", ha="center", va="center")
    for name, (x, y) in pos.items():
        ax.scatter([x], [y], s=2000, c="#cfe3f5", edgecolors="#33667a", linewidths=1.5, zorder=3)
    ...
    for (doc, sent), g in df.groupby(["Document ID", "Sentence ID"], sort=False):
        ...
        for _, r in g.iterrows():
            try:
                ...
                continue
            tok[tid] = {"form": r["Form"], "lemma": r["Lemma"], "pos": r["POS"], "head": head, "deprel": r["DepRel"]}
