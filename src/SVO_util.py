...
def visualize_SVOs(fileName, outputDir, chartPackage, dataTransformation, filesToOpen, openFiles):
    ...
    outputFiles = charts_util.plot(
        ...
        outputDir,
        columns=["Subject (S)"],
        title="Frequency Distribution of Subjects (" + label + ")",
        x_label="Subjects (" + label + ")",
        file_label="S-" + label1,
        plot_list=["Frequency"],
        title_label="Subjects (" + label + ")",
    )
    ...
    outputFiles = charts_util.plot(
        ...
        outputDir,
        columns=["Verb (V)"],
        title="Frequency Distribution of Verbs (" + label + ")",
        x_label="Verbs (" + label + ")",
        file_label="V-" + label1,
        plot_list=["Frequency"],
        title_label="Verbs (" + label + ")",
    )
    if openFiles and outputFiles != None:
        if isinstance(outputFiles, str):
            filesToOpen.append(outputFiles)
    ...
    outputFiles = charts_util.plot(
        ...
        outputDir,
        columns=["Object (O)"],
        title="Frequency Distribution of Objects (" + label + ")",
        x_label="Objects (" + label + ")",
        file_label="O-" + label1,
        plot_list=["Frequency"],
        title_label="Objects (" + label + ")",
    )
...
def lemmatize_filter_svo_old(
    ...
    if lemmatize_s or lemmatize_v or lemmatize_o:
        ...
        if filter_s or filter_v or filter_o:
            ...
            if filter_s:
                s_filtered_set = open(filter_s_fileName, encoding="utf-8-sig", errors="ignore").read().split("\n")
                s_filtered_set = set(s_filtered_set)
            if filter_v:
                v_filtered_set = open(filter_v_fileName, encoding="utf-8-sig", errors="ignore").read().split("\n")
                v_filtered_set = set(v_filtered_set)
            if filter_o:
                o_filtered_set = open(filter_o_fileName, encoding="utf-8-sig", errors="ignore").read().split("\n")
    ...
    for i in range(num_rows):  # num_rows in the unfiltered SVO
        ...
        if not pd.isna(unfiltered_svo[i]["Subject (S)"]):
            if lemmatize_s:
                lemmatized_svo[i]["Subject (S)"] = lemmatize_stanza_word(
                    stanzaPipeLine(unfiltered_svo[i]["Subject (S)"])
                )
                # lemmatized_svo.add(i,lemmatize_stanza_word(stanzaPipeLine(unfiltered_svo[i]['Subject (S)'])))
                if (filter_s and filter_s_fileName != "") and (unfiltered_svo[i]["Subject (S)"] not in s_filtered_set):
                    del filtered_svo[i]
        ...
        if not pd.isna(unfiltered_svo[i]["Verb (V)"]):
            if lemmatize_v:
                ...
                if (filter_v and filter_v_fileName != "") and (unfiltered_svo[i]["Verb (V)"] not in v_filtered_set):
                    if not deleted:
                        del filtered_svo[i]
                        # del lemmatized_filtered_svo[i]
                        deleted = True
        ...
        if not pd.isna(unfiltered_svo[i]["Object (O)"]):
            if lemmatize_o:
                ...
                if (filter_o and filter_o_fileName != "") and (unfiltered_svo[i]["Object (O)"] not in o_filtered_set):
                    if not deleted:
                        del filtered_svo[i]
                        # del lemmatized_filtered_svo[i]
                else:
                    ...
                    if not deleted and lemmatize_o and filtered_svo != {}:
                        filtered_svo[i]["Object (O)"] = lemmatize_stanza_word(
                            stanzaPipeLine(filtered_svo[i]["Object (O)"])
                        )
    ...
    if lemmatize_s or lemmatize_v or lemmatize_o:
        ...
        filesToOpen.append(svo_lemma_file_name)
        # save lemmatized file
        pd.DataFrame.from_dict(lemmatized_svo, orient="index").to_csv(
            svo_lemma_file_name, encoding="utf-8", index=False
        )
        nRecords_lemma, nColumns = IO_csv_util.GetNumberOf_Records_Columns_inCSVFile(svo_lemma_file_name)
        ...
        if filter_s or filter_v or filter_o:
            ...
            #                                                               index=False)
            # save filtered file
            pd.DataFrame.from_dict(filtered_svo, orient="index").to_csv(
                svo_filter_file_name, encoding="utf-8", index=False
            )
...
def normalize_date_svo(inputFilename, outputDir, chartPackage="Excel", dataTransformation="No transformation"):
    ...
    outputFiles = charts_util.plot(
        ...
        x_label="Date expression (includes " + nEmtyCells + " SVOs with no date)",
    ...
    outputFiles = charts_util.plot(
        ...
        x_label="Normalized date (includes " + nEmtyCells + " SVOs with no date)",
        file_label="date",
        plot_list=["Frequency"],
    ...
    outputFiles = charts_util.plot(
        ...
        plot_list=["Frequency"],
        title_label="Date Types (includes " + nEmtyCells + " SVOs with no date)",
    )
    if outputFiles != None:
        if isinstance(outputFiles, str):
            filesToOpen.append(outputFiles)
