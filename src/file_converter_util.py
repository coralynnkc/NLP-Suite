...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "file_converter_util", ["os", "__main__", "tkinter", "docx", "pdfminer", "striprtf", "errno"]
    )
    == False
):
    sys.exit(0)
...
def pdf_converter(
    ...
    if len(inputDir) > 0:
        msgbox_subDir = tk.messagebox.askyesnocancel(
    ...
    IO_user_interface_util.timed_alert(
        ...
        4000,
        "Analysis end",
        "Finished running pdf converter at",
        True,
        str(numberOfDocs)
        + " files were successfully converted from pdf to txt format and saved in directory "
...
def docx_converter(
    ...
    startTime = IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis start", "Started running docx to txt converter at", True, "", True, "", False
    )
    ...
    if len(inputDir) > 0:
        msgbox_subDir = tk.messagebox.askyesnocancel(
            "Process sub-directories", "Do you want to process files in subdirectories?"
        )
    if msgbox_subDir:
        ...
        inputDocs = IO_files_util.getFileList(
            inputFilename, inputDir, fileType=".docx", silent=False, configFileName=config_filename
        )
    nDocs = len(inputDocs)
    ...
    outputDir = IO_files_util.make_output_subdirectory(
        inputFilename, inputDir, outputDirSV, label="docx_2_txt", silent=True
    )
    ...
    IO_user_interface_util.timed_alert(
        ...
        2000,
        "Analysis end",
        "Finished running docx to txt converter at",
        True,
        str(numberOfDocs) + " txt files exported to the output subdirectory " + outputDir,
        True,
        startTime,
        False,
    )
...
def csv_converter(
    ...
    if inputFilename != "":
        ...
        if inputDir != "":
            tk.messagebox.showinfo(
                "csv converter",
                "No input filename. The csv converter works only on a single csv file, rather than a whole directory. Please, select an input csv file and try again.",
            )
            return
        else:
            tk.messagebox.showinfo(
                "csv converter", "No input filename. Please, select an input csv file and try again."
            )
            return
...
def rtf_converter(
    ...
    for docNum, doc in enumerate(inputRTFs):
        ...
        if fileExtension == "rtf":
            ...
            with open(doc, encoding="utf-8", errors="ignore") as rtf_fh:
