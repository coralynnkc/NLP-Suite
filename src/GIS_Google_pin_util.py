...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "GIS_Google_pin_util", ["os", "simplekml", "tkinter"]
    )
    == False
):
    sys.exit(0)
...
def pin_customizer(
    ...
    if description_var_list[j] == 1:
        if len(description_csv_field_var_list[j]) == 0:
            mb.showwarning(
                title="No CSV Field Selected for Description for Group No." + str(j + 1),
                message="The description checkbox is ticked but no csv field was selected for the Group No."
                + str(j + 1)
        ...
        if group_var == 1:
            if len(group_labels[j]) < 1:
                mb.showwarning(
                    title="No Group Labels Specified for Group No." + str(j + 1),
                    message="There is no group label specified for Group No."
                    + str(j + 1)
