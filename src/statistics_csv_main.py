...
def run():
    ...
    if all_csv_stats or csv_field_freq:
        ...
        if csv_field_freq and len(csv_list) == 0:
            mb.showwarning(
                title="Warning",
                message="You have selected to compute the frequency of a csv file field but no field has been selected.\n\nPlease, select a csv file field and try again.",
            )
            return
    ...
    if all_csv_stats:
        ...
        outputFiles = statistics_csv_util.compute_csv_column_statistics_NoGroupBy(
            window, inputFilename, outputDir, openOutputFiles, chartPackage, dataTransformation
        )
        if outputFiles:
                ...
                if stat_value_col == "" or stat_group_col == "":
                    ...
                    if n_groups >= 3:
                        outputFiles = statistics_statistical_tests_util.run_kruskal_wallis_test(
                            csv_file, outputDir, stat_value_col, stat_group_col, chartPackage, dataTransformation
                        )
                    else:
            ...
            if run_mk:
                if stat_value_col == "" or stat_group_col == "":
                    ...
                    outputFiles = statistics_statistical_tests_util.run_mann_kendall_trend_test(
                        csv_file, outputDir, stat_group_col, stat_value_col, chartPackage, dataTransformation
                    )
                    if outputFiles:
                        filesToOpen.extend(outputFiles)
            ...
            if run_perm:
                if stat_value_col == "" or stat_group_col == "":
                    ...
                    outputFiles = statistics_statistical_tests_util.run_permutation_test(
                        csv_file, outputDir, stat_value_col, stat_group_col, 10000, chartPackage, dataTransformation
                    )
                    if outputFiles:
                        filesToOpen.extend(outputFiles)
            ...
            if run_kappa:
                ...
                if len(rater_cols) < 2:
                    ...
                    outputFiles = statistics_statistical_tests_util.run_kappa_test(
                        csv_file, outputDir, rater_cols, chartPackage, dataTransformation
                    )
                    if outputFiles:
                        filesToOpen.extend(outputFiles)
            ...
            if run_sil:
                if stat_group_col == "":
                    ...
                    outputFiles = statistics_statistical_tests_util.run_silhouette_test(
                        csv_file, outputDir, stat_group_col, None, chartPackage, dataTransformation
                    )
                    if outputFiles:
                        filesToOpen.extend(outputFiles)
...
all_csv_stats_var.set(0)
all_csv_field_checkbox = tk.Checkbutton(
    window,
    text="Compute statistics on all csv-file fields (numeric fields only)",
    variable=all_csv_stats_var,
    onvalue=1,
    offvalue=0,
    command=lambda: get_script_to_run("Compute statistics on all csv-file fields (numeric fields only)"),
)
...
csv_field_freq_var.set(0)
csv_field_checkbox = tk.Checkbutton(
    window,
    text="Compute frequencies of csv-file field(s)",
    variable=csv_field_freq_var,
    onvalue=1,
    offvalue=0,
...
add_csv_field_button = tk.Button(
    window,
    text="+",
    width=GUI_IO_util.add_button_width,
    height=1,
    state="normal",
    command=lambda: activate_viewer_options(),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.open_TIPS_x_coordinate + 20,
    y_multiplier_integer,
    add_csv_field_button,
    True,
    False,
    False,
    False,
...
menu_values = [""]
reset_csv_button = tk.Button(
    window,
    text="Reset ",
    width=GUI_IO_util.reset_button_width,
    height=1,
    state="disabled",
    command=lambda: reset_csv_list(),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.statistics_csv_reset_csv_button_pos,
    y_multiplier_integer,
    reset_csv_button,
    True,
    False,
...
show_csv_button = tk.Button(
    window,
    text="Show",
    width=GUI_IO_util.show_button_width,
    height=1,
    state="disabled",
    command=lambda: show_csv_list(),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.statistics_csv_show_csv_button_pos,
    y_multiplier_integer,
    show_csv_button,
    True,
    False,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.statistics_csv_csv_field_lb_pos, y_multiplier_integer, csv_field_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.open_reminders_x_coordinate + 110,
    y_multiplier_integer,
    csv_field_menu,
    False,
    False,
...
def show_csv_list():
    if len(csv_list) == 0:
        ...
        mb.showwarning(
            title="Warning",
            message="The currently selected csv field options are:\n"
            "\n   CSV FIELD: "
            + ", ".join(csv_list)
            + "\n   GROUP-BY FIELD(S): "
            + ", ".join(groupBy_list)
            + "\n   HOVER-OVER FIELD(S): "
            + ", ".join(hover_over_list)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.statistics_csv_csv_groupBy_field_lb_pos, y_multiplier_integer, csv_groupBy_field_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.statistics_csv_csv_groupBy_field_menu_pos,
    y_multiplier_integer,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_setup_x_coordinate, y_multiplier_integer, csv_hover_over_field_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.open_setup_x_coordinate + 120,
    y_multiplier_integer,
...
def activate_plus2(*args):
    if csv_hover_over_field_var.get() in hover_over_list:
        mb.showwarning(
            title="Warning",
            message='The csv field "'
            + csv_hover_over_field_var.get()
...
stat_test_checkbox = tk.Checkbutton(window, text="Hypothesis tests", variable=stat_test_var, onvalue=1, offvalue=0)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.labels_x_coordinate,
    y_multiplier_integer,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, stat_value_col_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_reminders_x_coordinate, y_multiplier_integer, stat_group_col_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    False,
    90,
    GUI_IO_util.statistics_csv_csv_groupBy_field_lb_pos,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, stat_word_col_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_reminders_x_coordinate, y_multiplier_integer, stat_freq_col1_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_reminders_x_coordinate, y_multiplier_integer, stat_freq_col2_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, stat_corpus_col_lb, True
)
...
TIPS_options = (
    "Statistical tools in the NLP Suite",
