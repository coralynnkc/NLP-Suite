...
if (
    IO_libraries_util.install_all_Python_packages(GUI_util.window, "GIS", ["os", "tkinter", "tkcolorpicker", "PIL"])
    == False
):
    sys.exit(0)
...
def run():
    ...
    if not inputIsGeocoded:
        if locationColumnName == "" and (not inputIsGeocoded):
            mb.showwarning(
                title="No location column selected",
                message="No csv column containing location names has been selected.\n\nPlease, select a column and try again.",
            )
            return
    ...
    if group_var == 1 and group_number_var == 1:
        mb.showwarning(
            title="Only One Group Chose for Multiple Group Mapping",
            message="The group box is ticked for multiple groups mapping but only one group is chose.\n\nPlease, check your input and try again.",
        )
        return
    ...
    if group_var == 1 and len(icon_csv_field_var_name) < 1:
        mb.showwarning(
            title="No CSV field for Group Split Criterion Found",
            message="The group box is ticked for multiple groups mapping but no csv field is specified for group splitting criterion.\n\nPlease, check your input and try again.",
        )
        return
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, encodingValue, True
)
encoding_lb = tk.Label(window, text="Select encoding type (utf-8 default)")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, encoding_lb
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, location_field_lb, True
)
if menu_values != "":
    ...
    location_field = tk.OptionMenu(window, location_var, menu_values)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, location_field
)

date_field_lb = tk.Label(window, text="Column containing date")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, date_field_lb, True
)
if menu_values != "":
    ...
    date_field = tk.OptionMenu(window, date_var, menu_values)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, date_field, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.setup_IO_brief_coordinate, y_multiplier_integer, date_format_lb, True
)
date_format_menu = tk.OptionMenu(
    window, date_format_var, "mm-dd-yyyy", "dd-mm-yyyy", "yyyy-mm-dd", "yyyy-dd-mm", "yyyy-mm", "yyyy"
)
date_format_menu.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 400, y_multiplier_integer, date_format_menu
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, group_checkbox, True
)

group_number_var.set(1)
group_lb = tk.Label(window, text="Group ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, group_lb, True
)
group_number = tk.Entry(window, width=3, state="disabled", textvariable=group_number_var)

y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 50, y_multiplier_integer, group_number, True
)
...
add_group_button = tk.Button(
    ...
    height=1,
    state="disabled",
    command=lambda: add_group_to_list(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 100, y_multiplier_integer, add_group_button, True
)

reset_group_button = tk.Button(
    window,
    text="Reset ",
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 140, y_multiplier_integer, reset_group_button, True
)

csv_field_forGroups_lb = tk.Label(window, text="Select csv field ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.setup_IO_brief_coordinate, y_multiplier_integer, csv_field_forGroups_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 400, y_multiplier_integer, csv_field_forGroups_menu, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 540, y_multiplier_integer, group_values_lb, True
)
group_values_entry = tk.Entry(window, width=10, textvariable=group_values_entry_var)
group_values_entry.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 640, y_multiplier_integer, group_values_entry, True
)

group_lb = tk.Label(window, text="Group label ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 750, y_multiplier_integer, group_lb, True
)
group_label_entry = tk.Entry(window, width=10, textvariable=group_label_entry_var)
group_label_entry.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 840, y_multiplier_integer, group_label_entry
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, icon_lb, True
)
icon_value_lb = tk.Label(window, text="Icon type ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, icon_value_lb, True
)
icon_menu = tk.OptionMenu(
    window, icon_var, "Directions", "Paddles (teardrop)", "Paddles (square)", "Pushpins", "Shapes", "Other icons"
)
# icon_menu.config(state='disabled')
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 100, y_multiplier_integer, icon_menu, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.setup_IO_brief_coordinate, y_multiplier_integer, specific_icon_value_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 400, y_multiplier_integer, specific_icon_menu, True
)

image_lb = tk.Label(window)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 650, y_multiplier_integer, image_lb, False
)
...
def display_icon_image(pic_url, y_multiplier_integer_save):
    ...
    try:
        ...
        GUI_IO_util.placeWidget(
            window, GUI_IO_util.IO_configuration_menu + 650, y_multiplier_integer_save, image_lb, False
        )
    except Exception as e:
        print(
            "GIS_Google_Earth: could not display the icon preview image ("
            + type(e).__name__
            + ": "
            + str(e)
...
specific_icon_var.trace(
    "w", callback=lambda x, y, z: update_specific_icon_menu(icon_var, specific_icon_menu, y_multiplier_integer_save)
)
...
icon_var.trace(
    "w",
    callback=lambda x, y, z: activate_specific_icon(
        icon_var, specific_icon_value_lb, specific_icon_menu, y_multiplier_integer_save
    ),
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, name_checkbox, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, scale_lb, True
)
scale_entry = tk.Entry(window, width=4, textvariable=scale_var)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 50, y_multiplier_integer, scale_entry, True
)

opacity_lb = tk.Label(window, text="Opacity ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 100, y_multiplier_integer, opacity_lb, True
)
opacity_entry = tk.Entry(window, width=4, textvariable=opacity_var)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 170, y_multiplier_integer, opacity_entry, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.setup_IO_brief_coordinate, y_multiplier_integer, color_checkbox, True
)

color_style_var_list.append("")
color_lb = tk.Label(window, text="RGB color code ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 400, y_multiplier_integer, color_lb, True
)
color_entry = tk.Entry(window, width=10, textvariable=color_style_var)
color_entry.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 500, y_multiplier_integer, color_entry
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, description_checkbox, True
)

description_csv_field_var_list.clear()
field_lb = tk.Label(window, text="Select csv field ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, field_lb, True
)
if menu_values != "":
    ...
    description_csv_field_menu = tk.OptionMenu(window, description_csv_field_var, menu_values)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 100, y_multiplier_integer, description_csv_field_menu, True
)
...
def changed_GIS_filename(*args):
    ...
    if inputIsCoNLL == True:
        reminders_util.checkReminder(
            scriptName, reminders_util.title_options_Google_Earth_CoNLL, reminders_util.message_Google_Earth_CoNLL, True
        )
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.setup_IO_brief_coordinate, y_multiplier_integer, bold_checkbox, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu + 400, y_multiplier_integer, italic_checkbox
)
...
def activate_group_options(*args):
    if (
        group_var.get() == 1
        and len(icon_var_list) == int(group_number_var.get())
        and len(specific_icon_var_list) == int(group_number_var.get())
    ):
    ...
    if len(description_csv_field_var.get()) > 0:
        if len(description_csv_field_var_list) >= int(group_number_var.get()):
            del description_csv_field_var_list[
                -1
            ]  # delete the last selected if there is a new currently selected chart
...
heat_map_var.set(1)
heat_map_checkbox = tk.Checkbutton(
    window, text="Heat map (via Google Maps)", variable=heat_map_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, heat_map_checkbox, True
)
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "\n\nUsing the dropdown menu, if a date is present, select the column containing the date and the date format. If a date is present, it will be used to construct dynamic GIS models."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
...
readMe_message = (
    "This Python 3 script relies on the Python Geopy library to geocode locations, i.e., finding a locaton latitude and longitude so that it can be mapped using Google Earth Pro for pin maps and Google Maps for heat maps.\n\nFOR THESE OPTIONS YOU MUST HAVE GOOGLE API KEYS.\n\nYOU MUST DOWNLOAD AND INSTALL THE FREEWARE GOOGLE EARTH PRO at https://www.google.com/earth/versions/#download-pro.\n\nIn INPUT, the script can take:"
