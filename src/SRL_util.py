...
def availability_message():
    ...
    return (
        "Semantic Role Labeling needs a one-time setup that is not yet complete on this "
        "machine.\n\nMissing:\n - "
        + "\n - ".join(missing)
...
def run_SRL(window, inputFilename, inputDir, outputDir, chartPackage="No charts", dataTransformation=""):
    ...
    if not input_path:
        mb.showwarning(title="No input", message="Please select a txt file or a directory of txt files for SRL.")
        return []
    ...
    srl_dir = IO_files_util.make_output_subdirectory(inputFilename, inputDir, outputDir, label="SRL", silent=True)
    if not srl_dir:
        mb.showwarning(
            title="SRL output folder",
            message="Could not create the SRL output subdirectory.\n\nPlease check the output directory and try again.",
        )
        return []

    output_csv = IO_files_util.generate_output_file_name(input_path, inputDir, srl_dir, ".csv", "SRL")

    startTime = IO_user_interface_util.timed_alert(
        window, 2000, "Analysis start", "Started running SRL (Semantic Role Labeling) at", True, "", True
    )
    ...
    print(
        ">>> SRL: contrary to other NLP Suite algorithms, SRL CANNOT display 'Processing file' progress -- "
    ...
    try:
        proc = subprocess.run(
            [srl_python(), SRL_WORKER, input_path, output_csv, srl_model()], capture_output=True, text=True
        )
    ...
    if proc.returncode != 0:
        ...
        mb.showerror(title="SRL error", message="The SRL subprocess reported an error:\n\n%s" % tail)
        return []

    if not os.path.isfile(output_csv):
        mb.showwarning(
            title="SRL produced no output",
    ...
    IO_user_interface_util.timed_alert(
        ...
        2000,
        "Analysis end",
        "Finished running SRL (Semantic Role Labeling) at",
        True,
        "",
        True,
        startTime,
...
def _add_wordnet_column(srl_csv):
    ...
    try:
        ...
        mb.showwarning(
            title="SRL WordNet column", message="Could not read the SRL table to add the WordNet column:\n\n%s" % e
        )
        return
    ...
    try:
        ...
        mb.showwarning(
            title="SRL WordNet column", message="Could not load WordNet (NLTK) to add the WordNet column:\n\n%s" % e
        )
        return
    ...
    def _wn_sense(row):
        ...
        lemma = frame.split(".", maxsplit=1)[0].strip().lower() if frame and frame.lower() != "nan" else ""
        if not lemma:
    ...
    try:
        ...
        mb.showwarning(
            title="SRL WordNet column", message="Could not write the SRL table with the WordNet column:\n\n%s" % e
        )


def _build_srl_visualizations(window, srl_csv, srl_dir, inputFilename, inputDir, chartPackage, dataTransformation):
    ...
    try:
        ...
        mb.showwarning(title="SRL visualization", message="Could not read the SRL output for visualization:\n\n%s" % e)
        return outputs
    ...
    try:
        ...
        ].to_csv(svo_csv, index=False, encoding="utf-8")
        outputs.append(svo_csv)
    except Exception as e:
        mb.showwarning(title="SRL visualization", message="Could not write the SVO-format file:\n\n%s" % e)
        return outputs
    ...
    try:
        ...
        net = charts_util.network_graph_visjs(
            svo_csv, srl_dir, "Agent (ARG0)", "Predicate", "Patient (ARG1)", date_col="Date" if use_date else None
        )
        if net:
        ...
        mb.showwarning(
            title="SRL network graph (interactive)",
    ...
    try:
        ...
        mb.showwarning(
            title="SRL network graph (Gephi)", message="Could not build the SRL Gephi network graph:\n\n%s" % repr(e)
        )
    ...
    try:
        ...
        mb.showwarning(title="SRL Sankey", message="Could not build the SRL Sankey chart:\n\n%s" % e)
        ...
        try:
            ...
            for ycol, title, xlabel, ftype in chart_specs:
                try:
                    ...
                    mb.showwarning(title="SRL chart", message="Could not build the '%s' chart:\n\n%s" % (title, e))
        except Exception as e:
            mb.showwarning(title="SRL charts", message="Could not build the SRL frequency charts:\n\n%s" % e)
    ...
    for m, a, p in zip(refined_maps, svo["Agent (ARG0)"].astype(str), svo["Patient (ARG1)"].astype(str)):
        a, p = a.strip(), p.strip()
    ...
    try:
        ...
        ].to_csv(role_csv, index=False, encoding="utf-8")
        outputs.append(role_csv)
        role_view_ok = True
    except Exception as e:
        mb.showwarning(
            title="SRL role visualization", message="Could not write the VerbNet-role relations file:\n\n%s" % e
        )
    ...
    if role_view_ok:
        ...
        try:
            ...
            rnet = charts_util.network_graph_visjs(
                role_csv, srl_dir, "Agent role", "Predicate", "Patient role", date_col="Date" if use_date else None
            )
            if rnet:
            ...
            mb.showwarning(
                title="SRL role network", message="Could not build the VerbNet-role network:\n\n%s" % repr(e)
            )
        # Role-flow Sankey: Agent-role -> Predicate -> Patient-role.
        try:
            import charts_util

            rsankey_out = IO_files_util.generate_output_file_name(role_csv, inputDir, srl_dir, ".html", "role-sankey")
            rsk = charts_util.Sankey(role_csv, rsankey_out, "Agent role", 5, "Predicate", 10, True, "Patient role", 20)
            if rsk:
                outputs.extend(rsk if isinstance(rsk, list) else [rsk])
                subset.extend(rsk if isinstance(rsk, list) else [rsk])
        except Exception as e:
            mb.showwarning(title="SRL role Sankey", message="Could not build the VerbNet-role Sankey:\n\n%s" % e)
    ...
    if chartPackage and chartPackage != "No charts":
        try:
            ...
            if role_rows:
                ...
                of = charts_util.plot(
                    ...
                    srl_dir,
                    columns=["Role"],
                    title="Frequency Distribution of SRL Refined Roles (VerbNet)",
                    x_label="Refined role (VerbNet)",
                    file_label="SRL-refined-role",
                    plot_list=["Frequency"],
                    title_label="Refined role (VerbNet)",
                )
                if of:
                    outputs.extend(of if isinstance(of, list) else [of])
        except Exception as e:
            mb.showwarning(
                title="SRL refined-role chart", message="Could not build the refined-role frequency chart:\n\n%s" % e
            )
        ...
        try:
            ...
            for cls, doc in zip(svo["VerbNet class"], svo["Document"].astype(str)):
                ...
                if cls:
                    vn_rows.append(
                        {"Document ID": _doc_id(doc), "Document": doc, "VerbNet class": class_names.get(cls, cls)}
                    )
            if vn_rows:
                ...
                of = charts_util.plot(
                    vn_freq_csv,
                    srl_dir,
                    columns=["VerbNet class"],
                    title="Frequency Distribution of SRL VerbNet Classes",
                    x_label="VerbNet class",
                    file_label="SRL-verbnet-class",
                    plot_list=["Frequency"],
                    title_label="VerbNet class",
                )
                if of:
                    outputs.extend(of if isinstance(of, list) else [of])
        except Exception as e:
            mb.showwarning(
                title="SRL VerbNet class chart", message="Could not build the VerbNet class frequency chart:\n\n%s" % e
            )
        ...
        try:
            ...
            if fn_rows:
                ...
                of = charts_util.plot(
                    fn_freq_csv,
                    srl_dir,
                    columns=["FrameNet frame"],
                    title="Frequency Distribution of SRL FrameNet Frames",
                    x_label="FrameNet frame",
                    file_label="SRL-framenet-frame",
                    plot_list=["Frequency"],
                    title_label="FrameNet frame",
                )
                if of:
                    outputs.extend(of if isinstance(of, list) else [of])
        except Exception as e:
            mb.showwarning(
                title="SRL FrameNet frame chart",
        ...
        try:
            ...
            if wn_rows:
                ...
                of = charts_util.plot(
                    ...
                    srl_dir,
                    columns=["WordNet"],
                    title="Frequency Distribution of SRL WordNet Senses",
                    x_label="WordNet sense",
                    file_label="SRL-wordnet-sense",
                    plot_list=["Frequency"],
                    title_label="WordNet sense",
                )
                if of:
                    outputs.extend(of if isinstance(of, list) else [of])
        except Exception as e:
            mb.showwarning(
                title="SRL WordNet sense chart", message="Could not build the WordNet sense frequency chart:\n\n%s" % e
            )
