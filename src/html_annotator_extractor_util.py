...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "html_annotator_extractor_util", ["os", "tkinter", "re", "csv", "ntpath"]
    )
    == False
):
    sys.exit(0)
...
def buildcsv(
    inputHTMLFile, inputHTMLFolder, outputDir, openOutputFiles, chartPackage, dataTransformation, configFileName
):
    filesToOpen = []
    outputFilename = IO_files_util.generate_output_file_name(
        "DBpedia annotations", "", outputDir, ".csv", "html extractor", "", ""
    )
    filesToOpen.append(outputFilename)
    annotatedHtmlFiles = []

    annotatedHtmlFiles = IO_files_util.getFileList(
        inputHTMLFile, inputHTMLFolder, ".html", silent=False, configFileName=configFileName
    )
    ...
    startTime = IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis start", "Started running html annotator extractor at", True, "", True, "", True
    )
    ...
    IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis end", "Finished running html annotator extractor at", True, "", True, startTime
    )
