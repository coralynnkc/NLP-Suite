...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "semantic_analysis_main.py", ["os", "csv", "tkinter", "ntpath", "collections", "subprocess"]
    )
    == False
):
    sys.exit(0)
...
def run():
    ...
    if WSI_var == 1:
        if WSI_keywords_var == "":
            mb.showwarning(
                ...
                message='The "Word Sense Induction" algorithm requires a comma-separated list of case-sensitive keywords taken from the corpus in order to run.\n\nPlease, enter the keywords and try again.',
            )
            return
    ...
    if WSD_var:
        if "CoNLL" not in inputFilename and "CoNLL" not in csv_file.get():
            mb.showwarning(
                title="Warning",
                message="The script Word Sense Disambiguation (WSD) algorithm requires a CoNLL FILE in INPUT.\n\n"
        ...
        wsd_outputDir = (
            IO_files_util.make_output_subdirectory(inputFilename, inputDir, outputDir, label="WSD", silent=True)
            or outputDir
        )
        noun_verb_list = ["NOUN", "VERB"]
        for noun_verb in noun_verb_list:
            outFiles = semantic_aggregation_util.wsd_aggregate_WordNet(
                inputFilename, wsd_outputDir, noun_verb, chartPackage, dataTransformation
            )
            if outFiles:
    ...
    if SSC_var == 1:
        ...
        simDir = (
            IO_files_util.make_output_subdirectory(
                inputFilename, inputDir, outputDir, label="semantic_similarity", silent=True
            )
            or outputDir
        )
        outFiles = semantic_similarity_util.document_similarity_clustering(
            window, inputFilename, inputDir, simDir, openOutputFiles, chartPackage, dataTransformation
        )
        if outFiles:
            filesToOpen.extend(outFiles)
    ...
    if vocabulary_analysis_var == True:
        ...
        if "*" in vocabulary_analysis_menu_var or "Coreference" in vocabulary_analysis_menu_var:
            ...
            if mb.askyesno(
                ...
                if outDir and os.path.isdir(outDir):
                    cands = sorted(
                        [d for d in glob.glob(os.path.join(outDir, "coref_*")) if os.path.isdir(d)],
                    ...
                    if cands:
                        ...
                        for r, ds, fs in os.walk(cands[0]):
                            if any(f.lower().endswith(".txt") for f in fs):
                                ...
                                break
                if not coref_dir:
                    coref_dir = filedialog.askdirectory(
                        title="Select the coreferenced corpus folder (its coreferenced txt files)"
                    )
                if coref_dir:
                    ...
                    try:
                        if getattr(GUI_util, "IO_setup_brief_display_area", None) is not None:
                            io_display = (
                                "INPUT DIR: "
                                + os.path.basename(os.path.normpath(coref_dir))
                                + "\nOUTPUT DIR: "
                                + os.path.basename(os.path.normpath(GUI_util.output_dir_path.get()))
                            )
        ...
        if "*" == vocabulary_analysis_menu_var or "Abstract" in vocabulary_analysis_menu_var:
            if language == "English":
                ...
                if not "*" == vocabulary_analysis_menu_var:
                    mb.showwarning(
                        title="Warning",
                        message="The Abstract/concrete vocabulary analysis algorithm is only available for the English language.",
                    )
        ...
        if "*" == vocabulary_analysis_menu_var or "Iconic" in vocabulary_analysis_menu_var:
            if language == "English":
                ...
                if not "*" == vocabulary_analysis_menu_var:
                    mb.showwarning(
                        title="Warning",
                        message="The Iconicity analysis algorithm is only available for the English language.",
                    )
        ...
        if "*" in vocabulary_analysis_menu_var or "TF-IDF" in vocabulary_analysis_menu_var:
            ...
            outputFiles = statistics_corpus_tfidf_util.compute_tfidf(
...
extra_GUIs_menu = tk.OptionMenu(
    ...
    "Spelling/grammar checker (Open GUI)",
    "Corpus statistics (Open GUI)",
    "N-grams & Co-Occurrences (Open GUI)",
    "Nominalization (Open GUI)",
    "CoNLL table analyzer (Open GUI)",
    "WordNet (Open GUI)",
...
WSI_var.set(0)
WSI_checkbox = tk.Checkbutton(
    window,
    text="Word Sense Induction (WSI) (via BERT (English language model))",
    variable=WSI_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.open_setup_x_coordinate - 40,
    y_multiplier_integer,
    k_means_max_var,
    True,
    False,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.run_button_x_coordinate - 55, y_multiplier_integer, ngrams_lb, True
)
ngrams_menu_var = tk.StringVar()
ngrams_menu_var.set("1-grams")
ngrams_menu = tk.OptionMenu(
    window, ngrams_menu_var, "1-grams (unigrams)", "2-grams (bigrams)", "3-grams (trigrams)", "4-grams (quadgrams)"
)
# place widget with hover-over info # memory_pos
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.run_button_x_coordinate + 25,
    y_multiplier_integer,
    ngrams_menu,
    True,
    False,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.close_button_x_coordinate + 50,
    y_multiplier_integer,
    top_keywords_var,
    False,
    False,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, WSI_keywords_lb, True
)
...
WSIdictionary_button = tk.Button(
    window,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.labels_x_indented_coordinate + 100,
    y_multiplier_integer,
...
openInputFile_button = tk.Button(
    ...
    width=GUI_IO_util.open_file_directory_button_width,
    text="",
    command=lambda: IO_files_util.openFile(window, WSI_keywords_var.get()),
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.labels_x_indented_coordinate + 250,
    y_multiplier_integer,
    openInputFile_button,
    True,
    False,
...
WSD_var.set(0)
WSD_checkbox = tk.Checkbutton(
    window,
    text="Word Sense Disambiguation (WSD) (via CoNLL)",
    variable=WSD_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, WSD_checkbox, False
)

SRL_var.set(0)
SRL_checkbox = tk.Checkbutton(
    window,
    text="Semantic Role Labelling (SRL) (Open GUI)",
    variable=SRL_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, SRL_checkbox, False
)

SSC_var.set(0)
SSC_checkbox = tk.Checkbutton(
    window,
    text="DOCUMENT embeddings: Semantic similarity & clustering",
    variable=SSC_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, SSC_checkbox, False
)

classify_words_by_semantic_closeness_button = tk.Button(
    window,
    width=90,
    text="WORD embeddings: Classify words by semantic proximity: Word2Vec & word embeddings (Open GUI)",
    command=lambda: run_script_util.run_script("Word2Vec_main.py"),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, classify_words_by_semantic_closeness_button, False
)

semantic_aggregation_button = tk.Button(
    window,
    width=90,
    text="Aggregate words by semantics (Open GUI)",
    command=lambda: run_script_util.run_script("semantic_aggregation_main.py"),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, semantic_aggregation_button, False
)


vocabulary_analysis_var.set(0)
vocabulary_analysis_checkbox = tk.Checkbutton(
    window,
    text="More semantic analyses",
    variable=vocabulary_analysis_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, vocabulary_analysis_checkbox, True
)
...
# place widget with hover-over info
vocabulary_analysis_menu = tk.OptionMenu(
    window,
...
TIPS_options = (
    "Word Sense Induction (WSI) (via BERT & K-means)",
    "Word Sense Disambiguation (WSD) (via CoNLL & WordNet)",
