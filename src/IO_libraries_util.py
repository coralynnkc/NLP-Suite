...
def get_java_executable():
    ...
    def _layouts(home):
        return [os.path.join(home, "bin", exe), os.path.join(home, "Contents", "Home", "bin", exe)]
...
def which_shell():
    ...
    if shell != "zsh":
        ...
        answer = tk.messagebox.askyesno("MacOS shell error", error_msg + "\n\nDo you want to open the TIPS file now?")
        if answer:
...
def install_all_Python_packages(window, calling_script, modules_to_try):
    ...
    if missingModules:
        ...
        message = (
            "FATAL ERROR. Please, read carefully. The NLP Suite will exit.\n\nThe script '"
            + calling_script
            + "' needs to import the following modules:\n\n"
            + ", ".join(missingModules)
            ...
            if github_version != local_version:
                if not silent:
                    reminders_util.checkReminder(
                        ...
                        reminders_util.title_options_CoreNLP_version,
                        reminders_util.message_CoreNLP_version,
                        True,
                    )
...
def check_inputExternalProgramFile(calling_script, software_dir, programName, readingConfig=True, silent=False):
    ...
    if "Java" in programName:
        ...
        if Java_errorFound and not silent:
            mb.showwarning(
                title=programName + " installation.", message=programName + " IS NOT INSTALLED ON YOUR MACHINE."
            )
            return False
        else:
            if software_dir == "" and not silent:
                mb.showwarning(
                    title=programName + " installation.",
                    message=programName
    ...
    if message != "" and not silent:
        mb.showwarning(title=programName.upper() + " installation error", message=message)
...
def open_url(
    ...
    if ask_to_open:
        answer = tk.messagebox.askyesno(message_title, message)
        if answer == False:
            return False
...
def get_missing_external_software_list(calling_script, external_software_config_file, software_name, silent=False):
    ...
    for row in external_software_config_file[1:]:  # skip header line
        ...
        if (
            ...
            or os.path.isdir(software_dir) == False
            or ExternalProgramFile_result == False
        ):
...
def save_software_config(existing_software_config, missing_software_string, silent=False):
    ...
    with csv_file:
        ...
        if not silent:
            mb.showwarning(title="Config installation file saved", message=message)
...
def display_download_installation_messages(
    ...
    if software_name == "":
        ...
        # NL_menu__main
        if (software_dir == "" or software_dir == None) and (
            ...
            if software_name != "":
                ...
                if "Java" in software_name:
                    ...
                    if not Java_errorFound:
                        ...
                        mb.showwarning(
                            title=software_name + " installation.",
                            message=software_name
                            + " IS ALREADY INSTALLED ON YOUR MACHINE:\n\n"
                            + software_dir
    ...
    # INSTALL messages -------------------------------------------------------------------------
        ...
        temp_software_dir = tk.filedialog.askdirectory(
            initialdir=software_dir, title="Select a directory for " + software_name + ". Press Esc or Cancel to exit."
        )
