...
def adverb_stats(inputFilename, outputDir, data, data_divided_sents, openOutputFiles, chartPackage, dataTransformation):
    ...
    startTime = IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis start", "Started running ADVERB ANALYSES at", True, "", True, "", True
    )
    ...
    adverbs_stats_file_name = IO_files_util.generate_output_file_name(
    ...
    adverbs_postag_stats_file_name = IO_files_util.generate_output_file_name(
    ...
    adverbs_deprel_stats_file_name = IO_files_util.generate_output_file_name(
    ...
    IO_csv_util.df_to_csv(
        GUI_util.window, df, adverbs_postag_list_file_name, headers=headers, index=False, language_encoding="utf-8"
    )
    ...
    IO_csv_util.df_to_csv(
        GUI_util.window, df, adverbs_deprel_list_file_name, headers=headers, index=False, language_encoding="utf-8"
    )
    ...
    if chartPackage != "No charts":
        ...
        outputFiles = charts_util.plot(
            ...
            x_label="Adverb POS tag",
            file_label="adverb_POS",
        )
        ...
        outputFiles = charts_util.plot(
            ...
            x_label="Adverb DEPREL tag",
            file_label="adverb_DEPREL",
        )
    ...
    IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis end", "Finished running ADVERB ANALYSES at", True, "", True, startTime, True
    )
