...
if (
    IO_libraries_util.install_all_Python_packages(GUI_util.window, "CoNLL Table Analyzer", ["csv", "os", "collections"])
    == False
):
    sys.exit(0)
...
def adjective_stats(
    ...
    startTime = IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis start", "Started running ADJECTIVE ANALYSES at", True, "", True, "", True
    )
    ...
    adjective_stats_file_name = IO_files_util.generate_output_file_name(
    ...
    adjective_postag_stats_file_name = IO_files_util.generate_output_file_name(
        inputFilename, "", outputDir, ".csv", "AVA", "Adjective", "POSTAG_stats"
    )
    adjective_ner_stats_file_name = IO_files_util.generate_output_file_name(
        inputFilename, "", outputDir, ".csv", "AVA", "Adjective", "NER_stats"
    )
    adjective_deprel_stats_file_name = IO_files_util.generate_output_file_name(
    ...
    IO_csv_util.df_to_csv(
        GUI_util.window, df, adjective_postag_list_file_name, headers=headers, index=False, language_encoding="utf-8"
    )
    ...
    IO_csv_util.df_to_csv(
        GUI_util.window, df, adjective_ner_list_file_name, headers=headers, index=False, language_encoding="utf-8"
    )
    ...
    IO_csv_util.df_to_csv(
        GUI_util.window, df, adjective_deprel_list_file_name, headers=headers, index=False, language_encoding="utf-8"
    )
    ...
    if chartPackage != "No charts":
        ...
        outputFiles = charts_util.plot(
            ...
            title="Frequency Distribution of Adjective POS Tags",
            x_label="Adjective POS tag",
            count=count_var,
            file_label="adjective_POS",
        )
        ...
        outputFiles = charts_util.plot(
            ...
            outputDir,
            columns=columns_to_be_plotted_yAxis,
            title="Frequency Distribution of Adjective NER Tags",
            x_label="Adjective NER tag",
            count=count_var,
            file_label="adjective_NER",
        )
        ...
        outputFiles = charts_util.plot(
            ...
            outputDir,
            columns=columns_to_be_plotted_yAxis,
            title="Frequency Distribution of Adjective DEPREL Tags",
            x_label="Adjective DEPREL tag",
            count=count_var,
            file_label="adjective_DEPREL",
        )
    ...
    IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis end", "Finished running ADJECTIVE ANALYSES at", True, "", True, startTime, True
    )
