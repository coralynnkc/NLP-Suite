...
if (
    IO_libraries_util.install_all_Python_packages(GUI_util.window, "Wordclouds", ["os", "tkinter", "webbrowser"])
    == False
):
    sys.exit(0)
...
def run():
    ...
    if (differentColumns_differentColors == True) and ((len(inputFilename) == 0) or (inputFilename[-3:] != "csv")):
        mb.showerror(
            title="Warning",
            message="You have selected the option of using different colors for different columns of a single csv file. But... you have not selected in input a csv file.\n\nPlease, select an appropriate csv file in input and try again.",
        )
        return
    ...
    if (differentColumns_differentColors == True) and len(csvField_color_list) == 0:
        mb.showerror(
            title="Warning",
            message="You have selected the option of using different colors for different columns of a single csv file. But... you have not selected in input the csv file field.\n\nPlease, select an appropriate csv file field and try again.",
        )
        return
    ...
    if differentColumns_differentColors == True and "|" not in str(csvField_color_list):
        mb.showerror(
            title="Warning",
            message="You have selected the option of using different colors for different columns of a csv file. But... you have not selected the colors to be used.\n\nPlease, select a color by ticking the Color checkbox, select your preferred color and try again.",
        )
        return
...
doNotCreateIntermediateFiles_var = (
    tk.IntVar()
)  # when an entire directory is processed; could lead to an enourmus number of output files
...
extra_GUIs_checkbox = tk.Checkbutton(
    ...
    onvalue=1,
    offvalue=0,
    command=lambda: activate_GUI_options(),
)
...
extra_GUIs_menu = tk.OptionMenu(
    window, extra_GUIs_menu_var, "Statistics csv file", "Data visualization", "Parsers & annotators"
)
...
selectedImage_var.set("")
use_contour_only_var.set(1)
wordclouds = tk.OptionMenu(
    window, wordclouds_var, "Python WordCloud", "TagCrowd", "Tagul", "Tagxedo", "Wordclouds", "Wordle"
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate + 120, y_multiplier_integer, wordclouds, True
)
wordclouds_lb = tk.Label(window, text="Select the word cloud service you wish to use (txt file(s)/CoNLL table)")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, wordclouds_lb
)

y_multiplier_integer_SV = y_multiplier_integer

differentPOS_differentColor_var.set(0)
differentPOS_differentColor_checkbox = tk.Checkbutton(
    window, variable=differentPOS_differentColor_var, onvalue=1, offvalue=0
)

wordcloud_title_lb = tk.Label(window, text="Wordcloud title")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, wordcloud_title_lb, True
)
...
prefer_horizontal_checkbox = tk.Checkbutton(window, variable=prefer_horizontal_var, onvalue=1, offvalue=0)

prefer_horizontal_checkbox.config(text="Horizontal")
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.wordclouds_lowercase_pos,
    y_multiplier_integer,
...
def warnUser(*args):
    if prefer_horizontal_var.get() == True:
        reminders_util.checkReminder(
            scriptName,
            reminders_util.title_options_python_wordclouds_horizontal,
            reminders_util.message_python_wordclouds_horizontal,
            True,
        )
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.wordclouds_collocation_pos,
    y_multiplier_integer,
    font_lb,
    True,
    False,
    ...
    False,
    90,
    GUI_IO_util.wordclouds_font_lb,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, max_words_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.wordclouds_max_words_number,
    y_multiplier_integer,
    max_words,
    True,
    False,
...
lemmatize_var.set(1)
lemmatize_checkbox = tk.Checkbutton(window, variable=lemmatize_var, onvalue=1, offvalue=0)
lemmatize_checkbox.config(text="Lemmas")
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.wordclouds_lemmas_pos,
    y_multiplier_integer,
    lemmatize_checkbox,
    True,
    False,
...
stopwords_checkbox = tk.Checkbutton(window, variable=exclude_stopwords_var, onvalue=1, offvalue=0)

stopwords_checkbox.config(text="Stopwords")
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.wordclouds_stopwords_pos,
    y_multiplier_integer,
    stopwords_checkbox,
    True,
    False,
...
punctuation_checkbox = tk.Checkbutton(window, variable=exclude_punctuation_var, onvalue=1, offvalue=0)

punctuation_checkbox.config(text="Punctuation")
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.wordclouds_punctuation_pos,
    y_multiplier_integer,
...
lowercase_checkbox = tk.Checkbutton(window, variable=lowercase_var, onvalue=1, offvalue=0)
...
collocation_checkbox = tk.Checkbutton(window, variable=collocation_var, onvalue=1, offvalue=0)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.wordclouds_color_by_POS_tags,
    y_multiplier_integer,
...
prepare_image_checkbox = tk.Checkbutton(window, variable=prepare_image_var, onvalue=1, offvalue=0)
...
# width=20,
select_image_file_button = tk.Button(
    window,
    text="Select png image file",
    command=lambda: get_image(window, "Select INPUT png file", [("png files", "*.png")]),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
openImage_button = tk.Button(
    window,
    width=GUI_IO_util.open_file_directory_button_width,
    text="",
    state="disabled",
    command=lambda: IO_files_util.openFile(window, selectedImage_var.get()),
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.wordclouds_openImage_button,
    y_multiplier_integer,
    openImage_button,
    True,
    False,
...
selectedImage.config(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.wordclouds_selectedImage_file_path, y_multiplier_integer, selectedImage, True
)
...
differentColumns_differentColor_var.set(0)
differentColumns_differentColor_checkbox = tk.Checkbutton(
    window, variable=differentColumns_differentColor_var, onvalue=1, offvalue=0
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, field_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.wordclouds_select_csv_field,
    y_multiplier_integer,
    csv_field_menu,
    True,
    False,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.wordclouds_color_checkbox_pos,
    y_multiplier_integer,
    color_checkbox,
    True,
    False,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.wordclouds_RGB_lb, y_multiplier_integer, color_lb, True
)
color_entry = tk.Entry(window, width=10, textvariable=color_style_var)
color_entry.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.wordclouds_RGB, y_multiplier_integer, color_entry, True
)
...
add_button = tk.Button(
    window, text="+", width=GUI_IO_util.add_button_width, height=1, state="disabled", command=lambda: update_csvFields()
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.wordclouds_add_button, y_multiplier_integer, add_button, True
)
...
reset_button = tk.Button(
    ...
    height=1,
    state="disabled",
    command=lambda: clear_field_color_list(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.wordclouds_reset_button, y_multiplier_integer, reset_button, True
)
...
def showList():
    if len(csvField_color_list) == 0:
        mb.showwarning(
            title="Warning", message="There are no currently selected combinations of csv fields and colors."
        )
    else:
        mb.showwarning(
            title="Warning",
            message="The currently selected combination of csv fields and colors is:\n\n"
            + ",".join(csvField_color_list)
            + "\n\nPlease, press the RESET button (or ESCape) to start fresh.",
        )


show_columns_button = tk.Button(
    window, text="Show", width=GUI_IO_util.show_button_width, height=1, state="disabled", command=lambda: showList()
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.wordclouds_show_button, y_multiplier_integer, show_columns_button
)
    ...
    if csv_field_var.get() != "":
        ...
        if state != "disabled":
            if csv_field_var.get() in csvField_color_list:
                mb.showwarning(
                    title="Warning",
                    message="The selected csv field value, "
                    + csv_field_var.get()
...
def activate_Python_options(*args):
    ...
    if "Python" not in wordclouds_var.get():
        selectedImage_var.set("")
        use_contour_only_var.set(1)
...
doNotCreateIntermediateFiles_checkbox = tk.Checkbutton(
    window, variable=doNotCreateIntermediateFiles_var, onvalue=1, offvalue=0
)
doNotCreateIntermediateFiles_checkbox.config(
    text="Do NOT produce intermediate word cloud files when processing all txt files in a directory"
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, doNotCreateIntermediateFiles_checkbox
)
...
def changeLabel_nomin(*args):
    if doNotCreateIntermediateFiles_var.get() == 1:
        ...
        doNotCreateIntermediateFiles_checkbox.config(
            text="Produce intermediate word cloud files when processing all txt files in a directory"
        )
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox to open the web service Removebg (https://www.remove.bg/) that will prepare an image for use in the Python wordclouds algorithm, removing all image background and turning it into white.\n\nYou can then use the output png image file to create the wordclouds (see the widget 'Select png image' file).\n\nYOU MUST BE CONNECTED TO THE INTERNET.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
