...
def run():
    ...
    if SRL_var.get() == 1:
        ...
        if inputFilename and inputFilename[-4:].lower() == ".csv":
            ...
            return
        srl_result = SRL_util.run_SRL(
            GUI_util.window, inputFilename, inputDir, outputDir, chartPackage, dataTransformation
        )
        ...
        if srl_files:
            ...
            IO_files_util.OpenOutputFiles(
                GUI_util.window, openOutputFiles, srl_files, outputDir, scriptName, filesToOpenSubset=srl_subset
            )
        return
