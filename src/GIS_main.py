...
if (
    IO_libraries_util.install_all_Python_packages(GUI_util.window, "GIS", ["os", "tkinter", "pandas", "subprocess"])
    == False
):
    sys.exit(0)
...
def run():
    ...
    if map_characters:
        outputDir_mc = IO_files_util.make_output_subdirectory(
        ...
        if trackingFiles:
            ...
            if csv_files:
                mapFiles = charts_util.animated_migration_map(csv_files[0], outputDir_mc, "Entity", "Location")
                if mapFiles:
    ...
    if NER_extractor and csv_file == "":
        if "BERT" in NER_package:
            ...
            locationFiles = BERT_util.NER_tags_BERT(
                GUI_util.window,
                inputFilename,
                inputDir,
                outputDir,
            ...
            locationFiles = Stanza_util.Stanza_annotate(
                config_filename,
                inputFilename,
                inputDir,
                outputDir,
            ...
            locationFiles = spaCy_util.spaCy_annotate(
                config_filename,
                inputFilename,
                inputDir,
                outputDir,
        ...
        prepared = GIS_pipeline_util.normalize_NER_csv_for_GIS(
            ...
            filename_embeds_date_var=filename_embeds_date_var,
            date_format=date_format_var,
            items_separator=items_separator_var,
    ...
    if "Proportional" in GIS_package_var:
        ...
        outputfilename = charts_util.proportional_circle_map(NER_outputFilename, outputDir, locationColumnName)
        if outputfilename:
        ...
        if GIS_package_var != "":
            mb.showwarning(
                ...
                "The " + GIS_package_var + " option is not available yet.\n\nSorry! Please, check back soon...",
            )
            return
...
def check_csv_file_headers(csv_file):
    ...
    inputIsCoNLL, inputIsGeocoded, withHeader, headers, datePresent, filenamePositionInCoNLLTable = (
        GIS_file_check_util.CoNLL_checker(csv_file_var.get())
    )
    if inputIsCoNLL:
        reminders_util.checkReminder(
        ...
        reminders_util.checkReminder(
            scriptName, reminders_util.title_options_input_csv_file, reminders_util.message_input_csv_file, True
        )
    ...
    if ("Location" in headers and "Latitude" not in headers) or "Word" in headers:
        geocode_locations_var.set(1)
    ...
    elif "Location" not in headers and "Word" not in headers and "NER" not in headers:
        mb.showwarning(
            title="Warning",
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.IO_configuration_menu,
    "Open INPUT csv dictionary file",
)
...
open_symbolic_button = tk.Button(
    window,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, NER_extractor_checkbox, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate, y_multiplier_integer, location_field
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, geocode_locations_checkbox, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.label_columns, y_multiplier_integer, geocoder_lb, True
)
geocoder_var.set("Nominatim")
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.label_columns, y_multiplier_integer, country_bias_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.labels_x_coordinate,
    "Select a country to privilege geocoding locations in that country when similar locations are found in other countries (e.g., Rome, Georgia, US, and Rome, Italy)",
)

area_lb = tk.Label(window, text="Area")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_setup_x_coordinate, y_multiplier_integer, area_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, map_locations_checkbox, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.label_columns, y_multiplier_integer, GIS_package_lb, True
)

GIS_package_var.set("Python folium pin map & heatmap")
GIS_package = tk.OptionMenu(
    window,
    GIS_package_var,
    "Python folium pin map & heatmap",
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, open_API_config_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.open_reminders_x_coordinate,
    "Open csv file for Google API key",
)
...
def activate_Google_API_Google_Maps(*args):
    ...
    if (
        "folium" not in GIS_package_var.get()
        and "Google" not in GIS_package_var.get()
        and len(GIS_package_var.get()) > 0
    ):
...
def display_reminder(*args):
    if Google_Earth_OpenGUI.get():
        ...
        reminders_util.checkReminder(
            scriptName, reminders_util.title_options_Google_Earth, reminders_util.message_Google_Earth, True
        )
        routine_options = reminders_util.getReminders_list(scriptName)
        return
...
TIPS_options = (
    ...
    "Google Earth Pro From KML to Excel",
    "QGIS: Getting started KML (PC-ACE) files coordinate systems and QGIS project files",
    "QGIS: Import a KML file in QGIS",
    "QGIS: Import data from Excel in QGIS",
    "QGIS: Import more attributes than NAME and DESCRIPTION",
    "QGIS: Join data (mapping multiple variables)",
    "QGIS: Dot map",
    "QGIS: Feature map",
    "QGIS: Heat map",
    "QGIS: Frequency map (graduated color map choropleth)",
    "QGIS: Frequency map (proportional symbol)",
    "QGIS: Shape files",
    "QGIS: Change dots style",
    "QGIS: Change pin style",
    "QGIS: Change pin or map colors",
    "QGIS: Change style of displayed data",
    "QGIS: Display attributes attached to a data point",
    "QGIS: Display different attributes",
    "QGIS: View data behind layers",
    "QGIS: Finishing touches in producing a map",
    "QGIS: Data points not visible on the map",
    "QGIS: KML file error",
    "QGIS: Layer error",
    "QGIS: Map not visible on the screen",
    "QGIS: Shapefile error",
)
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    if IO_setup_display_brief == False:
        ...
        y_multiplier_integer = GUI_IO_util.place_help_button(
            ...
            y_multiplier_integer,
            "NLP Suite Help",
            GUI_IO_util.msg_corpusData + GUI_IO_util.msg_Esc,
        )
        y_multiplier_integer = GUI_IO_util.place_help_button(
            window,
            help_button_x_coordinate,
            y_multiplier_integer,
            "NLP Suite Help",
            GUI_IO_util.msg_outputDirectory + GUI_IO_util.msg_Esc,
        )
    else:
        y_multiplier_integer = GUI_IO_util.place_help_button(
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        "The option uses the same NER extraction as the GIS pipeline but focuses on PERSON entities "
        "and their co-occurring locations rather than geocoding all location mentions.\n\n"
        "NOTE ON 'SPACE': here 'space' means GEOGRAPHIC, geocodable space. For SYMBOLIC / SOCIAL space that has NO map coordinates - e.g. house vs. field, or how a social attribute (gender, race, class) maps onto types of space - use the 'Symbolic / social space' option below."
        + GUI_IO_util.msg_Esc,
    )
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, using the dropdown menu, select the Google Api key you wish to visualize, then click on the button to open the config file to view/change the key."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
...
reminders_util.checkReminder(
    scriptName, reminders_util.title_options_GIS_default, reminders_util.message_GIS_default, True
)

# routine_options = reminders_util.getReminders_list(config_filename)
result = reminders_util.checkReminder(scriptName, reminders_util.title_options_GIS_GUI, reminders_util.message_GIS_GUI)
if result != None:
    routine_options = reminders_util.getReminders_list(scriptName)
