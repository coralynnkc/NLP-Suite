...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "style_analysis_main.py", ["os", "csv", "tkinter", "ntpath", "collections", "subprocess"]
    )
    == False
):
    sys.exit(0)
...
def run():
    ...
    if complexity_readability_analysis_var == True:
        ...
        if complexity_readability_analysis_menu_var == "":
            mb.showwarning(
                "Warning",
                "No option has been selected for Complexity/readability analysis.\n\nPlease, select an option from the dropdown menu and try again.",
            )
            return
...
extra_GUIs_checkbox = tk.Checkbutton(
    window,
    text="GUIs available for style analysis ",
...
complexity_readability_analysis_var.set(0)
complexity_readability_analysis_checkbox = tk.Checkbutton(
    window,
    text="Complexity/readability analysis",
    variable=complexity_readability_analysis_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, complexity_readability_analysis_checkbox, True
)
...
complexity_readability_analysis_menu = tk.OptionMenu(
    window, complexity_readability_analysis_menu_var, "*", "Sentence complexity", "Text readability"
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
gender_guesser_checkbox = tk.Checkbutton(
    window,
    text="Who wrote the text - Gender guesser",
    variable=gender_guesser_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, gender_guesser_checkbox
)
...
TIPS_options = (
    ...
    "Sentence complexity",
    "Text readability",
    "Yule measures of vocabulary richness",
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the 'Who wrote the text' checkbox if you wish to run the Gender Guesser algorithm to determine an author's gender based on the words used.\n\nYou will need to copy and paste a document content to the website http://www.hackerfactor.com/GenderGuesser.php#About\n\nYou need to be connected to the internet.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
