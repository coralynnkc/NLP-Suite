...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "data_manipulation_util.py", ["os", "tkinter", "pandas", "functools"]
    )
    == False
):
    sys.exit(0)
...
def distinct_values(csv_path, column, limit=500):
    ...
    values = set()
    try:
        with open(csv_path, encoding="utf-8", errors="ignore", newline="") as fin:
...
def export_csv_to_csv_txt(outputDir, operation_results_text_list, export_type=".csv", cols=None):
    ...
    for index, df_ex in enumerate(df_list):
        if operation_results_text_list[index].split(",")[4] in ["and", "''"]:
            ...
            mb.showwarning(
                title="Missing and/or condition",
...
def drop(outputDir, operation_results_text_list):
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        filepath, os.path.dirname(filepath), outputDir, ".csv", "drop", "", "", "", "", False, True
    )
...
def split_field(outputDir, operation_results_text_list):
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        filepath, os.path.dirname(filepath), outputDir, ".csv", "split", "", "", "", "", False, True
    )
    df.to_csv(outputFilename, encoding="utf-8", index=False)
    return outputFilename
...
def sort_rows(outputDir, operation_results_text_list):
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        filepath, os.path.dirname(filepath), outputDir, ".csv", "sort", "", "", "", "", False, True
    )
    df.to_csv(outputFilename, encoding="utf-8", index=False)
    return outputFilename
...
def deduplicate(outputDir, operation_results_text_list):
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        filepath, os.path.dirname(filepath), outputDir, ".csv", "deduplicate", "", "", "", "", False, True
    )
    df.to_csv(outputFilename, encoding="utf-8", index=False)
    return outputFilename
...
def rename_field(outputDir, operation_results_text_list):
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        filepath, os.path.dirname(filepath), outputDir, ".csv", "rename", "", "", "", "", False, True
    )
    df.to_csv(outputFilename, encoding="utf-8", index=False)
    return outputFilename
