...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "NLP_welcome_main", ["os", "tkinter", "itertools", "PIL", "subprocess"]
    )
    == False
):
    sys.exit(0)
...
def make_images(canvas_width, canvas_height):
    images.clear()
    image_1_list = [
        GUI_IO_util.image_libPath + os.sep + "visual1.jpg",
    ...
    image_2_list = [
        GUI_IO_util.image_libPath + os.sep + "visual5.jpg",
        GUI_IO_util.image_libPath + os.sep + "visual6.jpg",
        GUI_IO_util.image_libPath + os.sep + "visual7.jpg",
    ]
    image_3_list = [
        GUI_IO_util.image_libPath + os.sep + "visual8.jpg",
    ...
    photos3 = cycle(
        photos_list[len(image_1_list) + len(image_2_list) : len(image_1_list) + len(image_2_list) + len(image_3_list)]
    )
...
def run_slides():
    ...
    canvas_img1 = canvas1.create_image(
        canvas1.winfo_width() / 2, canvas1.winfo_height() / 2, image=img, anchor=tk.CENTER
    )

    canvas2 = tk.Canvas(window)
    canvas2.grid(row=7, column=2, columnspan=2)
    canvas_img2 = canvas2.create_image(
        canvas1.winfo_width() / 2, canvas1.winfo_height() / 2, image=img2, anchor=tk.CENTER
    )  # the 100,100 is not where image is on page, it's position WITHIN canvas!
...
def display_text():
    ...
    welcome_line1 = tk.Label(
        window, text="Welcome to the NLP Suite", foreground="red", font=("Arial", 22, "bold", "italic")
    )
    welcome_line1.grid(row=0, column=0, columnspan=6)

    welcome_line3 = tk.Label(
        window,
...
def display_bottom_line_buttons():
    roberto_franzosi = tk.Label(window, text="Roberto Franzosi", foreground="black", font=("Arial", 12, "italic"))
    roberto_franzosi.grid(row=8, column=0, columnspan=3, sticky=(tk.S, tk.W), padx=30)
    emory = tk.Label(window, text="Emory University", foreground="black", font=("Arial", 12, "italic"))
    ...
    enter_button.bind(
        "<Enter>",
        lambda e: (
            e.widget.config(background="red", foreground="black", text=label_enter),
            GUI_IO_util.display_widget_info(window, e, x_coordinate1, y_coordinate1, x_coordinate1, text_info_enter),
        ),
    )
    enter_button.bind(
        "<Leave>",
        lambda e: (
            GUI_IO_util.delete_display_widget_lb(window, e, text_info_enter),
    ...
    close_button = tk.Button(window, text="CLOSE", width=15, height=1, font=("Arial", 14), command=lambda: close_NLP())
    ...
    text_info_close = (
        "Pressing the CLOSE button in any of the GUIs triggers the automatic update of the NLP Suite, pulling the latest release from GitHub.\nThe new release is displayed the next time you open your local NLP Suite."
    ...
    close_button.bind(
        "<Enter>",
        lambda e: (
            e.widget.config(background="red", text=label_close),
            GUI_IO_util.display_widget_info(window, e, x_coordinate2, y_coordinate2, x_coordinate2, text_info_close),
        ),
    )
    close_button.bind(
        "<Leave>",
        lambda e: (
            e.widget.config(background="#F0F0F0", text=label_close),
...
scolling_labl = tk.Label(
    window,
    textvariable=svar,
    height=10,
    width=GUI_IO_util.get_GUI_width(1),
    foreground="black",
