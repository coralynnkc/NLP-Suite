...
if (
    IO_libraries_util.install_all_Python_packages(GUI_util.window, "CoNLL table_analyzer", ["os", "tkinter", "pandas"])
    == False
):
    sys.exit(0)
...
def run():
    ...
    if (
        extra_GUIs_var.get() == False
        and all_analyses_var.get() == False
        and advanced_analyses_var.get() == False
        and search_token_var.get() == False
        and WordNet_var.get() == False
        and compute_sentence_var.get() == False
        and k_sentences_var.get() == False
    ):
        mb.showwarning(
    ...
    if all_analyses_var.get():
        ...
        startTime = IO_user_interface_util.timed_alert(
            ...
            2000,
            "Analysis start",
            "Started running CoNLL table " + label + " analyses at",
            True,
            "",
            True,
            "",
    ...
    if advanced_analyses_var.get():
        ...
        adv_startTime = IO_user_interface_util.timed_alert(
            GUI_util.window,
            2000,
            "Analysis start",
            "Started running CoNLL Advanced analyses at",
            True,
            "",
            True,
            "",
        ...
        adv_outputDir = (
            IO_files_util.make_output_subdirectory(
                inputFilename, "", outputDirSV, label="CoNLL_Advanced_analyses", silent=True
            )
            or outputDirSV
        )
        ...
        if sel == "*" or sel == "Word Sense Disambiguation (WSD)":
            ...
            for noun_verb in noun_verb_list:
                outFiles = semantic_aggregation_util.wsd_aggregate_WordNet(
                    inputFilename, adv_outputDir, noun_verb, chartPackage, dataTransformation
                )
                if outFiles:
                    filesToOpen.extend(outFiles)
        if sel == "*" or sel == "Zoom OUT/UP by Sentence Index":
            ...
            cfg = config_util.read_NLP_package_language_config()
            cfg_language = cfg[4] if len(cfg) > 4 else ""
            nf_csv, noun_lemma_csv, vf_csv, verb_lemma_csv = CoNLL_util.get_nouns_verbs_CoNLL(
                inputFilename, adv_outputDir
            )
            for noun_verb in noun_verb_list:
                lemma_csv = noun_lemma_csv if noun_verb == "NOUN" else verb_lemma_csv
                # generate the aggregation dictionary (Zoom OUT/UP) for this resource + class
                agg_files = semantic_aggregation_util.aggregate(
                    resource,
                    "",
                    lemma_csv,
                    adv_outputDir,
                ...
                dict_path = next(
                    (
                        f
                        for f in (agg_files or [])
                        if "_up_" in os.path.basename(f).lower() and "frequency" not in os.path.basename(f).lower()
                    ),
                    "",
                )
                ...
                outputFilename = IO_files_util.generate_output_file_name(
                    inputFilename, "", adv_outputDir, ".csv", resource + "_" + noun_verb, "conll"
                )
                outFiles = semantic_aggregation_WordNet_util.Wordnet_bySentenceID(
                    inputFilename,
                    dict_path,
                    outputFilename,
                    adv_outputDir,
        ...
        if sel == "*" or sel == "Beginning-End K sentences analyzer (repetition finder)":
            ...
            if not end_k:
                end_k = 2
            temp_outputDir, outFiles = CoNLL_k_sentences_util.k_sent(
                inputFilename, adv_outputDir, chartPackage, dataTransformation, begin_k, end_k
            )
            if outFiles:
                filesToOpen.extend(outFiles)
        IO_user_interface_util.timed_alert(
            ...
            2000,
            "Analysis end",
            "Finished running CoNLL Advanced analyses at",
            True,
            "",
            True,
            adv_startTime,
    ...
    if search_token_var.get() and searchField_kw != "e.g.: father":
        ...
        startTime = IO_user_interface_util.timed_alert(
            GUI_util.window, 2000, "Analysis start", "Started running CoNLL search at", True, "", True, "", True
        )
    ...
    if WordNet_var.get():
        ...
        output = semantic_aggregation_util.aggregate(
            "*",
            WordNetDir,
            inputFilename_nouns,
            outputDir,
...
noun_verb_menu_var.set(
    "NOUN & VERB"
)  # default: process both; pick 'NOUN' or 'VERB' in the (user-placed) dropdown to restrict
...
WordNet_var = (
    tk.IntVar()
)  # dormant: old WordNet checkbox removed (Advanced menu replaces it); kept defined so run()/clear() don't NameError
...
csv_file_button = tk.Button(
    ...
    command=lambda: IO_files_util.get_corpus_CoNLL_csv(
        window, csv_file_var, "Select INPUT csv CoNLL table file", [("csv files", "*.csv")], True
    ),
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.IO_configuration_menu,
    "Open INPUT csv CoNLL table file",
)
...
extra_GUIs_menu = tk.OptionMenu(
    window,
    extra_GUIs_menu_var,
    "Ngrams searches & VIEWER",
    "Word searches",
    "Wordnet searches",
    "Data manipulation GUI for more options on querying the CoNLL table",
    "Corpus statistics",
    "Style analysis",
)
...
run_parser_checkbox = tk.Checkbutton(
    ...
    onvalue=1,
    offvalue=0,
    command=lambda: run_parser_toggled(),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
all_analyses_var = tk.IntVar()
all_analyses_checkbox = tk.Checkbutton(
    window,
    state="disabled",
    variable=all_analyses_var,
    text="Basic CoNLL analyses",
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, all_analyses_checkbox, True
)

all_analyses.set("*")
all_analyses_menu = tk.OptionMenu(
    window,
    all_analyses,
    "*",
    "Clause analysis",
    ...
    "Adjective analysis",
    "Adverb analysis",
    "Function (junk/stop) words analysis",
    "Content/Function ratio analysis",
)
all_analyses_menu.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, all_analyses_menu, False
)

advanced_analyses_var = tk.IntVar()
advanced_analyses_checkbox = tk.Checkbutton(
    window,
    state="disabled",
    variable=advanced_analyses_var,
    text="Advanced CoNLL analyses",
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, advanced_analyses_checkbox, True
)
advanced_analyses.set("*")
advanced_analyses_menu = tk.OptionMenu(
    window,
    advanced_analyses,
    "*",
    "Classification of Nouns & Verbs via FrameNet, VerbNet, WordNet",
    "Beginning-End K sentences analyzer (repetition finder)",
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, advanced_analyses_menu, True
)
...
searchToken_checkbox = tk.Checkbutton(
    window,
    state="disabled",
    variable=search_token_var,
    text="Search token/word",
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, searchToken_checkbox, True
)
...
entry_searchField_kw = tk.Entry(
    window, width=GUI_IO_util.combobox_width, state="disabled", textvariable=searchField_kw_var
)
# place widget with hover-over info
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, searchedCoNLLdescription_csv_field_menu_lb
)

k_words_var.set(0)
k_words_checkbox = tk.Checkbutton(
    window,
    text="Before-After K words",
    variable=k_words_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
...
before_K_words_entry_lb = tk.Label(window, text="Before K-words")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_reminders_x_coordinate, y_multiplier_integer, before_K_words_entry_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.open_reminders_x_coordinate + 130,
    y_multiplier_integer,
    before_K_words_entry,
    True,
    False,
    False,
    False,
...
after_K_words_entry_lb = tk.Label(window, text="After K-words")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.run_button_x_coordinate, y_multiplier_integer, after_K_words_entry_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.run_button_x_coordinate + 120,
    y_multiplier_integer,
    after_K_words_entry,
    False,
    False,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, search_token_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate + 140, y_multiplier_integer, POS_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, postag_menu_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_setup_x_coordinate, y_multiplier_integer, DEPREL_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_setup_x_coordinate + 70, y_multiplier_integer, deprel_menu_lb
)

# Co-Occurring POSTAG menu
CoOc_lb = tk.Label(window, text="Co-occurring tokens")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, CoOc_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate + 140, y_multiplier_integer, POSTAG_CoOc_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, co_postag_menu_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_setup_x_coordinate, y_multiplier_integer, DEPREL_CoOc_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_setup_x_coordinate + 70, y_multiplier_integer, co_deprel_menu_lb
)
...
def activate_all_options():
    ...
    if extra_GUIs_var.get():
        ...
        reminders_util.checkReminder(
            scriptName, reminders_util.title_options_CoreNLP_nn_parser, reminders_util.message_CoreNLP_nn_parser, True
        )
...
videos_lookup = {
    "CoNLL Table Analyzer 1": "https://www.youtube.com/watch?v=I-knso52gbM",
    "CoNLL Table Analyzer 2": "https://youtu.be/5pkHaCJmsPk",
}
...
TIPS_options = (
    "CoNLL Table",
    "POSTAG (Part of Speech Tags)",
    "DEPREL (Stanford Dependency Relations)",
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Tick 'Run the default parser (Open GUI)' when you do not yet have a CoNLL table: it opens the Parsers & Annotators GUI (with the parser and 'Open CoNLL table analyzer' pre-ticked). Run it there to parse your corpus with the configured parser, and this analyzer reopens loaded with the fresh CoNLL table."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to search the CoNLL table for a selected search word and extract a selected number of words appearing BEFORE and AFTER in a sentence."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        ...
        "Select DEPREL value for searched token (e.g., nsubjpass for passive nouns that are subjects; RETURN for ANY DEPREL value)."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        ...
        "Select DEPREL value for token co-occurring in the same sentence (e.g., DEPREL nsubjpass for passive nouns that are subjects; RETURN for ANY DEPREL value)."
        + GUI_IO_util.msg_Esc,
    )
