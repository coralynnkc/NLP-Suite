...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "GIS_symbolic_main", ["os", "tkinter", "pandas", "numpy", "scipy", "matplotlib", "nltk"]
    )
    == False
):
    sys.exit(0)
...
def run():
    ...
    if not inputFilename or not os.path.isfile(inputFilename):
        ...
        if do_extract:
            ...
            conlls = CoNLL_util.find_corpus_CoNLL(
                outputDir, GUI_util.inputFilename.get(), GUI_util.input_main_dir_path.get()
            )
            if conlls and mb.askyesno(
                title="Use this CoNLL for BUILD?",
    ...
    if not outputDir:
        mb.showwarning(
            title="No output directory", message="Please select an OUTPUT files directory (the top I/O row, or Setup)."
        )
        return
    ...
    try:
        ...
        mb.showerror(title="Could not read CSV", message=f"Failed to read the input CSV:\n{e}")
        return

    if location_col not in df.columns:
        mb.showwarning(
            title="Column not found", message=f"The location column '{location_col}' is not in the CSV."
        )
        return
    ...
    if do_distribution:
        if not attribute_col:
            mb.showwarning(
                title="No attribute column",
                message="The Distribution analysis needs an Attribute column (gender, race, class…).",
            )
            return
        ...
        if table.empty:
            ...
            csv_out = os.path.join(outputDir, f"symbolic_space_{base}_x_space.csv")
            ss.save_crosstab_csv(table, csv_out, stats)
            filesToOpen.append(csv_out)
            png = os.path.join(outputDir, f"symbolic_space_{base}_x_space_heatmap.png")
            ss.plot_attribute_space_heatmap(
                table, png, values="residuals", stats=stats, title=f"{attribute_col} × non-geocodable space"
            )
...
def select_csv_file():
    ...
    if not matches:
        mb.showinfo(
            title="No Social-actors table yet",
        ...
        f = filedialog.askopenfilename(
            title="Select INPUT csv file", filetypes=[("csv files", "*.csv"), ("All files", "*.*")]
        )
        if f:
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, location_col_label, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, sequence_col_label, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, attribute_col_label, True
)
...
def refresh_columns(*args):
    ...
    if path and os.path.isfile(path):
        try:
            ...
            cols = []
    for col_var, menu in (
        (location_col_var, location_col_menu),
...
TIPS_lookup = {
    "Narrative / non-geocodable symbolic space": "TIPS_NLP_GIS Narrative non-geocodable symbolic space.pdf",
...
TIPS_options = (
    "Narrative / non-geocodable symbolic space",
...
readMe_message = (
    "This script analyzes NON-geocodable space — the kinds of place that carry social / symbolic "
