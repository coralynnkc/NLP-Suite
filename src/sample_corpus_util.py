...
def sample_corpus_by_search_words_inFileName(window, inputDir, configFileName, keywords_inFilename):
    ...
    if Ndocs == 0:
        ...
        mb.showwarning(
            ...
            + keywords_inFilename
            + '" in the filename were copied from the input directory\n\n'
            + inputDir
...
def sample_corpus_by_document_id(table, inputDir, outputDir):
    ...
    inputFilename = ""
    target_dir = IO_files_util.make_output_subdirectory(
        inputFilename, inputDir, outputDir, label="sampleDir", silent=True
    )
    if target_dir == "":
        return
    ...
    mb.showwarning(
        title="Warning",
        message="The sample function successfully copied "
        + str(len(doc_loc))
        + " files to the sub-folder "
        + target_dir
        + extra_msg,
    )
    return
