...
def csvFile_has_header(file_path):
    ...
    if not os.path.exists(file_path):
        return is_header
    reader = csv.reader(open(file_path, encoding="utf-8", errors="ignore"))
    i = next(reader)
...
def get_csvfile_headers(csvFile, ask_Question=False):
    ...
    if csvFile != "" and answer == True:
        with open(csvFile, encoding="utf-8-sig", errors="ignore") as f:
            ...
            except OSError as e:  # empty files will break the next
...
def GetNumberOf_Records_Columns_inCSVFile(inputFilename, encodingValue="utf-8"):
    ...
    nColumns = 0
    try:
        nColumns = pd.read_csv(inputFilename, encoding=encodingValue, on_bad_lines="skip", dtype=str, nrows=0).shape[1]
        for chunk in pd.read_csv(
            inputFilename, encoding=encodingValue, on_bad_lines="skip", dtype=str, chunksize=100000
        ):
            nRecords += len(chunk)
    except Exception:
        # RF @@@@@
