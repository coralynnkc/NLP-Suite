...
if (
    IO_libraries_util.install_all_Python_packages(GUI_util.window, "CoReference Resolution", ["os", "tkinter", "re"])
    == False
):
    sys.exit(0)
...
def createCorefCompareWindow(origin_display, coref_display, origin_non_coref, corefed_non_coref, root, result):
    ...
    top.title(
        "Corefed results compared (LEFT: ORIGINAL text; RIGHT: COREFED text). In BLUE pronouns NOT corefed; in RED pronouns corefed. EDIT text on the right. Use FIND bar to search text. CLOSE to exit and save changes"
    )
    ...
    for i in range(len(origin_display)):
        ...
        for highlight in each[1]:
            text1.tag_add("here", str(i + 1) + "." + str(highlight[0]), str(i + 1) + "." + str(highlight[1]))
            text1.tag_config("here", background="red", foreground="blue")
        for highlight in non_coref[1]:
            text1.tag_add("coref", str(i + 1) + "." + str(highlight[0]), str(i + 1) + "." + str(highlight[1]))
            text1.tag_config("coref", background="blue", foreground="yellow")
    ...
    for i in range(len(coref_display)):
        ...
        for highlight in each[1]:
            text2.tag_add("here", str(i + 1) + "." + str(highlight[0]), str(i + 1) + "." + str(highlight[1]))
            text2.tag_config("here", background="red", foreground="blue")
        for highlight in non_coref[1]:
            text2.tag_add("coref", str(i + 1) + "." + str(highlight[0]), str(i + 1) + "." + str(highlight[1]))
            text2.tag_config("coref", background="blue", foreground="yellow")
    ...
    def exit_btn():
        ...
        answer = tk.messagebox.askyesnocancel(
            "Exit Manual Editing", "Do you want to SAVE any changes you have made in manual editing before quitting?"
        )
        if answer:  # save
            exit_btn_save()
    ...
    def find():
        ...
        if s:
            ...
            while 1:
                ...
                idx = text2.search(s, idx, nocase=1, stopindex=END)
                if not idx:
                    break
            ...
            while 1:
                # searches for desired string from index 1
                idx = text1.search(s, idx, nocase=1, stopindex=END)
                if not idx:
                    break
                # last index sum of current index and
                # length of text
...
def compare_results(origin_text, corefed_text):
    ...
    for i in range(0, min(len(corefed_sentences), len(origin_sentences))):
        ...
        origin_display_tuple = (origin_sentences[i], origin_display_highlighted)

        corefed_display_tuple = (corefed_sentences[i], corefed_display_highlighted)
...
def manualCoref(original_file, corefed_file, outputFile):
    f = open(original_file, encoding="utf-8", errors="ignore")
    original_text = f.read()
    f.close()
    f = open(corefed_file, encoding="utf-8", errors="ignore")
    corefed_text = f.read()
    f.close()
    ...
    result = []
    result.append("\n".join(corefed_text.split("\n")[2:]))
    answer = createCorefCompareWindow(
        origin_display, corefed_display, origin_non_coref, corefed_non_coref, GUI_util.window, result
    )
...
def run(
    ...
    CoreNLPdir, existing_software_config, errorFound = IO_libraries_util.external_software_install(
        "Stanford_CoreNLP_coreference_util", "Stanford CoreNLP", "", silent=False, errorFound=False
    )
    ...
    if manual_Coref:
        if len(inputDir) == 0 and len(inputFilename) > 0:
            ...
            IO_user_interface_util.timed_alert(
                ...
                "Manual Coreference is only available when processing single file, not input directory.",
            )
