...
def run():
    ...
    if (
        ...
        and compute_baseline_distances == False
        and compute_consecutive_distances == False
    ):
        mb.showwarning(
            title="Warning", message="No options have been selected.\n\nPlease, select an option and try again."
        )
        return
    ...
    if compute_baseline_distances and baselineLocation != "":
        baselineFiles = GIS_distance_util.computeDistancesFromSpecificLocation(
            GUI_util.window,
            inputFilename,
            outputDir,
            geolocator,
    ...
    if compute_pairwise_distances:
        ...
        pairwiseFiles = GIS_distance_util.computePairwiseDistances(
            GUI_util.window,
            inputFilename,
            outputDir,
            distinctValues,
    ...
    if compute_consecutive_distances:
        ...
        consecutiveFiles = GIS_distance_util.computeConsecutiveDistances(
            GUI_util.window, inputFilename, outputDir, distinctValues, encoding
        )
...
def _apply_selected_csv(f):
    ...
    try:  # re-evaluate RUN now that a csv is selected
        ...
        GUI_util.activateRunButton(
            GUI_util.config_filename_selected_config.get() or config_filename,
            IO_setup_display_brief,
            scriptName,
            _missing,
            True,
...
def select_csv_file():
    ...
    matches = CoNLL_util.find_corpus_csv(
        ...
        path_filter=_plausible_gis,
        validator=_is_geocoded_csv,
    )
    chosen = IO_files_util.select_path_from_list(
        window,
        matches,
        "Select a GEOCODED GIS csv (Latitude & Longitude columns) found for your corpus, or browse for another file:",
    ...
    if chosen == "__BROWSE__":
        ...
        f = filedialog.askopenfilename(
            title="Select INPUT geocoded csv file", filetypes=[("csv files", "*.csv"), ("All files", "*.*")]
        )
    else:
        f = chosen
    if f:
...
extra_GUIs_menu = tk.OptionMenu(
    window,
    extra_GUIs_menu_var,
    "GIS: Mapping locations (Open GUI)",
    "Google Earth (Open GUI)",
    "Symbolic (non-geocodable) space (Open GUI)",
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 350, y_multiplier_integer, encodingValue, True
)
...
compute_pairwise_distances_var.set(0)
compute_pairwise_distances_checkbox = tk.Checkbutton(
    window, variable=compute_pairwise_distances_var, onvalue=1, offvalue=0
)
compute_pairwise_distances_checkbox.config(text="Compute pairwise distances (all-pairs of distinct locations)")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, compute_pairwise_distances_checkbox, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 350, y_multiplier_integer, pairwise_scope_menu
)

compute_baseline_distances_var.set(0)
compute_baseline_distances_checkbox = tk.Checkbutton(
    window, variable=compute_baseline_distances_var, onvalue=1, offvalue=0
)
compute_baseline_distances_checkbox.config(text="Compute distances from baseline location")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, compute_baseline_distances_checkbox, True
)

baselineLocation_value_lb = tk.Label(window, text="Enter location ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 350, y_multiplier_integer, baselineLocation_value_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 450, y_multiplier_integer, baselineLocation_entry
)

compute_consecutive_distances_var.set(0)
compute_consecutive_distances_checkbox = tk.Checkbutton(
    window, variable=compute_consecutive_distances_var, onvalue=1, offvalue=0
)
compute_consecutive_distances_checkbox.config(
    text="Compute distances between consecutive locations (movement per document)"
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, compute_consecutive_distances_checkbox
)
...
TIPS_lookup = {
    ...
    "Geographic distances": "TIPS_NLP_GIS distances.pdf",
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    if IO_setup_display_brief == False:
        ...
        y_multiplier_integer = GUI_IO_util.place_help_button(
            window, help_button_x_coordinate, y_multiplier_integer, GUI_IO_util.msg_corpusData + GUI_IO_util.msg_Esc
        )
        y_multiplier_integer = GUI_IO_util.place_help_button(
            window,
            help_button_x_coordinate,
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to compute distances of all locations listed in your input file from a specific location (e.g., Atlanta). You will need to enter the location name (e.g., again, Atlanta).\n\nIn INPUT the script expects either\n   1. a list of locations that will be geocoded before computing distances from a baseline location. The input file must have a column of locations (selected in the FIRST selected location names).\n   2. geocoded data with Latitude and Longitude values for a set of locations whose distances from a baseline location you want to compute. The input file must have a column with the FIRST selected location name, followed by its latitude and longitude."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to compute MOVEMENT distances, i.e., the distance between each geolocated location and the NEXT one within the same document (how far the narrative/characters move from place to place across a story).\n\nIn INPUT the script expects a single GEOCODED csv (as produced by the GIS mapping tool) with Latitude and Longitude columns. Locations are ordered by Sentence ID within each Document and paired in sequence, so a single-location geocoded file is all you need: the FIRST/SECOND location columns are NOT used by this option.\n\nIn OUTPUT the script lists each consecutive location pair with its geodesic and great circle distances (miles and Km), together with the Document and the from/to sentences."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
