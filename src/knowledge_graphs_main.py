...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "knowledge_graphs_main.py", ["os", "tkinter", "subprocess"]
    )
    == False
):
    sys.exit(0)
...
def run():
    ...
    if knowledge_graphs_var != "":
        if inputFilename != "" and inputFilename[-4:] != ".txt":
            mb.showwarning(
                title="Warning",
                message="You have selected to annotate your corpus, but the input file is not of type .txt as required by the selected annotator.\n\nPlease, select a .txt file (or a directory) and try again.",
            )
            return

    # create a subdirectory of the output directory
    outputDir = IO_files_util.make_output_subdirectory(
        inputFilename, inputDir, outputDir, label=knowledge_graphs_var, silent=False
    )
    if outputDir == "":
        return
    ...
    if "DBpedia" in knowledge_graphs_var:
        ...
        filesToOpen = knowledge_graphs_DBpedia_util.DBpedia_annotate(
            inputFilename,
            inputDir,
            outputDir,
            config_filename,
        ...
        # http://mappings.DBpedia.org/server/ontology/classes/
        color1 = "black"
        filesToOpen = knowledge_graphs_YAGO_util.YAGO_annotate(
            inputFilename,
            inputDir,
            outputDir,
            config_filename,
        ...
        filesToOpen = knowledge_graphs_Wikipedia_util.Wikipedia_annotate(
            inputFilename,
            inputDir,
            outputDir,
            config_filename,
...
GUI_label = (
    "Graphical User Interface (GUI) for HTML Annotating Documents Using the Knowledge Graphs (KG) DBpedia & YAGO"
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, knowledge_graphs_menu_lb, True
)
...
confidence_level_entry.set(0.5)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, ontology_class_lb, True
)

ontology_class_var.set("")
ontology_class = ttk.Combobox(
    window, state="disabled", width=GUI_IO_util.widget_width_long, textvariable=ontology_class_var
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, sub_class_entry_lb, True
)
...
create_HTML_files_checkbox = tk.Checkbutton(
    window, text="Create HTML output files", state="disabled", variable=create_HTML_files_var, onvalue=1, offvalue=0
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.labels_x_indented_indented_coordinate,
    y_multiplier_integer,
    create_HTML_files_checkbox,
    True,
    False,
    False,
    False,
...
bold_DBpedia_YAGO_checkbox = tk.Checkbutton(
    window, text="Bold", state="disabled", variable=bold_DBpedia_YAGO_var, onvalue=1, offvalue=0
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_reminders_x_coordinate, y_multiplier_integer, color_palette_DBpedia_YAGO_lb, True
)
color_palette_DBpedia_YAGO_menu = tk.OptionMenu(
    window, color_palette_var, "black", "blue", "green", "pink", "red", "yellow"
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    y_multiplier_integer,
    color_palette_DBpedia_YAGO_menu,
    False,
    False,
    False,
    False,
...
add_class_button = tk.Button(
    ...
    height=1,
    state="disabled",
    command=lambda: activate_class_var(),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
reset_class_button = tk.Button(
    window,
    text="Reset ",
    width=GUI_IO_util.reset_button_width,
    height=1,
    state="disabled",
    command=lambda: clear_ontology_list(),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
def show_class_color_list():
    ...
    if not color_map:
        if color_palette_var.get() != "":
            mb.showwarning(
                title="Warning",
                message="You have selected the color "
                + color_palette_var.get()
                + " for the ontology class "
                + ontology_class_var.get()
                + "\n\nYou must press the + button to OK your selection.",
            )
        else:
            mb.showwarning(
                title="Warning", message="There are no currently selected combinations of ontology class and color."
            )
    else:
        class_color_string = ""
        ...
        mb.showwarning(
            title="Warning",
            message="The currently selected combination of ontology classes and colors are:\n\n"
            + class_color_string
            + "\n\nPlease, press the RESET button (or ESCape) to start fresh.",
        )


show_class_color_button = tk.Button(
    window,
    text="Show",
    width=GUI_IO_util.show_button_width,
    height=1,
    state="disabled",
    command=lambda: show_class_color_list(),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
def activate_DBpedia_YAGO_Options(y_multiplier_integerSV, confidence_level_lb, confidence_level_entry, *args):
    ...
    if (
        "DBpedia" not in knowledge_graphs_var.get()
        and "YAGO" not in knowledge_graphs_var.get()
        and "Wiki" not in knowledge_graphs_var.get()
    ):
        ontology_list.clear()
        ontology_color_list.clear()
    ...
    if "DBpedia" in knowledge_graphs_var.get():
        ontology_class["values"] = constants_util.DBpedia_ontology_class_menu
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window, GUI_IO_util.setup_IO_brief_coordinate, y_multiplier_integerSV, confidence_level_lb, True
        )
        # place widget with hover-over info
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window,
            GUI_IO_util.confidence_level_entry_pos,
    ...
    if "DBpedia" in knowledge_graphs_var.get() or "YAGO" in knowledge_graphs_var.get():
        ...
        if firstTime == False:
            reminders_util.checkReminder(
                scriptName, reminders_util.title_options_DBpedia_YAGO, reminders_util.message_DBpedia_YAGO, True
            )
        firstTime = True
        ontology_class.configure(state="normal")
...
knowledge_graphs_var.trace(
    "w",
    callback=lambda x, y, z: activate_DBpedia_YAGO_Options(
        y_multiplier_integerSV, confidence_level_lb, confidence_level_entry
    ),
)
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Press the + button to add another ontology class.\n\nPress RESET (or ESCape) to delete all values entered and start fresh.\n\nPress SHOW to display all selected values.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
