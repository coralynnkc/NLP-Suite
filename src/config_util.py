...
def write_external_software_config_file(
    window, config_filename, currently_selected_options, currently_selected_parsers
):
...
def read_config_file(config_filename, config_input_output_numeric_options):
    ...
    if os.path.isfile(configFilePath) == True:
        ...
        try:
            ...
            mb.showwarning(
                title="Ill formatted config filename",
                message="The config filename\n\n"
                + configFilePath
        ...
        if len(config_input_output_alphabetic_options[0]) == 2:
            mb.showwarning(
                title="Obsolete csv config file structure",
                message="The config filename\n\n"
                + configFilePath
            ...
            while os.path.isfile(configFilePath) == False:
                ...
                if os.path.isfile(configFilePath) == False:
                    mb.showwarning(
                        title="Missing IO configuration data ",
...
def check_missing_IO(window, config_filename, scriptName, IO_setup_display_brief, missing_IO, silent=False):
    ...
    if missing_IO != "":
        Run_Button_Off = True
        if not silent:
            if (not IO_setup_display_brief) and ("setup_IO_main" not in scriptName):
                mb.showwarning(
                    title="Warning",
                    message="The following required INPUT/OUTPUT information is missing in config file "
                    + config_filename
                    + ":\n\n"
                    + missing_IO
                    + "\n"
                ...
                open_setup_IO_GUI = mb.askokcancel(
                    title="Warning",
                    message="The following required INPUT/OUTPUT information is missing in config file "
                    + config_filename
                    + ":\n\n"
...
def write_IO_config_file(
    window, config_filename, config_input_output_numeric_options, config_input_output_alphabetic_options, silent=False
):
    # check that the config directory exists inside the NLP main directory
    ...
    try:
        ...
        csv_file.close()
        # @@@
        config_file_exists, input_filename, input_dir, input_dir_last_item, new_config_filename = (
            get_input_file_directory(config_filename)
        )
