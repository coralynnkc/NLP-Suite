...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "file_checker_converter_cleaner_main.py", ["tkinter", "importlib"]
    )
    == False
):
    sys.exit(0)
...
pydict["Remove all characters between a set of characters (e.g., []) from text file(s)"] = [
    "file_cleaner_util.remove_characters_between_characters"
]
pydict["Remove end-of-line typesetting hyphenation and join split parts"] = [
    "file_cleaner_util.remove_typeseting_hyphenation"
]
...
pydict["Add missing blank after punctuation (e.g., blank between wrongly joined sentences)"] = [
    "file_cleaner_util.add_missing_blank_after_punctuation"
]
pydict["Add full stop (.) at the end of paragraphs without end-of-paragraph punctuation"] = [
    "file_cleaner_util.add_full_stop_to_paragraph"
]
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, check_lb, True
)
check_menu = tk.OptionMenu(
    window,
    check_tools_var,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, check_menu
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, convert_lb, True
)
convert_menu = tk.OptionMenu(
    window,
    convert_tools_var,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, convert_menu
)

clean_tools_var.set("")
clean_lb = tk.Label(window, text="Clean Files")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, clean_lb, True
)
clean_menu = tk.OptionMenu(
    window,
    clean_tools_var,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, clean_menu
)
...
bydictionary_value_checkbox = tk.Checkbutton(
    window,
    state="disabled",
    text="Replace strings in input file(s) via a csv file containg two columns: old string, new string.",
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.labels_x_indented_coordinate,
    "Tick the checkbox to clean txt files by replacing strings via a csv file containg two columns: old string, new string..\nOnly the first two columns will be considered; any other colum will be ignored.",
)
...
dictionary_button = tk.Button(
    ...
    command=lambda: get_dictionary_file(window, "Select INPUT string replace file", [("csv files", "*.csv")]),
)
# the button widget has hover-over effects (no_hover_over_widget=False) and the info displayed is in text_info
# the two x-coordinate and x-coordinate_hover_over must have the same values
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.labels_x_indented_coordinate,
    "Click to select the csv file to be used to replace strings in your corpus.",
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.file_search_byWord_openInputFile_button_pos,
    "Open selected string-replace csv file",
)
...
TIPS_lookup = {
    ...
    "File content checker & converter & cleaner": "TIPS_NLP_File checker & converter & cleaner.pdf",
    ...
    "Newspaper titles (separate titles from articles)": "TIPS_NLP_Newspaper title.pdf",
}
TIPS_options = (
    "File content checker & converter & cleaner",
    ...
    "File merger",
    "File splitter",
    "Newspaper titles (separate titles from articles)",
)
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, using the dropdown menu, select one of the options available for checking txt files.\n\nWhen a directory is selected as the input option, all files in a directory and its subdirectories can be checked. The script will ask users whether they want to check files in subdirectories.\n   When a file is selected in input, it must be of the type (e.g., pdf) required by the converter (e.g., pdf converter)."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
