...
if (
    IO_libraries_util.install_all_Python_packages(GUI_util.window, "file_search_byWord_main.py", ["os", "tkinter"])
    == False
):
    sys.exit(0)
...
def run():
    ...
    if extra_GUIs_var == False and search_by_dictionary == False and search_by_keyword == False:
        mb.showwarning(
            title="Input error",
            message="No search options have been selected.\n\nPlease, select a search option and try again.",
        )
        return

    if search_options_menu_var.get() != "" and search_options_menu_var.get() not in str(search_options_list):
        result = mb.askyesno(
            title="Warning",
            message="There is a search value '"
    ...
    IO_user_interface_util.timed_alert(
        GUI_util.window,
        2000,
        "Word/collocation search start",
        "Started running Word/collocation search at",
        True,
...
extra_GUIs_menu = tk.OptionMenu(
    ...
    extra_GUIs_menu_var,
    "Spell checker",
    "CoNLL table searches",
    "N-grams_Co-Occurrences searches & VIEWER",
    "Wordnet searches",
    "Style analysis",
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, search_options_menu_lb, True
)

search_options_menu = tk.OptionMenu(
    window,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, search_options_menu, True
)

add_search_button = tk.Button(
    window,
    text="+",
    width=GUI_IO_util.add_button_width,
    height=1,
    state="disabled",
    command=lambda: activate_search_var(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.file_search_byWord_add_search_button_pos, y_multiplier_integer, add_search_button, True
)
...
reset_search_button = tk.Button(
    ...
    height=1,
    state="disabled",
    command=lambda: reset_search_options_list(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.file_search_byWord_reset_search_button_pos, y_multiplier_integer, reset_search_button, True
)

show_search_button = tk.Button(
    window,
    text="Show",
    width=GUI_IO_util.show_button_width,
    height=1,
    state="disabled",
    command=lambda: show_search_options_list(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.file_search_byWord_show_search_button_pos, y_multiplier_integer, show_search_button
)
...
def show_search_options_list():
    if len(search_options_list) == 0:
        ...
        mb.showwarning(
            title="Warning",
            message="The currently selected SEARCH options are:\n\n  "
...
def activate_default_search_options(*args):
    if "Case sensitive (default)" not in search_options_list:
...
def activate_search_options(*args):
    if search_options_menu_var.get() != "":
        ...
        if "Search within document" in search_options_menu_var.get() and "Search within sentence (default)" in str(
            search_options_list
        ):
            search_options_list.remove("Search within sentence (default)")
            extract_sentences_checkbox.configure(state="disabled")
        if "Search within sentence (default)" in search_options_menu_var.get() and "Search within document" in str(
            search_options_list
        ):
        ...
        if search_options_menu_var.get() not in search_options_list:
...
search_by_dictionary_checkbox = tk.Checkbutton(
    window,
    text="Search corpus by dictionary value",
    variable=search_by_dictionary_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, search_by_dictionary_checkbox
)
...
search_by_keyword_checkbox = tk.Checkbutton(
    window,
    text="Search corpus by word(s)",
    variable=search_by_keyword_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.file_search_byWord_extract_sentences_search_words_entry_pos,
    y_multiplier_integer,
    keyword_value,
    True,
    False,
...
extract_sentences_checkbox = tk.Checkbutton(
    window, text="", variable=extract_sentences_var, onvalue=1, offvalue=0, command=lambda: activate_all_options()
)
extract_sentences_checkbox.configure(state="disabled")
...
coOccurring_keywords_checkbox = tk.Checkbutton(
    window, text="", variable=coOccurring_keywords_var, onvalue=1, offvalue=0, command=lambda: activate_all_options()
)
...
create_subcorpus_checkbox = tk.Checkbutton(
    window, text="", variable=create_subcorpus_var, onvalue=1, offvalue=0, command=lambda: activate_all_options()
)
