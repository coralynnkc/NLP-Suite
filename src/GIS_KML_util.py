...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "GIS_KML_util", ["tkinter", "csv", "simplekml", "datetime"]
    )
    == False
):
    sys.exit(0)
...
def generate_kml(
    ...
    startTime = IO_user_interface_util.timed_alert(
        window, 2000, "GIS kml generator", "Started running KML algorithm at", True, "", True, "", silent=True
    )
    ...
    IO_user_interface_util.timed_alert(
        window, 2000, "GIS kml generator", "Finished running KML algorithm at", True, "", True, startTime, silent=True
    )
    try:
