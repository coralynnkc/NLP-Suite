...
def run():
    ...
    if sample_by_keywords_inFilename:
        if keywords_inFilename == "":
            mb.showwarning(
                title="Warning",
                message="You have selected the option of creating a subcorpus of the input files based on search words found in the filenames, but no search words have been entered.\n\nPlease, enter the search words that a filename must contain and try again.",
            )
            return
        import sample_corpus_util

        sample_corpus_util.sample_corpus_by_search_words_inFileName(
            window, inputDir, config_filename, keywords_inFilename
        )
    elif sample_by_keywords_inDocument:
        if keywords_inDocument == "":
            mb.showwarning(
                title="Warning",
                message="You have selected the option of creating a subcorpus of the input files based on search words foound in the input documents, but no search words have been entered.\n\nPlease, enter the search words that documents must contain and try again.",
            )
            return
        ...
        filesToOpen = file_search_byWord_util.search_sentences_documents(
            ...
            selectedCsvFile="",
            search_by_search_keywords=True,
            search_keywords_list=keywords_inDocument,
            create_subcorpus_var=True,
            search_options_list=[],
            lang="English",
        ...
        mb.showwarning(
            title="Warning", message="The selected option is not available yet. Please, check back soon.\n\nSorry!"
        )
        return
...
def option_not_available():
    mb.showwarning(title="Warning", message="The selected option is not available yet.\n\nSorry!")
...
y_multiplier_integer = y_multiplier_integer + 0.5
...
sample_by_documentID_checkbox = tk.Checkbutton(
    window,
    text="Sample files by Document ID",
    variable=sample_by_documentID_var,
    onvalue=1,
...
sample_by_documentID_button = tk.Button(
    window,
    text="Select csv file",
    width=GUI_IO_util.widget_width_extra_short,
    command=lambda: get_file(window, "Select INPUT csv file", [("csv files", "*.csv")]),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    ...
    90,
    GUI_IO_util.IO_configuration_menu,
    "Click on the button to select the input csv file",
)
...
openFile_button = tk.Button(
    ...
    text="",
    command=lambda: IO_files_util.openFile(window, selectedFile_var.get()),
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.setup_IO_brief_coordinate + 80, y_multiplier_integer, selectedFile
)

sample_by_date_var = tk.IntVar()
sample_by_date_checkbox = tk.Checkbutton(
    window,
    text="Sample files by date in filename",
    variable=sample_by_date_var,
    onvalue=1,
    command=lambda: activate_all_options(),
)
# place widget with hover-over info
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, where_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.setup_IO_brief_coordinate + 80,
    y_multiplier_integer,
    date_value,
    False,
    False,
    ...
    False,
    90,
    GUI_IO_util.date_char_sep_lb_coordinate,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, date_distance_value_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.entry_box_x_coordinate + 300,
    y_multiplier_integer,
    date_type,
    False,
    False,
...
sample_by_keywords_inFilename_checkbox = tk.Checkbutton(
    window,
    text="Sample files by string in filename",
    variable=sample_by_keywords_inFilename_var,
    onvalue=1,
    command=lambda: activate_all_options(),
)
# place widget with hover-over info
...
sample_by_keywords_inDocument_checkbox = tk.Checkbutton(
    window,
    text="Sample corpus by search word(s)",
    variable=sample_by_keywords_inDocument_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    GUI_IO_util.labels_x_coordinate,
    y_multiplier_integer,
    sample_by_keywords_inDocument_checkbox,
    True,
    False,
    True,
    False,
