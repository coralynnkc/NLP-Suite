...
def getReminders_list(scriptName, silent=False):
    ...
    try:
        ...
        if "Reset to ON all reminders" not in df.iloc[0, 1] or "Reset to OFF all reminders" not in df.iloc[1, 1]:
        ...
        if silent == False:
            mb.showwarning(
                title="Reminders file generated",
                message="The reminders.csv file saved in the reminders subdirectory was not found. If this is your first time running NLP Suite, do not worry. A default reminders.csv has been automatically generated for you.",
            )
...
def resetAllReminders(title):
    ...
    answer = tk.messagebox.askyesno(
        "Warning", "Are you sure you want to reset to " + status + " ALL reminders for ALL GUIs?"
    )
    if not answer:
        return
    df["Status"] = status
    ...
    tk.messagebox.showwarning(
        "Warning", "ALL reminders for ALL GUIs have been turned " + status + " in the " + remindersFile
    )
...
def resetReminder(scriptName, title):
    ...
    if title != "Open reminders":
        ...
        try:
            ...
            mb.showwarning(
                title="Reminders file error",
                message="The reminders.csv file saved in the reminders subdirectory does not contain the reminder '"
                + title
                + "'.\n\nPlease, let the NLP Suite development team know the problem so it can be fixed.",
            )
            return
