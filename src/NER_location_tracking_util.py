...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "NER_location_tracking_util", ["os", "tkinter", "pandas", "stanza"]
    )
    == False
):
    sys.exit(0)
...
def main(inputFilename, inputDir, outputDir, chartPackage="", dataTransformation="", language="en"):
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        inputFilename if inputFilename else inputDir, inputDir, outputDir, ".csv", "NER_entity_location_tracking"
    )
    ...
    summary = (
        ...
        .reset_index()
        .sort_values(["Entity", "First_Sentence"])
    )

    summaryFilename = IO_files_util.generate_output_file_name(
        inputFilename if inputFilename else inputDir, inputDir, outputDir, ".csv", "NER_entity_location_summary"
    )
    summary.to_csv(summaryFilename, index=False, encoding="utf-8")
    filesToOpen.append(summaryFilename)

    IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis end", "Finished running entity-location tracking at", True, "", True, startTime
    )
