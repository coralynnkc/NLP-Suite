...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "semantic_aggregation", ["os", "re", "csv", "tkinter", "nltk", "pandas"]
    )
    == False
):
    sys.exit(0)
...
def _aggregate_flat(resource, category_fn, inputFile, outputDir, noun_verb, chartPackage, dataTransformation):
    ...
    with open(csv1, "w", encoding="utf-8", newline="") as f:
        wtr = csv.DictWriter(f, fieldnames=["Word", cat_col])
        wtr.writeheader()
        wtr.writerows(rows)
    with open(csv2, "w", encoding="utf-8", newline="") as f:
        wtr = csv.DictWriter(f, fieldnames=[cat_col, "Frequency"])
        wtr.writeheader()
    ...
    if chartPackage and chartPackage != "No charts":
        of = charts_util.plot(
            ...
            outputDir,
            columns=[cat_col],
            title="Frequency of %s categories for %s" % (resource, noun_verb),
            x_label="%s %s category" % (resource, noun_verb),
            group_by=None,
        )
        if of:
...
def aggregate_VerbNet(inputFile, outputDir, noun_verb, chartPackage, dataTransformation):
    ...
    start = IO_user_interface_util.timed_alert(
        GUI_util.window, 3000, "Analysis start", "Started running VerbNet aggregation at", True, "", True
    )
    ...
    IO_user_interface_util.timed_alert(
        GUI_util.window, 3000, "Analysis end", "Finished running VerbNet aggregation at", True, "", True, start
    )
    return out
...
def aggregate_FrameNet(inputFile, outputDir, noun_verb, chartPackage, dataTransformation):
    ...
    start = IO_user_interface_util.timed_alert(
        GUI_util.window, 3000, "Analysis start", "Started running FrameNet aggregation at", True, "", True
    )
    ...
    IO_user_interface_util.timed_alert(
        GUI_util.window, 3000, "Analysis end", "Finished running FrameNet aggregation at", True, "", True, start
    )
    return out
...
def wsd_aggregate_WordNet(conll_file, outputDir, noun_verb, chartPackage, dataTransformation):
    ...
    try:
        ...
        mb.showwarning(
            title="Word sense disambiguation", message="Could not read the CoNLL table:\n\n%s\n\n%s" % (conll_file, e)
        )
        return []
    ...
    start = IO_user_interface_util.timed_alert(
        GUI_util.window, 3000, "Analysis start", "Started running Word sense disambiguation (Lesk) at", True, "", True
    )
    rows = []
    counts = defaultdict(int)
    ...
    IO_user_interface_util.timed_alert(
        ...
        3000,
        "Analysis end",
        "Finished running Word sense disambiguation (Lesk) at",
        True,
        "",
        True,
        start,
    ...
    with open(csv1, "w", encoding="utf-8", newline="") as f:
        ...
        wtr.writerows(rows)
    with open(csv2, "w", encoding="utf-8", newline="") as f:
        wtr = csv.DictWriter(f, fieldnames=["WordNet category", "Frequency"])
        wtr.writeheader()
        for cat, n in sorted(counts.items(), key=lambda x: -x[1]):
    ...
    if chartPackage and chartPackage != "No charts":
        of = charts_util.plot(
            ...
            title="Word-sense-disambiguated WordNet categories for %s" % noun_verb,
            x_label="WordNet category (WSD)",
            group_by=None,
        )
        if of:
            filesToOpen.extend(of if isinstance(of, list) else [of])
