...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "Stanford_CoreNLP_NER_extractor", ["os", "pandas", "tkinter"]
    )
    == False
):
    sys.exit(0)
...
def _find_main_NER_csv(files):
    ...
    for f in files:
        ...
        if "ner" in b and not any(
            x in b for x in ("freq", "chart", "hyperlink", "timeline", "summary", "tracking", "not-found", "geo-")
        ):
            return f
    return ""
...
def run():
    ...
    if not skip_NER_extraction and ("*" in NER_package or "BERT" in NER_package):
        if language != "English":
            mb.showwarning(
                title="Warning",
                message="NER in BERT is only available for the English language. Your currently selected language is "
                + language
                + "."
    ...
    if NER_entity_timeline:
        if "Stanza" not in NER_package and NER_package != "*":
            mb.showwarning(
                title="NER Entity Timeline",
                message="The NER Entity Timeline always uses Stanza as its NER engine, regardless of the NER package selected above ("
                + NER_package
                + ").\n\nThe NER extraction selected above will still run with "
                + NER_package
                + ".",
            )
        outputFiles = NER_entity_timeline_util.main(
    ...
    if not skip_NER_extraction:
        ...
        if nLocations > 0:
            if mb.askyesno(
                ...
                gis_subdir = IO_files_util.make_output_subdirectory(
                    "", "", os.path.dirname(ner_csv), label="GIS", silent=True
                )
                ...
                prepared_csv = GIS_pipeline_util.normalize_NER_csv_for_GIS(
                    ner_csv,
                    prepared_csv,
                    filename_embeds_date_var=filename_embeds_date_var,
                    date_format=date_format_var,
                ...
                if prepared_csv == "":
                    ...
                    gis_out = GIS_pipeline_util.GIS_pipeline(
                        GUI_util.window,
                        config_filename,
                        prepared_csv,
                        inputDir,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, NER_entry_lb, True
)
...
NER_packages_menu = tk.OptionMenu(
    window, NER_packages_var, "*", "BERT (English language model)", "IBM", "spaCy", "Stanford CoreNLP", "Stanza"
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.NER_NER_packages_menu_pos,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, NER_tag_lb, True
)
...
NER_tags_CoreNLP = [
    ...
    "PERSON",
    "ORGANIZATION",
    "--- All spatial expressions",
    "CITY",
    "STATE_OR_PROVINCE",
    "COUNTRY",
    "LOCATION",
    "--- All temporal expressions",
    "DATE",
    "TIME",
    "DURATION",
    "SET",
...
add_NER_button = tk.Button(
    window,
    text="+",
    width=GUI_IO_util.add_button_width,
    height=1,
    state="disabled",
    command=lambda: activate_NER_Options(True, False),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
reset_NER_button = tk.Button(
    window,
    text="Reset ",
    width=GUI_IO_util.reset_button_width,
    height=1,
    state="disabled",
    command=lambda: clear_NER_list(coming_from_add=False, coming_from_reset=True),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.NER_reset_NER_button_pos,
    y_multiplier_integer,
    reset_NER_button,
    True,
    False,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.NER_NER_entry_lb_pos, y_multiplier_integer, NER_entry_lb, True
)
...
NER_entity_timeline_checkbox = tk.Checkbutton(
    window,
    text="NER Entity Timeline (when people/places/organizations appear across narrative, via Stanza)",
    variable=NER_entity_timeline_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, NER_entity_timeline_checkbox
)
...
def add_NER_tag(coming_from_add, coming_from_reset):
    ...
    if ("BERT" in pkg) or ("spaCy" in pkg) or ("Stanza" in pkg):
        ...
        if "All NER tags" in sel:  # == '--- All NER tags':
            NER_list = NER_list
            NER_entry_var.set(
                "PERSON, ORGANIZATION, MISC, MONEY, NUMBER, ORDINAL, PERCENT, DATE, TIME, DURATION, SET, EMAIL, URL, CITY,STATE_OR_PROVINCE, COUNTRY, LOCATION, NATIONALITY, RELIGION, TITLE, IDEOLOGY, CRIMINAL_CHARGE,CAUSE_OF_DEATH"
            )
        elif sel == "--- All quantitative expressions":
    ...
    if sel != " " and "---" not in sel and "All NER tags" not in sel:
        ...
        if not current or current == full_default:
            ...
            mb.showwarning(
                ...
                message='The NER tag "'
                + sel
                + '" is already in your selection NER list: '
                + str(NER_entry_var.get())
        ...
        try:
            ...
            if Stanza_util.check_Stanza_annotator_availability(["NER"], short, long, silent=False):
                ...
                IO_user_interface_util.timed_alert(
                    ...
                    4000,
                    "Warning",
                    "There are no ad-hoc NER tags for the "
                    + language
                    + " language in Stanza.\n\nPlease, check NER output for "
                    + language
                    + "-specific NER tags.",
                )
    return NER_list
