...
def _run_ngrams(c):
    ...
    out, _ = statistics_txt_util.compute_character_word_ngrams(
        ...
        chartPackage=c["chartPackage"],
        dataTransformation=c["dataTransformation"],
    )
    return _files(out)
...
def _sentence_length_from_conll(conll_path, c):
    ...
    try:
        ...
        if not {"Form", "Sentence ID", "Document ID"}.issubset(df.columns):
            return []
        outdir = IO_files_util.make_output_subdirectory(
            c["inputFilename"], c["inputDir"], c["outputDir"], label="Statistics_txt_sent_length", silent=True
        )
        if not outdir:
            return []
        outfile = IO_files_util.generate_output_file_name(
            c["inputFilename"], c["inputDir"], outdir, ".csv", "sentence_length"
        )
        ...
        out_df = pd.DataFrame(
            rows, columns=["Sentence length (in words)", "Sentence ID", "Sentence", "Document ID", "Document"]
        )
        out_df.to_csv(outfile, index=False, encoding="utf-8")
        files = [outfile]
        charts = charts_util.plot(
            outfile,
            outdir,
...
def _run_sentence_length(c):
    ...
    if conll:
        ...
        if out:
            print(
                ">>> Sentence length: derived from the existing Stanza parse (%s) -- no re-parse"
    ...
    return _files(
        statistics_txt_util.compute_sentence_length(
            c["inputFilename"],
            c["inputDir"],
            c["outputDir"],
            c["config_filename"],
...
def _run_line_length(c):
    ...
    return _files(
        statistics_txt_util.compute_line_length(
            c["window"],
            c["config_filename"],
            c["inputFilename"],
            c["inputDir"],
...
def _run_yule(c):
    ...
    return _files(
        statistics_txt_util.yule(c["window"], c["inputFilename"], c["inputDir"], c["outputDir"], c["config_filename"])
    )
...
def _run_word_shape(keyword):
    ...
    def runner(c):
        ...
        return _files(
            statistics_txt_util.process_words(
                c["window"],
                c["config_filename"],
                c["inputFilename"],
                c["inputDir"],
...
def _run_unusual_words(c):
    ...
    return _files(
        file_spell_checker_util.nltk_unusual_words(
            c["window"],
            c["inputFilename"],
            c["inputDir"],
            c["outputDir"],
...
def _run_tfidf(c):
    ...
    return _files(
        statistics_corpus_tfidf_util.compute_tfidf(
...
def _run_lexical_diversity(c):
    ...
    return _files(
        statistics_corpus_lexical_diversity_util.compute_lexical_diversity(
            c["inputFilename"], c["inputDir"], c["outputDir"], c["chartPackage"], c["dataTransformation"]
        )
    )
...
def _run_word_frequency(c):
    ...
    return _files(
        statistics_corpus_word_frequency_util.compute_word_frequency(
            c["inputFilename"], c["inputDir"], c["outputDir"], c["chartPackage"], c["dataTransformation"]
        )
    )
...
def _find_existing_svo_csv(c):
    ...
    return _find_existing_parse_csv(c, ("svo", _configured_parser_tag(c)), ("Subject (S)", "Verb (V)", "Object (O)"))
...
def _find_existing_sentiment_csv(c):
    ...
    return _find_existing_parse_csv(c, ("sentiment", "stanza"), ("Sentiment score", "Sentiment label"))
...
def _run_ner(c):
    ...
    if existing:
        print(
            ">>> Entities/NER: reusing an existing %s NER table (%s) -- skipping the ~hour re-parse"
    ...
    if _is_corenlp_package(c.get("package")) and _corenlp_available():
        ...
        out = Stanford_CoreNLP_util.CoreNLP_annotate(
            ...
            c["export_json_var"],
            c["memory_var"],
            c["document_length_var"],
            c["limit_sentence_length_var"],
            NERs=NER_list,
        )
        return _files(out)
    if "spacy" in str(c.get("package", "")).lower():
...
def _run_gender_dialogue_dates(c):
    ...
    picked = _cache_pick_any(c.get("_corenlp_files"), "corenlp_gender", "corenlp_quote", "corenlp_normalized-date")
    if picked:
...
def _run_semantic_classes(c):
    ...
    for f, tag in ((verb_file, "VERB"), (noun_file, "NOUN")):
        ...
        try:
            out += _files(
                semantic_aggregation_WordNet_util.aggregate_GoingUP(
    ...
    if verb_file:
        for agg, name in (
            (semantic_aggregation_util.aggregate_VerbNet, "VerbNet"),
    ...
    if noun_file:
        try:
            out += _files(semantic_aggregation_util.aggregate_FrameNet(noun_file, c["outputDir"], "NOUN", cp, dt))
        except Exception as e:
...
def _find_existing_pos_csv(c):
    ...
    for p in glob.glob(os.path.join(outdir, "**", "*.csv"), recursive=True):
        ...
        if "pos" not in b or "stanza" not in b:
            continue
        if any(x in b for x in ("binned", "frequency", "chart", "no_hyperlinks", "group", "bydoc", "stats", "lemma")):
            continue
        try:
            head = pd.read_csv(p, nrows=5, encoding="utf-8", on_bad_lines="skip")
        except Exception:
...
def _run_pos_stats(c):
    ...
    if existing:
        print(
            ">>> Syntax/Semantics: reusing an existing Stanza POS table (%s) -- skipping the ~hour re-parse"
...
def _corenlp_available():
    ...
    print(
        ">>> Corpus Profiler: Stanford CoreNLP + Java installed = %s "
...
def _run_svo(c):
    ...
    if existing:
        print(
            ">>> Narrative/SVO: reusing an existing %s SVO table (%s) -- skipping the ~hours re-parse"
            % (_configured_parser_tag(c), os.path.basename(existing[0]))
        )
        return list(dict.fromkeys(_files(existing)))
    if _is_corenlp_package(c.get("package")) and _corenlp_available():
...
def _run_srl(c):
    ...
    if existing:
        print(
            ">>> Narrative/SRL: reusing an existing SRL table (%s) -- skipping the ~hours re-run"
            % os.path.basename(existing[0])
        )
        return list(dict.fromkeys(_files(existing)))
    import GUI_util
...
def _run_sentiment(c):
    ...
    if existing:
        print(
            ">>> Sentiment: reusing an existing Stanza sentiment table (%s) -- skipping the ~hours re-parse"
            % os.path.basename(existing[0])
        )
        return list(dict.fromkeys(_files(existing)))
    import Stanza_util
...
def _run_topics(c):
    ...
    return _files(
        topic_modeling_gensim_util.run_Gensim(
            c["window"],
            c["inputDir"],
            c["outputDir"],
            c["config_filename"],
...
def _run_character_arcs(c):
    ...
    return _files(
        character_emotion_arcs_util.main(
            c["inputFilename"],
            c["inputDir"],
            c["outputDir"],
            c["chartPackage"],
...
def _run_character_movement(c):
    ...
    if files:
        ...
        files = _files(
            NER_location_tracking_util.main(
                c["inputFilename"], c["inputDir"], c["outputDir"], c["chartPackage"], c["dataTransformation"]
            )
        )
    ...
    try:
        ...
        files += _files(
            charts_util.animated_migration_map(
...
REGISTRY = {
    # --- counts ---
    "statistics": dict(
        category="counts", kind="batch", run=_run_statistics, label="Statistics (sentences, words, syllables)"
    ),
    "ngrams": dict(category="counts", kind="batch", run=_run_ngrams, label="N-grams"),
    "sentence_length": dict(category="counts", kind="batch", run=_run_sentence_length, label="Sentence length"),
    ...
    "yule": dict(
        category="counts", kind="batch", run=_run_yule, label="Vocabulary richness (word type/token ratio or Yule's K)"
    ),
    "lexical_diversity": dict(
        category="counts", kind="batch", run=_run_lexical_diversity, label="Lexical diversity (TTR, MTLD, vocd-D)"
    ),
    "word_frequency": dict(
        category="counts", kind="batch", run=_run_word_frequency, label="Word frequency distribution (Zipf's Law)"
    ),
    "tfidf": dict(
        category="counts", kind="batch", run=_run_tfidf, label="TF-IDF (most distinctive words per document)"
    ),
    ...
    "unusual_words": dict(category="counts", kind="batch", run=_run_unusual_words, label="Unusual words (via NLTK)"),
    "abstract_concrete": dict(
        category="counts", kind="batch", run=_run_abstract_concrete, label="Abstract / concrete vocabulary"
    ),
    "iconic": dict(category="counts", kind="batch", run=_run_iconic, label="Iconic vocabulary"),
    "capital_words": dict(
        category="counts", kind="batch", run=_run_word_shape("capital"), label="Capital-initial words"
    ),
    ...
    "ner": dict(category="counts", kind="batch", run=_run_ner, label="People, organizations, locations (NER)"),
    ...
    "gender_dialogue_dates": dict(
        category="entities", kind="batch", run=_run_gender_dialogue_dates, label="Gender, dates, dialogue (via CoreNLP)"
    ),
    ...
    "semantic_classes": dict(
        category="semantics", kind="batch", run=_run_semantic_classes, label="Noun & verb classes (WordNet top synsets)"
    ),
    ...
    "semantics_more": dict(
        category="semantics",
        kind="gui",
        gui_script="semantic_analysis_main.py",
    ...
    "syntax_more": dict(
        category="syntax",
        kind="gui",
        gui_script="CoNLL_table_analyzer_main.py",
    ...
    "topics_gensim": dict(category="topics", kind="batch", run=_run_topics, label="Topic modeling (Gensim LDA)"),
    "topics_more": dict(
        category="topics",
        kind="gui",
        gui_script="topic_modeling_main.py",
    ...
    "narrative_svo": dict(category="narrative", kind="batch", run=_run_svo, label="SVO (Subject-Verb-Object)"),
    "narrative_srl": dict(category="narrative", kind="batch", run=_run_srl, label="SRL (Semantic Role Labeling)"),
    "narrative_more": dict(
        category="narrative", kind="gui", gui_script="SVO_main.py", label="Coreference · 5 Ws  (opens SVO GUI)"
    ),
    # --- sentiment (VADER runs by default; other engines open from the Sentiment GUI) ---
    "sentiment_stanza": dict(category="sentiment", kind="batch", run=_run_sentiment, label="Sentiment (Stanza)"),
    "sentiment_more": dict(
        category="sentiment",
        kind="gui",
        gui_script="sentiment_analysis_main.py",
    ...
    "character_arcs": dict(
        category="characters",
        kind="batch",
        run=_run_character_arcs,
        label="Emotion arcs (NRC 8 emotions, per character across the story)",
    ),
    "character_movement": dict(
        category="characters",
        kind="batch",
        run=_run_character_movement,
...
def setup_profile_output_dir(window, inputFilename, inputDir, outputDir):
    ...
    return IO_files_util.make_output_subdirectory(
        inputFilename, inputDir, outputDir, label="corpus_profile", silent=False
    )
...
def _prime_parse_cache(ctx, selected):
    ...
    if len(annotators) >= 2:  # only worth combining when 2+ CoreNLP dimensions are on
        ...
        if existing:
            print(
                ">>> Corpus Profiler: reusing an existing CoreNLP pass (%d files) -- skipping the "
            ...
            try:
                ...
                NER_list = ["PERSON", "ORGANIZATION", "CITY", "STATE_OR_PROVINCE", "COUNTRY", "LOCATION"]
                print(
                    ">>> Corpus Profiler: ONE combined CoreNLP pass for [%s] (replacing separate passes)"
                    % ", ".join(annotators)
                )
                files = _files(
                    Stanford_CoreNLP_util.CoreNLP_annotate(
                        ...
                        False,
                        ctx["chartPackage"],
                        ctx["dataTransformation"],
                        annotators,
                        False,
                        ctx["language"],
                        ctx["export_json_var"],
                        ctx["memory_var"],
                        ctx["document_length_var"],
                        ctx["limit_sentence_length_var"],
                        NERs=NER_list,
                    )
                )
...
def run_profile(ctx, selected):
    ...
    for aid in selected:
        ...
        rec = dict(
            ...
            files=[],
            error="",
            gui_script=entry.get("gui_script", ""),
        )
        if entry["kind"] == "batch":
            _done += 1
            _t0 = _time.time()
            print("\nStarted running %s (%d of %d) at %s" % (entry["label"], _done, _n, _time.strftime("%H:%M:%S")))
            try:
                rec["files"] = entry["run"](ctx) or []
                _dur = IO_user_interface_util.convert_time(_time.time() - _t0)[3]
                print(
                    "Finished running %s (%d of %d)%s."
...
"""
    ...
    parts.append(
        ...
        "paper-style summary</a>.</div></header>" % (_esc(corpus_name), _esc(run_config.get("subtitle", "")))
    )
    ...
    for key, lab in tiles:
        parts.append(
            ...
            % (_esc(f"{header_stats.get(key, 0):,}"), _esc(lab))
        )
    parts.append('<div class="stat"><b>%s</b><span>output files</span></div>' % _esc(f"{total_files:,}"))
    parts.append("</div>")
    ...
    for cat in CATEGORY_ORDER:
        ...
        parts.append(
            '<details id="%s" open><summary>%s &nbsp;<span class="empty">(%d files)</span></summary>'
            '<div class="cat-body"><table>' % (cat, _esc(CATEGORY_TITLE[cat]), n)
        )
        for r in recs:
            parts.append(
                '<tr><td class="analysis" colspan="2">%s%s</td></tr>'
                % (
                    _esc(r["label"]),
            ...
            if r.get("kind") == "gui":
                ...
                for f in r["files"]:
                    parts.append(
                        '<tr><td style="width:60px"></td>'
                        '<td><a href="%s">%s</a></td></tr>' % (_esc(_rel(f, report_dir)), _esc(os.path.basename(f)))
                    )
            elif not r["error"]:
                parts.append('<tr><td></td><td class="empty">(no output files)</td></tr>')
        parts.append("</table></div></details>")

    parts.append("</main><footer>%s</footer></body></html>" % _esc(run_config.get("footer", "")))
...
def _read_csv(path):
    ...
    try:
        ...
        for kw in ({"engine": "python", "on_bad_lines": "skip"}, {"engine": "python", "error_bad_lines": False}):
            try:
...
def _interp_vocabulary(files):
    ...
    return [
        (
            "Vocabulary richness (Yule’s K — lower means a more varied, less repetitive vocabulary) "
...
def _interp_tfidf_similarity(files):
    ...
    if not vals:
        return []
    line = (
        "Document similarity (TF-IDF cosine) gauges how much each PAIR of documents shares the same "
    ...
    if best[0] is not None:
        line += (
            ", and the most alike documents are “%s” and “%s” (%.2f). In the heatmap, bright blocks are "
...
def _interp_ner(files):
    ...
    if not f:
        ...
        f = _find(
            files, "_ner", exclude=("bydoc", "chart", "group", "no_hyperlinks", "stats", "gender", "svo", "lemma")
        )
    if not f:
        return findings
    df = _read_csv(f)
    if df is not None and "NER" in df.columns and "Word" in df.columns:
        ...
        if tags.sum() > 0:
            ...
            findings.append("The parser tagged %s entity mentions, led by %s." % (_thousands(tags.sum()), top_tags))
...
def _interp_entities(files):
    ...
    if fg:
        ...
        if df is not None and "Gender" in df.columns:
            ...
            if freqcol:
                ...
                if male + female > 0:
                    findings.append(
                        "Gendered references skew %s: %s male vs %s female mentions (%.0f%% male)."
                        % (
    ...
    if not findings:
        ...
        findings.append(
            _EMPH + "Gender, dialogue/quotes and normalized dates were NOT extracted: these "
...
def _interp_semantics(files):
    ...
    if vn:
        findings.append(
            "The same verbs sort into VerbNet classes (Levin-style syntactic-semantic verb classes) led by %s." % vn
        )
    ...
    if not findings:
        ...
        findings.append(
            "Noun & verb knowledge-base classes (WordNet / VerbNet / FrameNet) were not "
    ...
    if any(
        ("word2vec_vector" in os.path.basename(str(f)).lower() or "tsne" in os.path.basename(str(f)).lower())
        and str(f).lower().endswith((".html", ".htm"))
        for f in files
    ):
        findings.append(
            "BERT (all-distilroberta-v1) embedded the corpus’s most frequent words and projected them "
    ...
    if findings:
        ...
        findings.append(
            _EMPH + "NOMINALIZATION — recasting an action as a thing (“they decided” → “the "
...
def _interp_narrative(files):
    ...
    if fs:
        ...
        if df is not None and len(df):
            findings.append(
                "Subject–Verb–Object extraction produced %s triples — the backbone of "
            ...
            def _top(sub, label, n=6, drop=frozenset()):
                ...
                vals = vals[
                    (vals.str.len() > 0)
                    & (low != "nan")
                    & (~low.isin(_PRONOUNS | drop))
                    & (~low.str.startswith("inferred"))
                ]  # drop SVO placeholder tokens
                vc = vals.value_counts()
                if not len(vc):
                    return None
                return "The most frequent %s are %s." % (
                    label,
                    ", ".join("%s (%d)" % (v, int(c)) for v, c in vc.head(n).items()),
                )

            for _line in (
                _top("verb", "actions (verbs)", drop=_LIGHT_VERBS),
    ...
    if fr:
        ...
        if df is not None and "Role" in df.columns and "Count" in df.columns:
            ...
            if len(df):
                ...
                findings.append(
                    "Semantic-role labeling filled %s role slots, dominated by %s."
...
def _interp_sentiment(files):
    for cand in files:
        ...
        return [
            (
                "Across %s scored sentences the mood leans %s: %.0f%% positive, %.0f%% negative, %.0f%% neutral."
...
def _interp_topics(files):
    ...
    if not preview:
        return []
    return [
        "Gensim LDA distilled %d topics; the leading ones cluster around %s." % (len(df), "; ".join(preview)),
...
def _interp_arcs(files):
    ...
    f = _find(files, "character_emotion_arcs", exclude=("chart", "no_hyperlinks", "bydoc")) or _find(
        files, "character", "arc", exclude=("chart", "no_hyperlinks")
    )
    if not f:
        return []
    df = _read_csv(f)
    ...
    findings = [
        "Emotion arcs were traced for %d named character%s across the story; the most-followed are %s."
        % (
            len(counts),
            "" if len(counts) == 1 else "s",
...
def _interp_syntax(files):
    ...
    breakdown = ", ".join(
        "%s %s (%.0f%%)" % (_thousands(g(t)), name, 100 * g(t) / total) for t, name in _POS_NAMES if g(t) > 0
    )
    ...
    if verbs:
        ratio = nouns / verbs
        style = (
            "a nominal, descriptive style (more naming than doing)"
            if ratio > 1.5
            else (
                "a verbal, action-driven style (more doing than naming)"
                if ratio < 0.9
    ...
    findings.append(
        "Lexical density — content words (nouns, verbs, adjectives, adverbs) as a share of all "
    ...
    if g("AUX"):
        findings.append(
            "Auxiliaries make up %s tokens (%.0f%%) — the be/have/do and modal verbs that mark "
    ...
    if findings:
        ...
        findings.append(
            _EMPH + "PASSIVE VOICE — “X was killed” with no killer named — backgrounds or erases "
...
def _interp_characters(files):
    ...
    if mv is not None and len(mv):
        ...
        distinct = (
            mv.assign(_e=mv["Entity"].astype(str), _l=mv["Location"].astype(str))
            .groupby("_e")["_l"]
            .nunique()
            .sort_values(ascending=False)
        )
        findings.append(
            "Characters move through %d distinct places; the most frequented are %s."
            % (locs.nunique(), ", ".join("%s (%d)" % (l, int(n)) for l, n in locs.head(5).items()))
        )
        if len(distinct):
            findings.append(
                "The most mobile character is %s, appearing across %d different places."
...
def _interp_spatial(files):
    ...
    for cand in files:
        ...
        return [
            "The corpus names %s distinct places; the most frequent are %s. The top %s were geocoded "
...
def _interp_counts_vocabulary(files):
    ...
    groups = [
        ("Counts & measures", _interp_counts(files)),
...
def _interpret(category, files):
    ...
    fn = {
        "counts": _interp_counts_vocabulary,
        "entities": _interp_entities,
        "semantics": _interp_semantics,
        "narrative": _interp_narrative,
        "syntax": _interp_syntax,
        "sentiment": _interp_sentiment,
        "characters": _interp_characters,
        "topics": _interp_topics,
        "spatial": _interp_spatial,
    }.get(category)
    if not fn:
        return []
    try:
...
def _svg_bar(title, pairs, max_bars=8):
    ...
    out = [
        '<figure class="chart"><svg viewBox="0 0 %d %d" class="svgchart" '
    ...
    for lab, val in pairs:
        ...
        out.append(
            '<text x="%d" y="%d" class="svg-lab" text-anchor="end">%s</text>' % (label_w - 6, y + 14, _esc(disp))
        )
        out.append('<rect x="%d" y="%d" width="%.1f" height="14" rx="3" class="svg-bar"/>' % (label_w, y + 3, bw))
        out.append('<text x="%.1f" y="%d" class="svg-val">%s</text>' % (label_w + bw + 5, y + 14, _esc(_fmt_num(val))))
        y += row_h
    ...
    try:
        ...
        if category == "entities":
            ...
            if f:
                ...
                if df is not None and "NER" in df.columns:
                    ...
                    if "Word" in df.columns:
                        named = (
                            df[~df["Word"].astype(str).str.lower().isin(_PRONOUNS)]["Word"]
                            .astype(str)
                            .value_counts()
                            .head(8)
                        )
            ...
            for label, sub in (("Noun classes (WordNet)", "noun"), ("Verb classes (WordNet)", "verb")):
                ...
                if cc and fc:
                    d = (
                        df.assign(_f=pd.to_numeric(df[fc], errors="coerce"))
                        .dropna(subset=["_f"])
            ...
            for f in files:
                ...
                df = _read_csv(f)
                lc = (
                    next((c for c in df.columns if "sentiment label" in str(c).lower()), None)
                    if df is not None
                    else None
                )
                if lc:
            ...
            f = _find(files, "character_emotion_arcs", exclude=("chart", "no_hyperlinks", "bydoc")) or _find(
                files, "character", "arc", exclude=("chart",)
            )
            if f:
                df = _read_csv(f)
                if df is not None:
                    present = [c for c in df.columns if str(c).strip().capitalize() in _EIGHT_EMOTIONS]
                    sums = [
                        (str(c).strip().capitalize(), float(pd.to_numeric(df[c], errors="coerce").sum()))
                        for c in present
                    ]
            ...
            if fr:
                ...
                if df is not None and "Role" in df.columns and "Count" in df.columns:
                    d = (
                        df.assign(_c=pd.to_numeric(df["Count"], errors="coerce"))
                        .dropna(subset=["_c"])
            ...
            if f:
                ...
                if df is not None:
                    ...
                    if wc:
                        ...
                        pairs = [
                            ("Doc " + str(l), float(v))
                            for l, v in zip(labels, pd.to_numeric(df[wc], errors="coerce"))
                            if v == v
                        ]
                        if pairs:
            ...
            if f:
                ...
                if df is not None:
                    ...
                    if kc:
                        ...
                        pairs = [
                            ("Doc " + str(l), float(v))
                            for l, v in zip(labels, pd.to_numeric(df[kc], errors="coerce"))
                            if v == v
                        ]
                        if pairs:
...
def _make_wordcloud(inputDir, inputFilename, outputDir):
    ...
    try:
        ...
        paths = (
            sorted(_glob.glob(os.path.join(inputDir, "*.txt")))
            if inputDir
...
def build_paper_summary(
    outputDir, corpus_name, results, header_stats, run_config, report_basename="NLP_corpus_profile.html"
):
    ...
    abstract = (
        ...
        % (
            ...
            _fmt(avg_sents),
            len(ran),
            "is" if len(ran) == 1 else "es",
            len(cats_present),
            "" if len(cats_present) == 1 else "s",
            _esc(dims),
            _fmt(total_files),
...
"""
    ...
    parts.append(
        '<header class="paper"><div class="eyebrow">Corpus Profile · paper-style summary</div>'
        '<h1>%s</h1><div class="byline">%s</div></header>' % (_esc(corpus_name), _esc(run_config.get("subtitle", "")))
    )
    ...
    for _i, _cat in enumerate(cats_present, 1):
        _toc.append(
            '<li><span class="tnum">%d.</span><a href="#sec-%d">%s</a></li>' % (_i, _i, _esc(CATEGORY_TITLE[_cat]))
        )
    _toc.append(
        '<li><span class="tnum">+</span><a href="#sec-stats">Going further — statistics on these results</a></li>'
    )
    _toc.append("</ol></nav>")
    ...
    kpis = [
        (_fmt(n_docs), "documents"),
        (_fmt(n_words), "words"),
        (_fmt(avg_words), "avg words / document"),
        (_fmt(total_files), "output files"),
    ]
    ...
    if _wc_uri:
        parts.append(
            '<figure class="wordcloud"><img src="%s" alt="corpus wordcloud" loading="lazy">'
            "<figcaption>The corpus at a glance — its most frequent words.</figcaption></figure>" % _wc_uri
        )
    ...
    for cat in cats_present:
        ...
        sec_no += 1
        parts.append(
            '<section class="dim" id="sec-%d"><h2 class="dim"><span class="num">%d</span>%s</h2>'
        ...
        for _finding in _interpret(cat, cat_files):
            if _finding.startswith(_SUBHEAD):  # bold sub-heading labelling a sub-group of the dimension
                parts.append('<p style="font-weight:700;margin:16px 0 3px">%s</p>' % _esc(_finding[len(_SUBHEAD) :]))
            elif _finding.startswith(_EMPH):  # high-salience callout (e.g. agency markers) -> red + bold
                parts.append(
                    '<p class="finding" style="color:#c1121f;font-weight:700">%s</p>' % _esc(_finding[len(_EMPH) :])
                )
            else:
        ...
        if len(all_imgs) > _shown:
            parts.append(
                '<p class="note">+%d more chart%s for this dimension in the '
                '<a href="%s">full report</a>.</p>'
                % (len(all_imgs) - _shown, "" if len(all_imgs) - _shown == 1 else "s", _esc(report_basename))
            )
        ...
        if html_charts:
            ...
            links = " &nbsp;·&nbsp; ".join(
                '<a href="%s" style="color:var(--accent);font-weight:700">▶&nbsp;%s</a>'
        ...
        if srcs:
            links = " · ".join(
                '<a href="%s">%s</a>' % (_esc(_rel(s, report_dir)), _esc(os.path.basename(s))) for s in srcs[:12]
            )
        ...
        if gui_ptrs and not all_imgs and not srcs:
            parts.append(
                '<p class="note" style="color:#c1121f;font-weight:700">This dimension is explored '
                "interactively — open the <b>%s</b> tool from the NLP Suite menu.</p>" % _esc(", ".join(gui_ptrs))
            )

        # failures surfaced, never swallowed
        for lab, err in errs:
            parts.append('<p class="note err">%s could not be completed: %s</p>' % (_esc(lab), _esc(err)))
    ...
    if failed:
        parts.append(
            '<p class="note err">%d analys%s did not complete; see the notes above.</p>'
            % (len(failed), "is" if len(failed) == 1 else "es")
        )
    parts.append(
        '<footer class="paper">Generated by the NLP Suite Corpus Profiler with default '
