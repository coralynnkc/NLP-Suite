...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "sentence_analysis_main.py", ["tkinter", "subprocess", "ast"]
    )
    == False
):
    sys.exit(0)
...
def run():
    ...
    if (
        compute_sentence_length_var == False
        and visualize_bySentenceIndex_var == False
        and sentence_complexity_var == False
        and text_readability_var == False
        and visualize_sentence_structure_var == False
    ):
        mb.showwarning(
...
pydict["Abstract/Concrete vocabulary analysis"] = [
    "style_analysis_abstract_concreteness_analysis_util.main",
    0,
    3,
    "txt",
]
...
extract_sentences_button = tk.Button(
    window,
    width=GUI_IO_util.widget_width_short,
    text="Search/Extract sentences from corpus (Open GUI)",
    command=lambda: run_script_util.run_script("file_search_byWord_main.py"),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
compute_sentence_length_checkbox = tk.Checkbutton(
    window, text="Compute sentence length", variable=compute_sentence_length_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, compute_sentence_length_checkbox
)

sentence_complexity_var.set(0)
sentence_complexity_checkbox = tk.Checkbutton(
    window, text="Sentence complexity", variable=sentence_complexity_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, sentence_complexity_checkbox
)

text_readability_var.set(0)
text_readability_checkbox = tk.Checkbutton(
    window, text="Sentence/text readability (via textstat)", variable=text_readability_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, text_readability_checkbox
)
...
visualize_sentence_structure_var.set(0)
visualize_sentence_structure_checkbox = tk.Checkbutton(
    window,
    text="Visualize sentence structure (via dependency tree)",
    variable=visualize_sentence_structure_var,
    onvalue=1,
    offvalue=0,
)
# place widget with hover-over info
...
TIPS_options = (
    ...
    "Sentence length",
    "Sentence complexity measures (CLAS)",
)
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, click the button 'Style analysis' if you wish to open the style analysis GUi where a large variety of style tools are available, not necessarily at the sentence level, but at the document level."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, click the button 'Search/extract sentences from corpus' if you wish to open the search/extract GUI where a variety of search and extract options are available."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to compute the sentence lengths of your document(s).\n\nIn INPUT, the script expects a single txt file or a directory with a set of txt files."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to run the sentence complexity algorithm to provide different measures of sentence complexity: Yngve Depth, Frazer Depth, and Frazer Sum. These measures are closely associated to the sentence clause structure.\n\nThe Frazier and Yngve scores are very similar, with one key difference: while the Frazier score measures the depth of a syntactic tree, the Yngve score measures the breadth of the tree.\n\nIn INPUT, the script expects a single txt file or a directory with a set of txt files."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to run the Python 3 sentence_text_readability function to compute various measures of text readability, also closely associated to the sentence clause structure.\n\n  12 readability score requires HIGHSCHOOL education;\n  16 readability score requires COLLEGE education;\n  18 readability score requires MASTER education;\n  24 readability score requires DOCTORAL education;\n  >24 readability score requires POSTDOC education.\n\nIn INPUT, the script expects a single txt file or a directory with a set of txt files.\n\nIn OUTPUT, the script produces a txt file with readability scores for an entire text and a csv file with readability scores for each sentence in a text."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to visualize the sentence structure as a png image of the dependency tree."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
