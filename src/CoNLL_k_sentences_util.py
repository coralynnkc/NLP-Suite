...
def k_sent(inputFilename, outputDir, chartPackage, dataTransformation, Begin_K_sent_var, End_K_sent_var):
    ...
    def _content_lemmas(rows):
        ...
        for form, lemma, pos, sid in zip(
            rows["Form"].astype(str), rows["Lemma"].astype(str), rows["POS"].astype(str), rows["Sentence ID"]
        ):
            if (
                CoNLL_util.is_noun_POS(pos) or CoNLL_util.is_verb_POS(pos) or CoNLL_util.is_adjective_POS(pos)
            ) and form.isalpha():
                lem = lemma.lower()
                if lem:
    ...
    for doc_id in conll["Document ID"].unique():
        ...
        if repeated:
            sent_text = doc_rows.groupby("Sentence ID")["Form"].apply(lambda f: " ".join(f.astype(str))).to_dict()
            for section, K, content in (
                ("First", Begin_K_sent_var, first_content),
    ...
    if result_counts:
        ...
        ch = charts_util.plot(
            outFile,
            outputDir,
            columns=["Nouns Proportion", "Verbs Proportion", "Adjectives Proportion", "Proper-Nouns Proportion"],
            title="Word-class proportions in the first and last K ("
    ...
    if result_rep:
        ...
        outFile = IO_files_util.generate_output_file_name(
            inputFilename, "", outputDir, ".csv", label + "_rep_words", "", "", "", "", False, True
        )
        ...
        ch = charts_util.plot(
            ...
            outputDir,
            columns=["Lemma"],
            title="Words repeated in BOTH the first and last K ("
            + str(Begin_K_sent_var)
            + "-"
            + str(End_K_sent_var)
            + ") sentences (bookend repetition)",
            x_label="Words",
            file_label=str(Begin_K_sent_var) + "-" + str(End_K_sent_var) + "-sent_rep_words",
            group_by=None,
        )
        if ch is not None:
            filesToOpen.extend([ch] if isinstance(ch, str) else ch)
