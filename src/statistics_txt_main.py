...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "statistics_txt_main.py", ["os", "csv", "tkinter", "ntpath", "collections", "subprocess"]
    )
    == False
):
    sys.exit(0)
...
def run():
    ...
    if corpus_statistics_var:
        ...
        if "Compute line length" in corpus_statistics_options_menu_var or "*" in corpus_statistics_options_menu_var:
            ...
            outputFiles = statistics_txt_util.compute_line_length(
                window, config_filename, inputFilename, inputDir, outputDir, False, chartPackage, dataTransformation
            )
            if outputFiles != None:
                if isinstance(outputFiles, str):
                    filesToOpen.append(outputFiles)
    ...
    if corpus_statistics_var and (
        "TF-IDF" in corpus_statistics_options_menu_var or "*" in corpus_statistics_options_menu_var
    ):
    ...
    if corpus_statistics_var and (
        "Lexical diversity" in corpus_statistics_options_menu_var or "*" in corpus_statistics_options_menu_var
    ):
    ...
    if corpus_statistics_var and (
        "Readability" in corpus_statistics_options_menu_var or "*" in corpus_statistics_options_menu_var
    ):
    ...
    if corpus_statistics_var and (
        "Word frequency" in corpus_statistics_options_menu_var or "*" in corpus_statistics_options_menu_var
    ):
...
corpus_statistics_var.set(0)
corpus_statistics_checkbox = tk.Checkbutton(
    window,
    text="Compute corpus statistics",
    variable=corpus_statistics_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, corpus_statistics_checkbox, True
)

corpus_statistics_options_menu_var.set("*")

corpus_statistics_options_menu = tk.OptionMenu(
    window,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.style_corpus_text_options_menu_lb_pos, y_multiplier_integer, corpus_options_menu_lb, True
)
corpus_text_options_menu = tk.OptionMenu(
    window, corpus_text_options_menu_var, "*", "Lemmatize words", "Exclude stopwords & punctuation"
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.style_corpus_options_menu_men_pos, y_multiplier_integer, corpus_text_options_menu
)
...
corpus_statistics_byPOS_var.set(0)
corpus_statistics_byPOS_checkbox = tk.Checkbutton(
    window,
    text="Compute corpus statistics by POS (Part of Speech) tag value",
    variable=corpus_statistics_byPOS_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
...
TIPS_options = (
    ...
    "Text readability",
    "TF-IDF, Lexical diversity, Word frequency",
    "Stopwords",
)
