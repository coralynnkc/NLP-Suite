...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "statistics_corpus_tfidf_util", ["os", "tkinter", "pandas", "sklearn"]
    )
    == False
):
    sys.exit(0)
...
def compute_tfidf(
    ...
    outputDir = IO_files_util.make_output_subdirectory(inputFilename, inputDir, outputDir, label="TF-IDF", silent=True)
    if outputDir == "":
        return filesToOpen

    startTime = IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis start", "Started running TF-IDF analysis at", True
    )
    ...
    vectorizer = TfidfVectorizer(max_features=max_features, stop_words=stop_words, sublinear_tf=True)
    tfidf_matrix = vectorizer.fit_transform(documents)
    feature_names = vectorizer.get_feature_names_out()

    tfidf_df = pd.DataFrame(tfidf_matrix.toarray(), columns=feature_names, index=doc_names)
    tfidf_df.index.name = "Document"

    full_output = IO_files_util.generate_output_file_name(
        inputFilename, inputDir, outputDir, ".csv", "TF-IDF_matrix", "", "", "", "", False, True
    )
        ...
        for rank, idx in enumerate(top_indices, 1):
            if scores[idx] > 0:
                top_words_rows.append(
                    {"Document": doc, "Rank": rank, "Word": feature_names[idx], "TF-IDF Score": round(scores[idx], 4)}
                )

    top_df = pd.DataFrame(top_words_rows)
    top_output = IO_files_util.generate_output_file_name(
        inputFilename, inputDir, outputDir, ".csv", "TF-IDF_top_words", "", "", "", "", False, True
    )
    ...
    if len(documents) > 1:
        ...
        sim_output = IO_files_util.generate_output_file_name(
            inputFilename, inputDir, outputDir, ".csv", "TF-IDF_similarity", "", "", "", "", False, True
        )
        sim_df.to_csv(sim_output, encoding="utf-8")
        filesToOpen.append(sim_output)

        if len(documents) <= 50:
            fig, ax = plt.subplots(figsize=(max(8, len(documents) * 0.5), max(6, len(documents) * 0.4)))
    ...
    IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis end", "Finished running TF-IDF analysis at", True, "", True, startTime
    )
