...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "NER_entity_timeline_util", ["os", "tkinter", "pandas", "numpy", "matplotlib", "stanza"]
    )
    == False
):
    sys.exit(0)
...
def main(inputFilename, inputDir, outputDir, chartPackage="Excel", dataTransformation="No transformation", top_n=10):

    filesToOpen = []

    outputDir = IO_files_util.make_output_subdirectory(
        inputFilename, inputDir, outputDir, label="NER_timeline", silent=True
    )
    if outputDir == "":
        return filesToOpen

    startTime = IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis start", "Started running NER Entity Timeline at", True
    )
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        inputFilename, inputDir, outputDir, ".csv", "NER_entity_timeline", "", "", "", "", False, True
    )
    ...
    if len(df["Document ID"].unique()) > 1:
        ...
        doc_counts_file = IO_files_util.generate_output_file_name(
            inputFilename, inputDir, outputDir, ".csv", "NER_per_document_counts", "", "", "", "", False, True
        )
        doc_entity_counts.to_csv(doc_counts_file, encoding="utf-8")
        filesToOpen.append(doc_counts_file)

    IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis end", "Finished running NER Entity Timeline at", True, "", True, startTime
    )
