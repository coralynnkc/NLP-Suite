...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "GIS_distance_util", ["tkinter", "csv", "pandas", "geopy"]
    )
    == False
):
    sys.exit(0)
...
def create_distance_distribution_charts(distanceoutputFilename, outputDir, filesToOpen, encodingValue="utf-8"):
    ...
    for measure in ["Geodesic distance in miles", "Great circle distance in miles"]:
        ...
        try:
            ...
            fig.tight_layout()
            pngFilename = IO_files_util.generate_output_file_name(
                distanceoutputFilename,
                "",
                outputDir,
                ".png",
                "GIS",
...
def computePairwiseDistances(
    ...
    for one_scope in requested:
        ...
        if chartPackage != "No charts":
            try:
                filesToOpen = createCharts(
                    distanceoutputFilename, outputDir, filesToOpen, chartPackage, dataTransformation
                )
            except Exception as e:
    ...
    IO_user_interface_util.timed_alert(
        ...
        2000,
        "Analysis end",
        "Finished running GIS pairwise (all-pairs) distances at",
        True,
        "",
        True,
        startTime,
...
def computeDistancesFromSpecificLocation(
    ...
    if distinctValues == True:
        distanceoutputFilename = IO_files_util.generate_output_file_name(
            "", "", outputDir, ".csv", "GIS", "distance", baselineLocation, locationColumnName, "DISTINCT", False, True
        )
    else:
        distanceoutputFilename = IO_files_util.generate_output_file_name(
            "", "", outputDir, ".csv", "GIS", "distance", locationColumnName, baselineLocation, "ALL", False, True
        )
    filesToOpen.append(distanceoutputFilename)
    ...
    if InputIsGeocoded == False:
        ...
        if inputIsCoNLL == True:
            ...
            locations = GIS_location_util.extract_csvFile_locations(
                GUI_util.window, inputFilename, withHeader, locationColumnNumber, encodingValue, False, 0
            )
    ...
    # geocoded input
        ...
        try:
            ...
            mb.showerror(
                title="Input file error",
                message="There was an error in the function 'Compute GIS distance from specific location' reading the input file\n"
                + str(inputFilename)
    ...
    if chartPackage != "No charts":
        try:
            filesToOpen = createCharts(
                distanceoutputFilename, outputDir, filesToOpen, chartPackage, dataTransformation, baselineLocation
            )
        except Exception as e:
...
def computeConsecutiveDistances(
    window, inputFilename, outputDir, distinctValues, encodingValue, groupColumn="Document", orderColumn="Sentence ID"
):
    filesToOpen = []
    startTime = IO_user_interface_util.timed_alert(
        window,
    ...
    distanceoutputFilename = IO_files_util.generate_output_file_name(
        "", "", outputDir, ".csv", "GIS", "distance", "consecutive", "movement", "", False, True
    )
