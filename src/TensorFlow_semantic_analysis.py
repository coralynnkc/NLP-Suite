...
aggregate_correlations = alpha ** (
    np.array(correlation_matrices).sum(axis=0) - len(correlation_matrices) * np.eye(corr_size)
)  # Subtract out diagonals - we already know they are 100% similar
sns.heatmap(aggregate_correlations / len(NYTarticles), xticklabels=tick_labels, yticklabels=tick_labels)
