...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "DB_SQL_main.py", ["os", "tkinter", "pandas", "sqlite3"]
    )
    == False
):
    sys.exit(0)
...
def run():
    ...
    if select_SQLite_DB_var.get() != "":
        if not SQL_query_var or SQL_query_var.strip() == "":
            mb.showwarning(
                title="Warning", message="The SQL query area is empty. Please, create or import a query and try again."
            )
            return
        ...
        if qname.startswith("Cross-complex:") and inputDir:
            ...
            if not os.path.exists(pcace_subdir):
                pcace_subdir = IO_files_util.make_output_subdirectory(
                    "", "", outputDir, label=os.path.basename(pcace_subdir), silent=False
                )
            if pcace_subdir:
        ...
        if qname.startswith("Cross-complex:"):
            ...
            if len(parts) >= 2:
                ...
                csv_name = f"SQL_{src_part}_{tgt_part}.csv"
            else:
...
def _auto_build_sqlite(*args):
    ...
    data_files = [f for f in os.listdir(in_dir) if f.lower().endswith((".xlsx", ".csv")) and not f.startswith("~$")]
    if not data_files:
        _disable_all_widgets()
    ...
    if os.path.exists(dbOutput) and _check_sqlite_version(in_dir):
        ...
        if os.path.exists(dbOutput):
            IO_user_interface_util.timed_alert(
                ...
                "Rebuilding SQLite database for "
                + db_folder_name
                + " ("
                + str(len(data_files))
...
def view_grammar():
    ...
    DB_PCACE_data_analyzer_util.view_grammar(
...
def open_pcace_analyzer():
    ...
    if not in_dir or not os.path.isdir(in_dir):
        mb.showwarning(
            title="Warning", message="No input directory selected.\n\nPlease, select a PC-ACE input directory first."
        )
        return
...
view_relations_button = tk.Button(
    window, text="View table relations", width=17, height=1, state="disabled", command=lambda: view_relations()
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.labels_x_coordinate,
...
view_grammar_button = tk.Button(
    window, text="View grammar", width=17, height=1, state="disabled", command=lambda: view_grammar()
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
update_grammar_button = tk.Button(
    window, text="Update grammar", width=17, height=1, state="disabled", command=lambda: update_grammar()
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.labels_x_coordinate + 300,
    y_multiplier_integer,
    update_grammar_button,
    True,
    False,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 450, y_multiplier_integer, object_type_lb, True
)
...
def _update_combo_hover(combo, y_row, x_coord, x_hover, count, base_text):
    ...
    combo.bind(
        ...
        lambda e, t=tip: GUI_IO_util.display_widget_info(
            window, e, x_coord, GUI_IO_util.basic_y_coordinate + GUI_IO_util.y_step * y_row, x_hover, t
        ),
    )
    combo.bind("<Leave>", lambda e: GUI_IO_util.delete_display_widget_lb(window, e, ""))
...
def _toggle_required_sql():
    ...
    if proceed:
        success = DB_PCACE_data_analyzer_util.toggle_required_value(
            obj_type, obj_name, new_val, GUI_util.input_main_dir_path.get()
        )
        if success:
            mb.showwarning(
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, cross_complex_lb, True
)
...
expand_complex_cb = tk.Checkbutton(
    window, text="", variable=expand_complex_var, onvalue=1, offvalue=0, state="disabled"
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.labels_x_coordinate + 880,
    y_multiplier_integer,
    expand_complex_cb,
    True,
    False,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, where_filter_lb, True
)
...
where_operator_menu = ttk.Combobox(
    window, textvariable=where_operator_var, state="disabled", width=5, values=["LIKE", "=", "!=", "NOT LIKE"]
)
...
def _update_extra_targets_label(*args):
    ...
    for s, ssx, t, tsx in _saved_pairs:
        parts.append(f"{obj_num}:{s}")
        obj_num += 1
        parts.append("{}:{}".format(obj_num, ssx or "*"))
        obj_num += 1
        parts.append(f"{obj_num}:{t}")
        obj_num += 1
    ...
    if src:
        parts.append(f"{obj_num}:{src}")
        obj_num += 1
        src_sx = source_simplex_var.get() or "*"
        parts.append(f"{obj_num}:{src_sx}")
        obj_num += 1
    tgt = target_complex_var.get()
    if tgt:
        parts.append(f"{obj_num}:{tgt}")
        obj_num += 1
        tgt_sx = target_simplex_var.get() or "*"
        parts.append(f"{obj_num}:{tgt_sx}")
...
def _generate_cross_complex_query():
    ...
    if len(all_pairs) == 1:
        ...
        query_name_var.set(f"Cross-complex: {src} → {tgt_name}")
        ...
        if not DB_PCACE_data_analyzer_util.get_cross_complex_simplex_names(src):
            ...
            msg = f"'{src}' has no simplex attributes."
            if children:
        ...
        if len(sources) > 1:
            ...
            pair_labels = [f"{s} → {t}" for s, _, t, _ in all_pairs]
...
def _populate_cross_complex_menus(*args):
    ...
    # Reset all
    for combo, var in [
        (source_complex_menu, source_complex_var),
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, select_DB_tables_lb, True
)
...
select_DB_tables_menu.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 70, y_multiplier_integer, select_DB_tables_menu, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.labels_x_coordinate + 280,
    y_multiplier_integer,
    select_DB_table_fields_lb,
    True,
    False,
    True,
    False,
...
select_DB_table_fields_menu.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 380, y_multiplier_integer, select_DB_table_fields_menu, True
)

auto_SQL_lb = tk.Label(window, text="Templates")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 510, y_multiplier_integer, auto_SQL_lb, True
)

auto_SQL_var = tk.StringVar()
auto_SQL_value = tk.OptionMenu(
    window,
    auto_SQL_var,
    "SQL standard",
    "SQL count",
...
distinct_checkbox = tk.Checkbutton(
    window, text="Distinct", variable=distinct_var, onvalue=1, offvalue=0, state="disabled"
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.labels_x_coordinate + 680,
...
def _get_query_dir():
    ...
    input_dir = (
        ...
        if hasattr(GUI_util.input_main_dir_path, "get")
        else GUI_util.input_main_dir_path
    )
    if input_dir:
...
def import_query(window, title, fileType):
    ...
    if init_dir and os.path.isdir(init_dir):
        ...
        if len(txt_files) == 0:
            ...
            return
    filePath = tk.filedialog.askopenfilename(title=title, initialdir=init_dir, filetypes=fileType)
    if len(filePath) > 0:
        SQL_query_entry.delete(0.1, tk.END)
        with open(filePath, encoding="utf_8", errors="ignore") as file:
...
def save_query():
    ...
    if len(boxContent) > 0:
        ...
        filePath = tk.filedialog.asksaveasfile(
            initialdir=save_dir,
            initialfile="saveQry.txt",
            title="Save SQL query file",
        ...
        if len(filePath) > 0:
            with open(filePath, "w+", encoding="utf_8", errors="ignore") as file:
                ...
                file.close()
            mb.showwarning(title="Warning", message="The SQL query has been saved to\n\n" + filePath)


import_query_button = tk.Button(
    window,
    width=15,
    text="Import SQL query",
    state="disabled",
    command=lambda: import_query(window, "Select INPUT SQL query file", [("SQL files", "*.txt")]),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 770, y_multiplier_integer, import_query_button, True
)

save_query_button = tk.Button(window, width=15, text="Save SQL query", state="disabled", command=lambda: save_query())
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 850, y_multiplier_integer, save_query_button
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate, y_multiplier_integer, csv_file, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, SQL_query_entry
)
...
def _update_sql_query_hover(*args):
    ...
    SQL_query_entry.bind("<Leave>", lambda e: GUI_IO_util.delete_display_widget_lb(window, e, ""))
...
def display_SQL(*args):
    ...
    if auto_SQL_var.get() == "SQL standard":
        ...
        SQL_text = (
            SQL_text
            + "[field1], COUNT([Field1]) AS FREQUENCY\n  FROM [table]\n  GROUP BY [field1]\n  ORDER BY COUNT([field1]) DESC;"
        )
        ...
        SQL_text = (
            SQL_text
            + "[field1], GROUP_CONCAT([field2], '; ') AS Combined\n  FROM [table]\n  GROUP BY [field1]\n  ORDER BY [field1];"
        )
...
TIPS_options = (
    "DB SQL GUI (what input it needs; where the database is saved)",
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Click View table relations to open the PC-ACE table relations diagram.\nClick View grammar to export the grammar.\nClick Update grammar to refresh the grammar in setup_complex.\nClick Open PC-ACE analyzer to open the PC-ACE data analyzer GUI with the current input directory."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Enter an SQL query in the form SELECT ...\n\nYou can also generate a new SQL query, import a saved query or use a template from the dropdown menu.\n\nHover over the query area to see the name of the currently loaded query."
        + GUI_IO_util.msg_Esc,
    )

    y_multiplier_integer = GUI_IO_util.place_help_button(
        window, help_button_x_coordinate, y_multiplier_integer + 4.5, "NLP Suite Help", GUI_IO_util.msg_openOutputFiles
    )
