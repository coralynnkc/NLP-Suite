...
def run():
    ...
    if utf8_var == 1 or ASCII_var == 1:
        mb.showwarning(
            title="Option not available", message="The utf-8 and ASCII options are not available yet.\n\nSorry!"
        )
        list_var = 1
        return
    ...
    if delete_var == 1:
        ...
        command = tk.messagebox.askyesno(
            "Deleting files",
            "You are about to delete files. Make sure you have a backup! Files deleted via a Python command will not be recoverable from the Recycle Bin\n\nAre you sure you want to do continue?",
        )
        if command == False:
            return
    ...
    if options == 1:
        if count_file_manager_var == 0:
            if list_var == 0 and by_file_type_var == "" and by_prefix_var == 0 and by_substring_var == 0:
                mb.showwarning(
                    title="File manager",
                    message="You have selected a file manager option, but no specific criteria for managing the files: By file type, By prefix value, or By substring value.\n\nPlease, select the file criteria to use and try again.",
                )
                return
    ...
    if options > 1:
        mb.showwarning(
            title="File manager",
            message="Only one option at a time can be selected. You have selected "
            + str(options)
    ...
    if by_creation_date_var == 1:
        if file_type_menu_var != "" and file_type_menu_var != "doc" and by_file_type_var != "docx":
            mb.showwarning(
                title="File manager",
                message='You have selected the options "By file type" as '
                + file_type_menu_var
    ...
    startTime = IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis start", "Started running File Manager at", True, "", True, "", True
    )
    ...
    if rename_var == 1:
        ...
        if (
            by_prefix_var == False
            and by_substring_var == False
            and by_foldername_var == False
            and by_embedded_items_var == False
        ):
            mb.showwarning(
                title="File manager",
                message="You have selected the option to Rename files but you have not selected any of the available options for renaming the files.\n\nPlease, make a selection and enter the appropriate values and try again.",
            )
            return
        if (by_prefix_var or by_substring_var) and string_entry_var == "":
            mb.showwarning(
                title="File manager",
                message="You have selected the option to Rename files by prefix/sub-string value but you have not entered the values necessary for renaming the filename.\n\nPlease, enter the missing values in the fields 'Enter value' and/or 'New renaming value' and try again.",
            )
            return
        if by_foldername_var and folder_character_separator_var == "":
            mb.showwarning(
                title="File manager",
                message="You have selected the option to Rename files by Folder name but you have not entered the Separator character(s).\n\nPlease, enter appropriate values in the 'Separator character(s)' field and try again.",
            )
            return
    ...
    if by_prefix_var == 1 and by_substring_var == 1:
        mb.showwarning(
            title="File manager",
            message='Only one option at a time, "By prefix value" or "By sub-string value," can be selected.\n\nPlease, deselect one option and try again.',
        )
        return
    ...
    if by_prefix_var == 1:
        if len(string_entry_var) == 0:
            mb.showwarning(
                title="File manager",
                message='You have selected the option "By prefix value" but no string value has been entered.\n\nPlease, enter the prefix value in the "Enter value" field and try again.',
            )
            return
        if len(msg) > 0:
    ...
    if by_substring_var == 1:
        if len(string_entry_var) == 0:
            mb.showwarning(
                title="File manager",
                message='You have selected the option "By sub-string value" but no string value has been entered.\n\nPlease, enter the sub-string value in the "Enter value" field and try again.',
            )
            return
        if len(msg) > 0:
    ...
    if selectedCsvFile_var != "":
        if noHeaders == False:
            selectedCsvFile_colNum = IO_csv_util.get_columnNumber_from_headerValue(
                headers, selectedCsvFile_colName, selectedCsvFile_var
            )
        else:
        ...
        fileList = []
        with open(selectedCsvFile_var, encoding="utf-8", errors="ignore") as read_obj:
    ...
    outputFiles = charts_util.plot(
        ...
        title="Frequency Distribution of File Types",
        x_label="File type",
        file_label="File_Types",
        group_by=None,
        plot_list=["Frequency"],
    )
    if outputFiles:
    ...
    IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis end", "Finished running File manager at", True, "", True, startTime
    )
...
add_file_button = tk.Button(
    window, text="csv file", command=lambda: add_csvFile(window, "Select INPUT csv file", [("csv files", "*.csv")])
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, add_file_button, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 70, y_multiplier_integer, openInputFile_button, True
)

selectedCsvFile = tk.Entry(window, width=110, state="disabled", textvariable=selectedCsvFile_var)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 120, y_multiplier_integer, selectedCsvFile, True
)

select_csv_field_lb = tk.Label(window, text="Select csv field")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 800, y_multiplier_integer, select_csv_field_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 900, y_multiplier_integer, select_csv_field_menu
)

utf8_var.set(0)
utf8_checkbox = tk.Checkbutton(
    window, text="Check input filename(s) for utf-8 encoding ", variable=utf8_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, utf8_checkbox, True
)

ASCII_var.set(0)
ASCII_checkbox = tk.Checkbutton(
    window, text="Convert non-ASCII apostrophes & quotes in filename(s)", variable=ASCII_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 400, y_multiplier_integer, ASCII_checkbox
)

list_var.set(0)
list_checkbox = tk.Checkbutton(window, text="List", variable=list_var, onvalue=1, offvalue=0)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, list_checkbox, True
)

rename_var.set(0)
rename_checkbox = tk.Checkbutton(window, text="Rename", variable=rename_var, onvalue=1, offvalue=0)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 60, y_multiplier_integer, rename_checkbox, True
)

copy_var.set(0)
copy_checkbox = tk.Checkbutton(window, text="Copy", variable=copy_var, onvalue=1, offvalue=0)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 160, y_multiplier_integer, copy_checkbox, True
)

move_var.set(0)
move_checkbox = tk.Checkbutton(window, text="Move", variable=move_var, onvalue=1, offvalue=0)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 260, y_multiplier_integer, move_checkbox, True
)

delete_var.set(0)
delete_checkbox = tk.Checkbutton(window, text="Delete", variable=delete_var, onvalue=1, offvalue=0)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 360, y_multiplier_integer, delete_checkbox, True
)

count_file_manager_var.set(0)
count_checkbox = tk.Checkbutton(
    window, text="Count", state="normal", variable=count_file_manager_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 460, y_multiplier_integer, count_checkbox, True
)

split_file_manager_var.set(0)
split_checkbox = tk.Checkbutton(
    window, text="Split", state="normal", variable=split_file_manager_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 560, y_multiplier_integer, split_checkbox
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, by_file_type_checkbox, True
)

file_type_menu_lb = tk.Label(window, text="Select file type ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 280, y_multiplier_integer, file_type_menu_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 370, y_multiplier_integer, file_type_menu
)
...
by_creation_date_checkbox = tk.Checkbutton(
    window, text="By creation & modification date", variable=by_creation_date_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, by_creation_date_checkbox, True
)
# before_date,
# after_date,
# on_date,

by_author_checkbox = tk.Checkbutton(
    window, text="By author (Windows Office files)", variable=by_author_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 370, y_multiplier_integer, by_author_checkbox
)

by_prefix_var.set(0)
by_prefix_checkbox = tk.Checkbutton(window, text="By prefix value", variable=by_prefix_var, onvalue=1, offvalue=0)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, by_prefix_checkbox, True
)

by_substring_var.set(0)
by_substring_checkbox = tk.Checkbutton(
    window, text="By sub-string value", variable=by_substring_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 140, y_multiplier_integer, by_substring_checkbox, True
)

string_entry_lb = tk.Label(window, text="Old value")
# (case sensitive)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 280, y_multiplier_integer, string_entry_lb, True
)

string_entry = tk.Entry(window, width=20, textvariable=string_entry_var)
string_entry.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 370, y_multiplier_integer, string_entry, True
)

rename_new_entry_lb = tk.Label(window, text="New value")
# (case sensitive)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 580, y_multiplier_integer, rename_new_entry_lb, True
)

rename_new_entry = tk.Entry(window, width=20, textvariable=rename_new_entry_var)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 680, y_multiplier_integer, rename_new_entry, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 850, y_multiplier_integer, position_in_string_lb, True
)
position_in_string_value = tk.Entry(window, width=2, textvariable=position_in_string_var)
position_in_string_value.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 970, y_multiplier_integer, position_in_string_value
)
...
by_foldername_var.set(0)
by_foldername_checkbox = tk.Checkbutton(
    window, text="By subfolder name (renaming only)", variable=by_foldername_var, onvalue=1, offvalue=0
)
by_foldername_checkbox.config(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, by_foldername_checkbox, True
)

folder_character_separator_lb = tk.Label(window, text="Separator character(s)")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 280, y_multiplier_integer, folder_character_separator_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 420, y_multiplier_integer, folder_character_separator
)
...
by_embedded_items_var.set(0)
by_embedded_items_checkbox = tk.Checkbutton(
    window, text="By number of embedded items", variable=by_embedded_items_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, by_embedded_items_checkbox, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 280, y_multiplier_integer, comparison_menu, True
)

number_of_items_lb = tk.Label(window, text="Number of items")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 400, y_multiplier_integer, number_of_items_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 530, y_multiplier_integer, number_of_items_value, True
)

embedded_item_character_value_var.set("_")
embedded_item_character_value_lb = tk.Label(window, text="Separator character(s)")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 580, y_multiplier_integer, embedded_item_character_value_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 750, y_multiplier_integer, embedded_item_character_value, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 850, y_multiplier_integer, include_exclude_checkbox
)
...
character_count_checkbox = tk.Checkbutton(
    window, text="By number of embedded character(s)", variable=character_count_file_manager_var, onvalue=1, offvalue=0
)
character_count_checkbox.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, character_count_checkbox, True
)

characters_entry_lb = tk.Label(window, text="Enter character(s) ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 280, y_multiplier_integer, characters_entry_lb, True
)

characters_entry = tk.Entry(window, width=2, textvariable=character_entry_var)
characters_entry.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 420, y_multiplier_integer, characters_entry
)
...
fileName_embeds_date_checkbox = tk.Checkbutton(
    window, text="Filename embeds date", variable=fileName_embeds_date, onvalue=1, offvalue=0
)
fileName_embeds_date_checkbox.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, fileName_embeds_date_checkbox, True
)

date_format_lb = tk.Label(window, text="Date format ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 280, y_multiplier_integer, date_format_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate + 80, y_multiplier_integer, date_format_menu, True
)
...
include_subdir_checkbox = tk.Checkbutton(
    window, text="Include subdirectories", variable=include_subdir_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, include_subdir_checkbox
)
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox to restrict by file creation/modification date and by author the selected file handling option.\n\nThe 'By author' option is available for Windows Office files only (doc, docx, xls, xlsx, xlsm).",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox to rename files in a folder by embedding the LAST PART of folder path in the renamed filename (e.g., The Boston Globe_19-19-1919 found in the subfolder 'John Willis' of a folder path 'c:\\mydata\\newspapers\\lynching\\John Willis' will be remamed as The Boston Globe_19-19-1919__John Willis if __ is selected as the separator character(s).\n\nThe option is available only when renaming files.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox to process all files found in the input directory and all its subdirectories.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
