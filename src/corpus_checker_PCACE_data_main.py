...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "Corpus Checker (PC-ACE data)", ["os", "tkinter", "subprocess", "csv"]
    )
    == False
):
    sys.exit(0)
...
def missing_character(
    ...
    if IO_libraries_util.check_inputPythonJavaProgramFile("file_summary_checker_util.py") == False:
        return
    Excel_outputFile = file_summary_checker_util.main(
        CoreNLPdir,
        inputDir,
        input_secondary_dir_path,
        outputDir,
...
def intruder(
    CoreNLPdir, inputDir, outputDir, openOutputFiles, chartPackage, dataTransformation, similarityIndex_Intruder_var
):
    ...
    startTime = IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis start", "Started running INTRUDER at", True, "", True, "", True
    )
    # Windows...
    outputFiles = file_find_non_related_documents_util.main(
        CoreNLPdir, inputDir, outputDir, openOutputFiles, chartPackage, dataTransformation, similarityIndex_Intruder_var
    )
    ...
    IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis end", "Finished running INTRUDER at", True, "", True, startTime, True
    )
...
def plagiarist(
    ...
    if similarityIndex_Plagiarist_var < 0.8:
        mb.showwarning(
            title="Similarity Index warning",
            message="The level of similarity was set at "
...
def run():
    ...
    if config_aware_parser_util.requires_CoreNLP():
        ...
        CoreNLPdir, existing_software_config, errorFound = IO_libraries_util.external_software_install(
            "corpus_checker_PCACE_data_main", "Stanford CoreNLP", "", silent=False, errorFound=False
        )
        if CoreNLPdir == None or CoreNLPdir == "":
            return filesToOpen

    if (
        check_filename_var == False
        and character_var == False
        and character_home_var == False
        and missing_character_var == False
        and intruder_var == False
        and Levenshtein_var == False
        and ancestor_var == False
        and plagiarist_var == False
    ):
        mb.showwarning(
...
extra_GUIs_menu = tk.OptionMenu(
    window, extra_GUIs_menu_var, "PC-ACE data analysis (Open GUI)", "PC-ACE data validation (Open GUI)"
)
...
fileName_embeds_date_checkbox = tk.Checkbutton(
    window, text="Filename embeds date", state="disabled", variable=fileName_embeds_date, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate, y_multiplier_integer, date_format_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate + 90, y_multiplier_integer, date_format_menu, True
)

items_separator_var_lb = tk.Label(window, text="Date character separator ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate + 210, y_multiplier_integer, items_separator_var_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate + 360, y_multiplier_integer, items_separator_var_menu, True
)

date_position_var_menu_lb = tk.Label(window, text="Date position ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate + 390, y_multiplier_integer, date_position_var_menu_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate + 490, y_multiplier_integer, date_position_var_menu
)

check_filename_var.set(0)
check_filename_checkbox = tk.Checkbutton(
    window, text="Check the filenames well-formedness", variable=check_filename_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, check_filename_checkbox
)

character_var.set(0)
character_checkbox = tk.Checkbutton(
    window,
    text="Find the character & the ancestor (via WordNet, VerbNet, FrameNet)",
    variable=character_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, character_checkbox
)

missing_character_var.set(0)
missing_character_checkbox = tk.Checkbutton(
    window, text="Find the missing character", variable=missing_character_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, missing_character_checkbox, True
)

NER_var.set(0)
NER_checkbox = tk.Checkbutton(
    window, text="NER (Named Entity Recognition) ", state="disabled", variable=NER_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate, y_multiplier_integer, NER_checkbox
)

Levenshtein_var.set(0)
Levenshtein_checkbox = tk.Checkbutton(
    window, text="Check the character's name tag", variable=Levenshtein_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, Levenshtein_checkbox
)

character_home_var.set(0)
character_home_checkbox = tk.Checkbutton(
    window, text="Find the character's home", variable=character_home_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, character_home_checkbox
)

intruder_var.set(0)
intruder_checkbox = tk.Checkbutton(window, text="Find the intruder", variable=intruder_var, onvalue=1, offvalue=0)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, intruder_checkbox, True
)

similarityIndex_Intruder_var.set(0.2)
similarityIndex_Intruder_menu_lb = tk.Label(window, text="Relativity index threshold")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate, y_multiplier_integer, similarityIndex_Intruder_menu_lb, True
)
similarityIndex_Intruder_menu = tk.OptionMenu(
    window,
    similarityIndex_Intruder_var,
    0.1,
    0.15,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, plagiarist_checkbox, True
)

similarityIndex_Plagiarist_var.set(0.8)
similarityIndex_Plagiarist_menu_lb = tk.Label(window, text="Similarity index ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate, y_multiplier_integer, similarityIndex_Plagiarist_menu_lb, True
)
similarityIndex_Plagiarist_menu = tk.OptionMenu(
    window, similarityIndex_Plagiarist_var, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9
)
...
TIPS_lookup = {
    "Corpus checker (PC-ACE data) - the whole pipeline": "TIPS_NLP_Corpus checker (PC-ACE data).pdf",
...
TIPS_options = (
    "Corpus checker (PC-ACE data) - the whole pipeline",
    "Filename well-formedness",
    "WordNet",
    "Lexical databases (WordNet, VerbNet, FrameNet)",
    "Find the character's home (By date)",
    "Find the character's home (By NER)",
    "NER (Named Entity Recognition)",
    "Find the missing character",
    "Check the character's name tag",
    "Find the intruder",
    "Find the plagiarist",
    "CoNLL Table",
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    if not IO_setup_display_brief:
        ...
        y_multiplier_integer = GUI_IO_util.place_help_button(
            ...
            y_multiplier_integer,
            "NLP Suite Help",
            "Please, select the secondary INPUT directory of the TXT files to be analyzed."
            + GUI_IO_util.msg_openExplorer,
        )
        y_multiplier_integer = GUI_IO_util.place_help_button(
