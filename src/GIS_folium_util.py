...
if (
    IO_libraries_util.install_all_Python_packages(GUI_util.window, "GIS_folium_util", ["os", "pandas", "folium"])
    == False
):
    sys.exit(0)
...
def _load_geocoded_data(inputFilename):
    try:
        ...
        mb.showwarning("Warning", "Could not read the input csv file\n\n" + inputFilename + "\n\n" + str(e))
        return None
...
def create_folium_pin_map(window, inputFilename, outputDir, locationColumnName="Location"):
    startTime = IO_user_interface_util.timed_alert(
        window, 2000, "Folium pin map", "Started running Folium pin map at", True, "", True, "", False
    )
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        inputFilename, "", outputDir, ".html", "GIS", "Folium-pin", locationColumnName, "", "", False, True
    )
    m.save(outputFilename)

    IO_user_interface_util.timed_alert(
        window, 2000, "Folium pin map", "Finished running Folium pin map at", True, "", True, startTime
    )
...
def create_folium_heatmap(window, inputFilename, outputDir, locationColumnName="Location"):
    startTime = IO_user_interface_util.timed_alert(
        window, 2000, "Folium heatmap", "Started running Folium heatmap at", True, "", True, "", False
    )
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        inputFilename, "", outputDir, ".html", "GIS", "Folium-heat", locationColumnName, "", "", False, True
    )
    m.save(outputFilename)

    IO_user_interface_util.timed_alert(
        window, 2000, "Folium heatmap", "Finished running Folium heatmap at", True, "", True, startTime
    )

    return outputFilename


def create_folium_timestamped_map(
    window, inputFilename, outputDir, locationColumnName="Location", dateColumnName="Date"
):
    startTime = IO_user_interface_util.timed_alert(
        window, 2000, "Folium timestamped map", "Started running Folium timestamped map at", True, "", True, "", False
    )
    ...
    if len(df) == 0:
        mb.showwarning(
            "Warning", 'No valid dates could be parsed from the "' + dateColumnName + '" column in\n\n' + inputFilename
        )
        return ""
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        inputFilename, "", outputDir, ".html", "GIS", "Folium-time", locationColumnName, "", "", False, True
    )
    m.save(outputFilename)

    IO_user_interface_util.timed_alert(
        window, 2000, "Folium timestamped map", "Finished running Folium timestamped map at", True, "", True, startTime
    )
