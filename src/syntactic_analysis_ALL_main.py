...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window,
        "syntactic_analysis_ALL_main.py",
        ["os", "csv", "tkinter", "ntpath", "collections", "subprocess"],
    )
    == False
):
    sys.exit(0)
...
def run():
    ...
    if WSD_var:
        ...
        for noun_verb in noun_verb_list:
            outFiles = SVO_util.wsd_aggregate_WordNet(
...
parsers_annotators_button = tk.Button(
    window,
    width=42,
    text="Parsers & annotators (Open GUI)",
    command=lambda: run_script_util.run_script("parsers_annotators_main.py"),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, parsers_annotators_button, False
)

CoNLL_table_button = tk.Button(
    window,
    width=42,
    text="CoNLL table analyzer (Open GUI)",
    command=lambda: run_script_util.run_script("CoNLL_table_analyzer_main.py"),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, CoNLL_table_button, False
)

SVO_button = tk.Button(
    window,
    width=42,
    text="Subject-Verb-Object (SVO) (Open GUI)",
    command=lambda: run_script_util.run_script("SVO_main.py"),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, SVO_button, False
)


sentence_structure_button = tk.Button(
    window,
    width=42,
    text="Sentence structure (Open GUI)",
    command=lambda: run_script_util.run_script("sentence_analysis_main.py"),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, sentence_structure_button, False
)

style_analysis_button = tk.Button(
    window,
    width=42,
    text="Style analysis (Open GUI)",
    command=lambda: run_script_util.run_script("style_analysis_main.py"),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, style_analysis_button, False
)
