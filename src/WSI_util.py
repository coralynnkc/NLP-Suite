...
def _bert_first_download_alert_wsi():
    cache_dir = os.path.join(os.path.expanduser("~"), ".cache", "huggingface", "hub", "models--bert-base-uncased")
...
def get_centroids(all_sent, all_vocab, Word2Vec_Dir, k_range, sample=None):
    ...
    for w in tqdm(all_vocab, total=len(all_vocab), desc="Generating sense centroids..."):
        ...
        if len(seq) == 0:
            mb.showerror(
                title=":-(",
                message=f'There are no occurrences of "{w}" in the dataset. Check for misspellings and/or input other forms of the word, e.g. plural/singular forms, different tenses etc.',
            )
            raise
