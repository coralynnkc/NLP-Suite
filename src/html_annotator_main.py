...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "html_annotator_main.py", ["os", "tkinter", "subprocess"]
    )
    == False
):
    sys.exit(0)
...
def run():
    ...
    if html_annotator_add_dictionary_var == True or html_annotator_extractor == True:
        if inputFilename != "" and inputFilename[-5:] != ".html":
            mb.showwarning(
                title="Warning",
                message="You have selected to run an option that requires an input file of type .html.\n\nPlease, select an .html file (or a directory) and try again.",
            )
            return

    if (
        html_annotator_dictionary_var == True
        or html_annotator_add_dictionary_var == True
        and html_dictionary_file == ""
    ):
    ...
    if html_annotator_dictionary_var == True:
        ...
        html_annotator_extractor_util.buildcsv(
            inputFilename,
            input_main_dir_path,
            outputDir,
            openOutputFiles,
...
html_annotator_add_dictionary_var = (
    tk.IntVar()
)  # to add new annotations via dictionary to an already annotated document
...
knowledge_graphs_DBpedia_YAGO_button = tk.Button(
    window,
    width=GUI_IO_util.widget_width_long,
    text="HTML annotate corpus using the DBpedia, YAGO & Wikipedia knowledge graphs (Open GUI)",
    command=lambda: run_script_util.run_script("knowledge_graphs_main.py"),
)
...
semantic_aggregation_WordNet_button = tk.Button(
    window,
    width=GUI_IO_util.widget_width_long,
    text="HTML annotate corpus using the WordNet knowledge graphs (Open GUI)",
    command=lambda: run_script_util.run_script("semantic_aggregation_main.py"),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
html_gender_annotator_button = tk.Button(
    window,
    width=GUI_IO_util.widget_width_long,
    text="HTML gender annotator (Open GUI)",
    command=lambda: run_script_util.run_script("html_annotator_gender_main.py"),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
html_dictionary_annotator_checkbox = tk.Checkbutton(
    window,
    text="HTML annotate corpus using csv dictionary",
    variable=html_annotator_dictionary_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, html_dictionary_annotator_checkbox, True
)

html_annotator_add_dictionary_var.set(0)
html_annotator_add_dictionary_checkbox = tk.Checkbutton(
    window,
    text="Add annotations to a previously annotated HTML file using csv dictionary",
    variable=html_annotator_add_dictionary_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.html_annotator_add_dictionary_description,
    y_multiplier_integer,
    html_annotator_add_dictionary_checkbox,
)

annotator_dictionary_button = tk.Button(
    window,
    width=20,
    text="Select csv dictionary file",
    command=lambda: get_dictionary_file(window, "Select INPUT csv dictionary file", [("dictionary files", "*.csv")]),
)
...
openInputFile_button = tk.Button(
    ...
    state="disabled",
    text="",
    command=lambda: IO_files_util.openFile(window, html_annotator_dictionary_file.get()),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.html_annotator_openInputFile_button,
    y_multiplier_integer,
    ...
    90,
    GUI_IO_util.html_annotator_openInputFile_button,
    "Open displayed csv dictionary file",
)

html_annotator_dictionary_file = tk.Entry(
    window, width=GUI_IO_util.html_annotator_dictionary_width, textvariable=html_annotator_dictionary_file_var
)
html_annotator_dictionary_file.config(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.html_annotator_dictionary_file_button, y_multiplier_integer, html_annotator_dictionary_file
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.html_annotator_csv_field1_menu, y_multiplier_integer, csv_field1_menu, True
)

add_dictValue_button = tk.Button(
    window,
    text="+",
    width=GUI_IO_util.add_button_width,
    height=1,
    state="disabled",
    command=lambda: csv_field_value_menu.configure(state="normal"),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.html_annotator_add_dictValue_button, y_multiplier_integer, add_dictValue_button, True
)

reset_dictValue_button = tk.Button(
    window,
    text="Reset ",
    width=GUI_IO_util.reset_button_width,
    height=1,
    state="disabled",
    command=lambda: clear_dictionary_list(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.html_annotator_reset_dictValue_button, y_multiplier_integer, reset_dictValue_button, True
)
...
def showKeywordList():
    if len(csvValue_color_list) == 0:
        mb.showwarning(
            title="Warning", message="There are no currently selected combinations of csv field values and colors."
        )
    else:
        mb.showwarning(
            title="Warning",
            message="The currently selected combination of csv field values and colors are:\n\n"
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.html_annotator_show_keywords_button, y_multiplier_integer, show_keywords_button, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.html_annotator_csv_field2_lb, y_multiplier_integer, field2_lb, True
)
if menu_values != "":
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.html_annotator_csv_field2_menu, y_multiplier_integer, csv_field2_menu, True
)

value_lb = tk.Label(window, text="Select csv field value ")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.html_annotator_value_lb, y_multiplier_integer, value_lb, True
)
if menu_values != "":
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.html_annotator_csv_field_value_menu, y_multiplier_integer, csv_field_value_menu, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.html_annotator_color_palette_dict_lb, y_multiplier_integer, color_palette_dict_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.html_annotator_color_palette_dict_menu, y_multiplier_integer, color_palette_dict_menu, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.html_annotator_bold_checkbox, y_multiplier_integer, bold_checkbox
)
...
html_annotator_extractor_checkbox = tk.Checkbutton(
    window, text="Extract HTML annotations", variable=html_annotator_extractor_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, html_annotator_extractor_checkbox
)
...
TIPS_lookup = {
    ...
    "Annotator DBpedia": "TIPS_NLP_Annotator DBpedia.pdf",
    "DBpedia ontology classes": "TIPS_NLP_Annotator DBpedia ontology classes.pdf",
    "Annotator YAGO": "TIPS_NLP_YAGO.pdf",
    "Annotator Wikipedia": "TIPS_NLP_Wikipedia.pdf",
    "YAGO (schema.org) ontology classes": "TIPS_NLP_Annotator YAGO (schema.org) ontology classes.pdf",
    "YAGO (REDUCED schema.org) ontology classes": "TIPS_NLP_Annotator YAGO (schema reduced).pdf",
    "W3C, OWL, RDF, SPARQL": "TIPS_NLP_W3C OWL RDF SPARQL.pdf",
    "Annotator dictionary": "TIPS_NLP_Annotator dictionary.pdf",
    "Annotator extractor": "TIPS_NLP_Annotator extractor.pdf",
...
TIPS_options = (
    ...
    "YAGO (REDUCED schema.org) ontology classes",
    "W3C, OWL, RDF, SPARQL",
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, click on the button to open the GUI to run the DBpedia and/or YAGO knowledge graphs.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, click on the button to open the GUI to run the WordNet knowledge graphs.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, click on the button to open the gender annotator GUI for annotating text by gender (male/female), either via Stanford CoreNLP gender annotator or various gender databases (US Census, US Social Security, Carnegie Mellon).",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
