...
def download_url(review_url):
    ...
    try:
        ...
        download_title = (
            reviewer + "_" + review_title + "_" + "BOOK_AUTHOR_ERR" + "_" + "BOOK_TITLE_ERR" + "_" + rev_date + ".txt"
        )
        with open(os.path.join(review_output_dir, download_title), "w+", encoding="utf-8", errors="ignore") as f:
            for a in soup.findAll("p", {"class": "css-axufdj evys1bk0"}):
                f.write(str(a.text))
...
if download == True:
    for i in range(
        0, int(33361 / 20) + 1
    ):  # this number came from the number of books found #(https://developer.nytimes.com/docs/books-product/1/routes/lists/best-sellers/history.json/get)
        if i > 0 and i % 2 == 0:
            time.sleep(12)
        offset = i * 20
        url = (
            "https://api.nytimes.com/svc/books/v3/lists/best-sellers/history.json?offset="
            + str(offset)
            + "&api-key="
            + api_key
        )
