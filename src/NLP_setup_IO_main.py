...
if (
    IO_libraries_util.install_all_Python_packages(GUI_util.window, "IO_setup_main", ["os", "tkinter", "argparse"])
    == False
):
    sys.exit(0)
...
if (
    (config_input_output_numeric_options[0] == 0 and config_input_output_numeric_options[1] != 0)
...
if (
    config_input_output_numeric_options[0] != 0
    and config_input_output_numeric_options[1] != 0
    and config_input_output_numeric_options[2] == 0
):
    GUI_size = GUI_width + "x360"
...
if config_input_output_numeric_options[0] != 0:
    ...
    list_item = 0
    if (
        config_input_output_alphabetic_options[0][4] != ""
        and "Date: " not in config_input_output_alphabetic_options[0][1]
    ):
        # the index for config_input_output_numeric_options starts at 0 with I/O configuration label
...
if config_input_output_numeric_options[1] != 0:  # input dir
    ...
    if (
        config_input_output_alphabetic_options[1][4] != ""
    ):  # and not 'Date: ' in config_input_output_alphabetic_options[1][1]: # there is date format
        ...
        try:
            ...
            Error = True
    if "Date:" not in config_input_output_alphabetic_options[1][1]:
    ...
    if "  (Date: " in label:
        ...
        # [1][4] [1][3] and [1][5] contain date format, item character separator, and date position
        date_hover_over_label = (
            "The txt files in the input directory contain a date embedded in the filename with the following values:\n"
            + "Date format: "
            + str(config_input_output_alphabetic_options[1][4])
            + " Date character(s) separator: "
            + str(config_input_output_alphabetic_options[1][3])
...
filename_embeds_multiple_items_checkbox = tk.Checkbutton(
    window,
    text="Filename embeds multiple items",
    variable=filename_embeds_multiple_items_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_fields(),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate, y_multiplier_integer, items_separator_lb, True
)
...
def set_default_options():
    ...
    # set date format
    if (
        config_input_output_alphabetic_options[list_item][4] != ""
        and config_input_output_alphabetic_options[list_item][4] != "0"
    ):
        filename_embeds_date_var.set(1)
    ...
    # set date position
    if (
        config_input_output_alphabetic_options[list_item][5] != ""
        and config_input_output_alphabetic_options[list_item][5] != "0"
    ):
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.open_setup_x_coordinate,  # date_position_lb_coordinate
    y_multiplier_integer,
    sort_order_lb,
    True,
)
...
date_checkbox = tk.Checkbutton(
    window,
    text="Filename embeds date",
    variable=filename_embeds_date_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_fields(),
)
...
if config_filename == "NLP_default_IO_config.csv":
    ...
    if (
        config_input_output_alphabetic_options[0][0] == "" and config_input_output_alphabetic_options[1][0] == ""
    ) or config_input_output_alphabetic_options[3][0] == "":
    ...
    if (config_input_output_alphabetic_options[0][0] != "") or config_input_output_alphabetic_options[1][0] != "":
        ...
        if (config_input_output_alphabetic_options[0][0] != "") and config_input_output_numeric_options[0] == 0:
            err_msg = (
                "The Default I/O configuration used by all scripts in the NLP Suite is currently set up with a FILE in INPUT.\n\n"
        ...
        if (config_input_output_alphabetic_options[1][0] != "") and config_input_output_numeric_options[1] == 0:
            ...
            err_msg = (
                "The Default I/O configuration used by all scripts in the NLP Suite is currently set up with a DIRECTORY in INPUT.\n\n"
...
TIPS_lookup = {
    "Setup INPUT-OUTPUT options": "TIPS_NLP_Setup INPUT-OUTPUT options.pdf",
    "Date embedded in filename": "TIPS_NLP_Date embedded in filename.pdf",
}
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    if config_input_output_numeric_options[1] != 0:
        y_multiplier_integer = GUI_IO_util.place_help_button(
            ...
            y_multiplier_integer,
            "NLP Suite Help",
            "Please, select the main INPUT directory of the TXT files to be analyzed."
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        '\n\nUsing the dropdown menu, select the position in the filename of the date field. For example, in the filename sotu#lincoln#12-03-1861.txt the date is the third item (starting with 1 for the first item).\n\nIf filenames contain only a date and no other items, e.g., 1981.txt or 12-21-1995, select the appropriate date format and 1 as date position, with  need to tick the checkbox "Filename embeds multiple items" and the options "Separator" and "Sort order".'
        + GUI_IO_util.msg_save_uponClose,
    )
...
GUI_util.input_main_dir_path.trace(
    "w", lambda x, y, z: changed_filename(GUI_util.inputFilename.get(), GUI_util.input_main_dir_path.get())
)
...
def get_IO_options_list(saving=False):
    ...
    if not filename_embeds_date_var.get():
        fileName_no_date = IO_files_util.open_file_removing_date_from_filename(
            window, GUI_util.inputFilename.get(), False
        )
    ...
    if not filename_embeds_date_var.get():
        directory_no_date = IO_files_util.open_directory_removing_date_from_directory(
            window, GUI_util.input_main_dir_path.get(), False
        )
...
def create_new_config_filename(config_input_output_alphabetic_options):
    ...
    if config_input_output_alphabetic_options[0][1] != "":
        IO_setup_display_string = "INPUT FILE: " + str(
            os.path.basename(os.path.normpath(config_input_output_alphabetic_options[0][1]))
        )
...
def save_config(config_input_output_alphabetic_options):
    ...
    answer = tk.messagebox.askyesnocancel(
        "Warning",
        "Where would you like to save your changes?\n\n\n"
...
def close_GUI(IO_configuration_upon_entry):
    ...
    if not missing_input and missing_output:
        msg = (
            msg
            + "You MUST select an OUTPUT files directory where all output files from the NLP Suite algorithms will be saved."
        )
...
close_button = tk.Button(
    window, text="CLOSE", width=10, height=2, command=lambda: close_GUI(IO_configuration_upon_entry)
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
if missing_IO:
    answer = tk.messagebox.askyesno(
        "Warning", "Do you want to watch the video on how to setup Input/Output (I/O) options?"
    )
    if answer:
...
reminders_util.checkReminder(scriptName, reminders_util.title_options_IO_setup, reminders_util.message_IO_setup, True)
result = reminders_util.checkReminder(
    scriptName, reminders_util.title_options_IO_setup_date_options, reminders_util.message_IO_setup_date_options, True
)
