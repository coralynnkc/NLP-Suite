...
def run():
    ...
    ctx = dict(
        ...
        package=package,
    )  # so runners honor the CONFIGURED parser (e.g. SVO via Stanza, not forced CoreNLP)
    ...
    profiler_startTime = IO_user_interface_util.timed_alert(
        ...
        4000,
        "Corpus Profiler",
        "Started running the Corpus Profiler with "
        + str(len(selected))
        + " of "
        + str(_total_analyses)
        + " analyses checked at",
    ...
    try:
        ...
        _orig_dialogs = (_mbmod.showwarning, _mbmod.showerror, _mbmod.showinfo, _mbmod.askyesno, _mbmod.askokcancel)

        def _silent_dialog(title=None, message=None, **_kw):
            print("[Corpus Profiler: dialog suppressed] %s -- %s" % (title, str(message).replace("\n", " ")[:300]))
        ...
        run_config = dict(
            ...
            + "  ·  "
            + time.strftime("%Y-%m-%d"),
            footer="NLP Suite — Corpus Profiler.  "
            + str(len(results))
            + " analyses run.  Per-file detail lives in the category subfolders.",
            inputDir=inputDir,
            inputFilename=inputFilename,
        )  # so the summary can draw a corpus wordcloud
        ...
        IO_user_interface_util.timed_alert(
            ...
            4000,
            "Corpus Profiler",
            "Finished running the Corpus Profiler with "
            + str(len(selected))
            + " of "
            + str(_total_analyses)
            + " analyses checked at",
        ...
        try:
            (_mbmod.showwarning, _mbmod.showerror, _mbmod.showinfo, _mbmod.askyesno, _mbmod.askokcancel) = _orig_dialogs
        except Exception:
            pass
...
GUI_label = (
    "Corpus Profiler — what's in your corpus? (run a battery of analyses; get an HTML report and a paper-style summary)"
)
...
counts_checkbox = tk.Checkbutton(
    window,
    text="How big / how varied? (Counts, measures, vocabulary, and entities - people, organizations, locations)",
    variable=counts_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, counts_checkbox, True
)
counts_menu_var.set("*")
counts_menu = tk.OptionMenu(
    window,
    counts_menu_var,
    "*",
...
entities_var.set(1)
entities_checkbox = tk.Checkbutton(
    window,
    text="Who said what, and when? (gender, dialogue, dates) (via CoreNLP)",
    variable=entities_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, entities_checkbox, True
)
entities_menu_var.set("*")
entities_menu = tk.OptionMenu(window, entities_menu_var, "*", "Gender, dates, dialogue (via CoreNLP)")
...
spatial_var.set(1)
spatial_checkbox = tk.Checkbutton(
    window,
    text="Where does it all happen?  (geocodable and symbolic space)",
    variable=spatial_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, spatial_checkbox, True
)
spatial_menu_var.set("*")
spatial_menu = tk.OptionMenu(
    window,
    spatial_menu_var,
    "*",
...
syntax_var.set(1)
syntax_checkbox = tk.Checkbutton(
    window, text="Grammar & structure  (Syntax)", variable=syntax_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, syntax_checkbox, True
)
syntax_menu_var.set("*")
syntax_menu = tk.OptionMenu(
    window, syntax_menu_var, "*", "POS · dependency · clause · N/V/Adj/Adv · function words · complexity · readability"
)
...
semantics_var.set(1)
semantics_checkbox = tk.Checkbutton(
    window, text="What do the words mean?  (Semantics)", variable=semantics_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, semantics_checkbox, True
)
semantics_menu_var.set("*")
semantics_menu = tk.OptionMenu(
    window,
    semantics_menu_var,
    "*",
...
topics_var.set(1)
topics_checkbox = tk.Checkbutton(window, text="What is it about?  (Topics)", variable=topics_var, onvalue=1, offvalue=0)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, topics_checkbox, True
)
topics_menu_var.set("*")
topics_menu = tk.OptionMenu(window, topics_menu_var, "*", "Topic modeling (BERT / Gensim / MALLET)")
...
narrative_var.set(1)
narrative_checkbox = tk.Checkbutton(
    window, text="Who did what to whom?  (Narrative)", variable=narrative_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, narrative_checkbox, True
)
narrative_menu_var.set("*")
narrative_menu = tk.OptionMenu(
    window,
    narrative_menu_var,
    "*",
...
sentiment_var.set(1)
sentiment_checkbox = tk.Checkbutton(
    window, text="How does it feel?  (Sentiment)", variable=sentiment_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, sentiment_checkbox, True
)
sentiment_menu_var.set("*")
sentiment_menu = tk.OptionMenu(
    window,
    sentiment_menu_var,
    "*",
    "Sentiment (Stanza)",
...
characters_checkbox = tk.Checkbutton(
    window,
    text="Zooming in on characters: Characters' emotional arcs and movements in time and space",
    variable=characters_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, characters_checkbox, True
)
characters_menu_var.set("*")
characters_menu = tk.OptionMenu(
    window,
    characters_menu_var,
    "*",
...
readMe_message = (
    "The Corpus Profiler runs a battery of NLP analyses on your corpus with sensible DEFAULTS and "
