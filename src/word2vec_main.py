...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "word2vec_main.py", ["os", "tkinter", "gensim", "spacy"]
    )
    == False
):
    sys.exit(0)
...
def run():
    ...
    if "Do not" not in vis_menu_var and word_distance_file_var.get() == "":
        result = mb.askyesno(
    ...
    if Gensim_var:
        ...
        filesToOpen = word2vec_Gensim_util.run_Gensim_word2vec(
            inputFilename,
            inputDir,
            Word2Vec_Dir,
            config_filename,
...
remove_stopwords_var.set(1)
remove_stopwords_checkbox = tk.Checkbutton(
    window, text="Remove stopwords & punctuation", variable=remove_stopwords_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, remove_stopwords_checkbox, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.setup_pop_up_text_widget, y_multiplier_integer, lemmatize_checkbox
)

vis_var_lb = tk.Label(window, text="Select visualization option")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, vis_var_lb, True
)
vis_menu_var.set("Plot word vectors")
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.setup_pop_up_text_widget, y_multiplier_integer, dim_menu
)


def activate_plot_options(*args):
    if "Do not" not in vis_menu_var.get():
        dim_menu_var.set("2D")
...
## option for BERT
BERT_var.set(0)
BERT_checkbox = tk.Checkbutton(
    window,
    text="Word embeddings (via BERT (English language model))",
    variable=BERT_var,
    onvalue=1,
    offvalue=0,
...
## option for Gensim
Gensim_var.set(0)
Gensim_checkbox = tk.Checkbutton(
    window,
    text="Word2Vec (via Gensim)",
    variable=Gensim_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, sg_lb, True
)
sg_menu_var.set("Skip-Gram")
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, vector_size_lb, True
)

vector_size_var.set(100)
vector_size_entry = tk.Entry(window, width=5, textvariable=vector_size_var)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.Word2Vec_vector_size_entry_pos, y_multiplier_integer, vector_size_entry, True
)

## option for window size
window_lb = tk.Label(window, text="Window size")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, window_lb, True
)

window_var.set(5)
window_size_entry = tk.Entry(window, width=5, textvariable=window_var)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.Word2Vec_window_size_entry_pos, y_multiplier_integer, window_size_entry, True
)

## option for minimum count
min_count_lb = tk.Label(window, text="Minimum count")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.Word2Vec_min_count_lb_pos, y_multiplier_integer, min_count_lb, True
)

min_count_var.set(5)
min_count_entry = tk.Entry(window, width=5, textvariable=min_count_var)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.Word2Vec_min_count_entry_pos, y_multiplier_integer, min_count_entry
)

## option for visualization method

compute_distances_var.set(1)
compute_distances_checkbox = tk.Checkbutton(
    window, text="Compute word distances", variable=compute_distances_var, onvalue=1, offvalue=0
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, top_words_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.Word2Vec_top_words_pos,
    y_multiplier_integer,
    top_words_entry,
    False,
    False,
...
word_distance_button = tk.Button(
    window,
    text="Select distance file ",
    command=lambda: get_word_distance_file(window, "Select INPUT distance file", [("distance files", "*.csv")]),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
open_word_distance_file_button = tk.Button(
    window,
    width=GUI_IO_util.open_file_directory_button_width,
    text="",
    command=lambda: IO_files_util.openFile(window, word_distance_file_var.get()),
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    GUI_IO_util.IO_configuration_menu,
    y_multiplier_integer,
    open_word_distance_file_button,
    True,
    False,
    True,
    False,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, keywords_lb, True
)
...
reminders_util.checkReminder(scriptName, reminders_util.title_options_Word2Vec, reminders_util.message_Word2Vec, True)
...
if error:
    mb.showwarning(
        title="Warning",
        message="The Word2Vec algorithms expect in input either a txt file/directory of txt files or a csv file of previosuly computed Euclidean distances.",
    )
