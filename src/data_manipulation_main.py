...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "data_manipulation_main.py", ["os", "tkinter", "pandas", "functools"]
    )
    == False
):
    sys.exit(0)
...
def run():
    ...
    if (operation == "CONCATENATE") and len(operation_results_text_list) < 2:
        mb.showwarning(
            ...
            message="The "
            + str(operation).upper()
            + " operation requires at least two fields. Please, click on the + button to select a second field and try again.",
        )
    ...
    if extract_var:
        if output_to_csv_var:
            ...
            export_type = ".txt"
        outputFilename = data_manipulation_util.export_csv_to_csv_txt(
            outputDir, operation_results_text_list, export_type
        )
        if outputFilename != None:
            filesToOpen.append(outputFilename)
...
if __name__ == "__main__":
    ...
    output_to_csv_var.set(
        1
    )  # EXTRACT defaults to csv output (the csv/txt checkbox was dropped in the single-row redesign)
    drop_var = tk.IntVar()
    split_var = tk.IntVar()
    sort_var = tk.IntVar()
    ...
    def _refresh_csv_file_dropdown():
        ...
        for _f in csv_files_list:
            _m.add_command(
                label=os.path.basename(_f),
                command=lambda v=_f: (csv_file_dropdown_var.set(os.path.basename(v)), selectedCsvFile_var.set(v)),
            )
    ...
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window,
        GUI_IO_util.labels_x_coordinate + 1050,
        y_multiplier_integer,
    ...
    extra_GUIs_menu = tk.OptionMenu(
        window,
        extra_GUIs_menu_var,
        "CSV data manipulation with SQL (Open GUI)",
    ...
    def build_string_for_processing(operation, csv_field_menu_choice, comingFrom_Plus, comingFrom_OK):
        ...
        if ((not comingFrom_Plus) and (not comingFrom_OK)) and (
            csv_field_menu_choice != "" and (csv_field_menu_choice in selected_csv_fields_var.get())
        ):
            mb.showwarning(
                title="Warning",
                message="You have already selected the field "
                + csv_field_menu_choice
        ...
        if operation == "extract" or operation == "drop":
            if comparator_var.get() != "" and where_entry_var.get() == "":
                mb.showwarning(
                    title="Warning",
                    message="You have selected the comparator value "
                    + comparator_var.get()
    ...
    def operation_OK():
        ...
        if op == "":
            mb.showwarning(
                title="No operation", message="Please, select an operation from the dropdown menu before clicking OK."
            )
            return
    ...
    operation_options = ["Append", "Concatenate", "Deduplicate", "Drop", "Extract", "Merge", "Rename", "Sort", "Split"]
    ...
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, select_csv_field_lb, True
    )
    ...
    add_drop_field = tk.Button(
        window,
        text="+",
        width=GUI_IO_util.add_button_width,
        height=1,
        state="disabled",
        command=lambda: operation_plus_field(),
    )
    # place widget with hover-over info
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window,
        GUI_IO_util.labels_x_coordinate + 950,
        y_multiplier_integer,
        add_drop_field,
        True,
        False,
    ...
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window,
        GUI_IO_util.labels_x_coordinate + 1000,
        y_multiplier_integer,
        add_drop_file,
        True,
        False,
    ...
    OK_operation_button = tk.Button(
        window, text="OK", width=GUI_IO_util.OK_button_width, height=1, state="disabled", command=lambda: operation_OK()
    )
    # place widget with hover-over info
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window,
    ...
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, where_lb, True
    )
    ...
    def _refresh_where_values(*args):
        ...
        try:
            values = data_manipulation_util.distinct_values(selectedCsvFile_var.get(), select_csv_field_var.get())
        except Exception:
    ...
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window, GUI_IO_util.labels_x_indented_coordinate + 460, y_multiplier_integer, and_or_lb, True
    )
    ...
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, operation_results_text
    )
    ...
    def add_csvFile(window, title, fileType):
        ...
        filePath = tk.filedialog.askopenfilename(
            title=title, initialdir=initialFolder, initialfile=initialFile, filetypes=fileType
        )
        if len(filePath) > 0:
            select_csv_field_merge_var.set("")
            selected_csv_fields_var.set("")
    ...
    TIPS_lookup = {
        "Data manipulation GUI": "TIPS_NLP_Data manipulation.pdf",
    ...
    TIPS_options = (
        ...
        "Filtering data (thresholds, values, stop words)",
    )
