...
if (
    IO_libraries_util.install_all_Python_packages(GUI_util.window, "data_visualization_main.py", ["os", "tkinter"])
    == False
):
    sys.exit(0)
...
def run_categorical(
    ...
    if categorical_menu_var == "":
        mb.showwarning(
            "Warning", "Please, use the dropdown menu to select one of the options for categorical data and try again."
        )
        return
    ...
    if "*" in categorical_menu_var or "Colormap" in categorical_menu_var:
        ...
        mb.showwarning(
            ...
            + '\n\nThe last field "'
            + csv_file_categorical_field_list[len(csv_file_categorical_field_list) - 1][0].split("|")[0]
        ...
        if len(csv_file_categorical_field_list) < 2:
            ...
            if "*" in categorical_menu_var:
                outputFile2 = charts_util.stacked_bar_from_csv(
                    inputFilename, outputDir, group_field, segment_field, grouped=not is_grouped
                )
                if outputFile2:
...
def run_temporal(
    ...
    if "Time mapper" in temporal_menu_var:
        ...
        outputFile = charts_util.heatmap_calendar(
            inputFilename, outputDir, calendar_date_var, value_col=value_col, date_format=date_format_var
        )
        if outputFile:
            filesToOpen.append(outputFile)
...
def run_numeric(
    ...
    if "Boxplot" in visualizations_menu_var:
        ...
        if X_axis_var == "" and len(csv_file_field_Y_axis_list) < 1:
            mb.showwarning(
                "Warning", "No X-axis or Y-axis variable has been selected.\n\nPlease, select a variable and try again."
            )
            return
        ...
        outputFilename = IO_files_util.generate_output_file_name(
            inputFilename, "", outputDir, ".html", "histogram", csv_field_visualization_var
        )
        ...
        outputFile = charts_util.violin_plot(
            inputFilename, outputFilename, csv_field_visualization_var, points=violin_points_var, category=category
        )
        if outputFile:
            filesToOpen.append(outputFile)
...
input_csv_file_button = tk.Button(
    window,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, input_csv_file_button, True
)

open_input_csv_file_button = tk.Button(
    window,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate, y_multiplier_integer, input_csv_file_entry
)
...
def tab_help(parent, y, message):
    btn = tk.Button(parent, text="? HELP", command=lambda: mb.showinfo("NLP Suite Help", message))
...
def display_selected_csv_fields():
    if csv_field_relational_var.get() != "" and csv_field_relational_var.get() not in csv_file_relational_field_list:
...
add_button_relational = tk.Button(
    tab_relational, text="+", width=2, height=1, command=lambda: display_selected_csv_fields()
)
add_button_relational.place(x=580, y=42)

reset_button_relational = tk.Button(
    tab_relational, text="Reset", width=4, height=1, state="disabled", command=lambda: reset_relational()
)
...
categorical_menu = tk.OptionMenu(
    tab_categorical,
    categorical_menu_var,
    "*",
    "Colormap/heatmap",
...
search_values_categorical = tk.Entry(
    tab_categorical, state="disabled", textvariable=search_values_categorical_var, width=20
)
...
add_button_categorical = tk.Button(
    tab_categorical, text="+", width=2, height=1, command=lambda: add_combination_csvField_searchValues()
)
...
reset_button_categorical = tk.Button(
    tab_categorical, text="Reset", width=4, height=1, command=lambda: reset_categorical()
)
...
def show_categorical_list():
    if len(csv_file_categorical_field_list) == 0:
        mb.showwarning(
            title="Warning", message="There are no currently selected combinations of csv file field and search words."
        )
    else:
        mb.showwarning(
            title="Warning",
...
show_button_categorical = tk.Button(
    tab_categorical, text="Show", width=4, height=1, command=lambda: show_categorical_list()
)
...
filter_options_menu = tk.OptionMenu(
    tab_categorical, filter_options_var, "No filtering", "Fixed parameter", "Propagating parameter"
)
...
data_transformation_menu = tk.OptionMenu(
    tab_categorical, data_transformation_var, "No transform", "Min-Max", "Z-score", "Square root", "Log", "Ln"
)
...
comparative_bar_lb = tk.Label(
    tab_categorical, text="Comparative bar charts", foreground="red", font=("Courier", 12, "bold")
)
...
add_file = tk.Button(
    ...
    width=2,
    height=1,
    command=lambda: add_csvFile_comparative(window, "Select csv file", [("csv files", "*.csv")]),
)
...
openInputFile_button_comparative = tk.Button(
    tab_categorical, width=1, text="", command=lambda: IO_files_util.openFile(window, csv_file_var.get())
)
...
def activate_csv_fields_categorical_selection(comingFromPlus=True):
    ...
    if csv_field_categorical_var.get() != "":
        if csv_field_categorical_var.get() in csv_file_categorical_field_list:
            ...
            csv_file_categorical_field_string = (
                csv_file_categorical_field_string + "|" + csv_field_categorical_var.get()
            )
    if csv_field_categorical_var.get() != "":
...
date_format_menu = tk.OptionMenu(
    tab_temporal, date_format_var, "mm-dd-yyyy", "dd-mm-yyyy", "yyyy-mm-dd", "yyyy-dd-mm", "yyyy-mm", "yyyy"
)
...
visualizations_menu = tk.OptionMenu(
    tab_numeric,
    visualizations_menu_var,
    "Boxplots",
    "Bubble chart",
...
tab_help(tab_numeric, 45, "Select the csv file field to use as the Y-axis for charts, boxplots, or bubble charts.")
...
def check_selected_csv_file_field_Y_axis_list(main_Y_axis):
    ...
    if "Excel" not in visualizations_menu_var.get():
        return
...
add_Y_axis = tk.Button(
    tab_numeric, text="+", width=2, height=1, command=lambda: check_selected_csv_file_field_Y_axis_list(False)
)
...
reset_Y_axis_button = tk.Button(
    tab_numeric, text="Reset", width=4, height=1, state="normal", command=lambda: reset_csv_field_Y_axis_values()
)
...
split_data_byCategory_checkbox = tk.Checkbutton(
    tab_numeric,
...
geo_description = tk.Label(
    tab_geographic, text="Geographic visualization", font=("Courier", 12, "bold"), foreground="red"
)
geo_description.place(x=10, y=10)

geo_open_button = tk.Button(
    tab_geographic, text="Open GIS GUI", width=20, command=lambda: run_script_util.run_script("GIS_main.py")
)
geo_open_button.place(x=10, y=40)

geo_open_ge_button = tk.Button(
    tab_geographic,
...
geo_open_symbolic_button = tk.Button(
    tab_geographic,
...
geo_sep = tk.Label(
    tab_geographic, text="─── Animated movement map (from CSV) ───", font=("Courier", 10, "bold"), foreground="#555"
)
...
wc_description = tk.Label(
    tab_wordclouds, text="Wordcloud visualization", font=("Courier", 12, "bold"), foreground="red"
)
...
tab_help(tab_tree, 120, "Info / tooltip column (optional): extra text shown when you hover over a node.")
tab_help(
    tab_tree,
    155,
...
tree_description = tk.Label(
    tab_tree, text="Hierarchical tree visualization", font=("Courier", 12, "bold"), foreground="red"
)
