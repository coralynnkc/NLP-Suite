...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "knowledge_graph_DBpedia_util.py", ["os", "requests", "urllib", "ssl", "shutil", "bs4"]
    )
    == False
):
    sys.exit(0)
...
def DBpedia_annotate(
    ...
    startTime = IO_user_interface_util.timed_alert(
        ...
        2000,
        "Analysis start",
        "Started running DBpedia Knowledge Graph at",
        True,
    ...
    for i, file in enumerate(files, 1):
        ...
        while j < len(listOfSplitFiles):
            ...
            with open(doc, encoding="utf-8", errors="ignore") as _f:
                contents = _f.read()
            ...
            try:
                r = session.get(
                    SPOTLIGHT_URL, params=params, headers={"Accept": "application/json"}, timeout=REQUEST_TIMEOUT
                )
            ...
            try:
                r_html = session.get(
                    SPOTLIGHT_URL, params=params, headers={"Accept": "text/html"}, timeout=REQUEST_TIMEOUT
                )
        ...
        if numberOfSplitSubfiles > 0:
            outFilename = os.path.join(
                outputDir, "NLP_DBpedia_annotated_" + os.path.split(file)[1].split(".txt")[0] + ".html"
            )
            with open(outFilename, "w", encoding="utf-8", errors="ignore") as outfile:
                for htmlDoc in splitHtmlFileList:
                    ...
                    if os.path.exists(htmlDoc):
                        with open(htmlDoc, encoding="utf-8", errors="ignore") as infile:
    ...
    if not all_phrases:
        IO_user_interface_util.timed_alert(
            ...
            3000,
            "Analysis end",
            "Finished running DBpedia Knowledge Graph at",
            True,
            "",
            True,
            startTime,
    ...
    if not df.empty:
        ...
        outputFiles = charts_util.plot(
            csvname, outputDir, columns=["Token"], title="Frequency of DBpedia Words", x_label="DBpedia word"
        )
    ...
    IO_user_interface_util.timed_alert(
        GUI_util.window, 3000, "Analysis end", "Finished running DBpedia Knowledge Graph at", True, "", True, startTime
    )
    return filesToOpen
