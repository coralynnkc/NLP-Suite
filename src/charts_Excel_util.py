...
if (
    IO_libraries_util.install_all_Python_packages(
        GUI_util.window, "charts_Excel_util", ["csv", "tkinter", "os", "collections", "openpyxl"]
    )
    == False
):
    sys.exit(0)
    ...
    for i in range(
        ...
        if withHeader_var == 1:
            if hover_info_column_list[i] in headers:
                ...
                if len(hover_info_column_list[i]) > 0:
                    mb.showwarning(
                        ...
                        message="The hover-over data column for series No."
                        + str(i + 1)
            ...
            try:
                ...
                mb.showwarning(
                    title="Series No." + str(i + 1) + " " + hover_info_column_list[i] + " Hover Data Header",
                    message="The input csv file has no header so the expected hover-over column header should be numbers(o for A, 1 for B,...) but the ENTERED hover-over data column for series No."
                    + str(i + 1)
                    + " is not a number.\n\nPlease, exit the program, check your input and try again.",
                )
                return
...
def create_excel_chart(
    ...
    chart_outputFilename = IO_files_util.generate_output_file_name(
        inputFilename, "", outputDir, outputExtension, scriptType, chart_type_list[0], "chart"
    )
    ...
    if len(hover_info_column_list) > 0:  # hover-over effects are invoked and the Excel filename extension MUST be xlsm
        if len(chart_type_list) == 0:
            mb.showwarning(
                ...
                message="No chart type was specified (e.g., line, bubble). The chart could not be created.\n\nPlease, select a chart type and try again!",
            )
            return
            ...
            if first_chart_type.lower() == "bar":
                ...
                if len(chart_type_list) > 1:
                    mb.showwarning(
                        ...
                        message="If you selected pie chart as the intended chart type for display data, only one group of data can be displayed. The system indicates more than one group of data are selected.\n\nPlease, check your input and try again!",
                    )
                    return
                ...
                for l in range(len(data_to_be_plotted)):
                    ...
                    for i in data_to_be_plotted[l]:
                        ...
                        try:
                            ...
                            mb.showwarning(
                                title="Scatter Chart error",
                                message="If you selected a scatter chart as the intended chart type to display data, both X-axis and Y-axis can only contain numeric values. Among the columns selected, at least one contains non-numeric values.\n\nPlease, check your input and try again!",
                            )
                            return
                ...
                mb.showwarning(
                    title="Chart type error",
                    message="The hover-over feature is only available for Bar, Line, Pie, and Scatter charts. The selected chart type is not allowed.\n\nPlease, check your input and try again!",
                )
                return
        else:
            mb.showwarning(
                title="Chart type error",
                message="The hover-over feature for multiple groups of data requires that all  groups have the same chart type. The system indicated more than one chart type.\n\nPlease, check your input and try again!",
            )
            return
        ...
        if reverse_column_position_for_series_label == True:
            mb.showwarning(
                title="Reverse Series Label Variable Warning",
                message="The system indicates that you set reverse var for series labels to be true; however, in the hover-over feature, the series labels can only be the header of the Y-axis values (Column B, C, D,... in 'Data' sheet). Or you can specify series labels in series_label_list.\n\nPlease click 'OK' and continue.",
            )
        ...
        reminders_util.checkReminder(
            "*", reminders_util.title_options_Excel_Charts, reminders_util.message_Excel_Charts, True
        )
        ...
        if second_y_var == 0:  # we are NOT plotting with 2 y axes with different scales
            ...
            if len(series_label_list) > n:
                mb.showwarning(
                    ...
                    message="The system indicates that there are more series hover_over_values specified than the number of series ("
                    + str(n)
            ...
            if len(chart_type_list) > 2:
                mb.showwarning(
                    title="Number of series error",
                    message="When creating a chart with two y axis, you can ONLY choose two series of data. Here more than two series of data were specified. The chart could not be created.\n\nPlease, select a new pair of series and try again!",
                )
                return
            ...
            if chart_type_list[0].lower() == "bar":
                ...
                mb.showwarning(
                    title="Chart type 1 error",
                    message="Wrong chart type selected. Only bar, bubble, line and scatter chart are allowed to have y axis",
                )
