...
def get_GitHub_release_version(silent=False):
    ...
    print(
        "NLP Suite version check: could not reach GitHub to check the latest release after 3 tries "
...
def activateRunButton(config_filename, IO_setup_display_brief, scriptName, missing_IO, silent=False):
    ...
    run_button_state, _open_ignored = config_util.check_missing_IO(
        window, config_filename, scriptName, IO_setup_display_brief, missing_IO, silent=True
    )
    if missing_IO != "" and not silent:

        def _deferred_missing_IO(cf=config_filename, mio=missing_IO):
            _state, open_setup_IO_GUI = config_util.check_missing_IO(
                window, cf, scriptName, IO_setup_display_brief, mio, silent=False
            )
...
def open_paste_text_popup():
    ...
    tk.Label(
        popup, text="Paste or type your text below, then click 'Use this text'.", wraplength=660, justify="left"
...
def IO_config_setup_brief(window, y_multiplier_integer, config_filename, scriptName, silent):
    ...
    if sys.platform == "darwin" and not GUI_IO_util.grid_layout_enabled:
        io_configuration_menu_x = max(
            ...
            GUI_IO_util.labels_x_coordinate + IO_setup_button.winfo_reqwidth() + 5,
        )
    # place widget with hover-over info
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window,
    ...
    openTextWidget_button = tk.Button(
        window, width=GUI_IO_util.open_file_directory_button_width, text="", command=open_paste_text_popup
    )
    # place widget with hover-over info
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window,
        ...
        "Click to open a text-entry widget where you can paste text for a quick test run, instead of setting up a full I/O configuration.",
    )
...
def setup_IO_configuration_options(IO_setup_display_brief, scriptName, silent, open_setup_IO_GUI):
    ...
    if (
        "_ALL_" not in scriptName or config_input_output_numeric_options != [0, 0, 0, 0]
    ) and "package_language" not in scriptName:
        try:
...
def GUI_top(config_input_output_numeric_options, config_filename, IO_setup_display_brief, scriptName, silent=False):
    ...
    if config_filename != "license_config.csv":
        if GUI_IO_util.grid_layout_enabled:
            ...
            intro.grid(row=0, column=0, columnspan=GUI_IO_util._GRID_TOTAL_COLUMNS, padx=6, pady=(6, 4), sticky="")
        else:
...
def GUI_bottom(
    ...
    if (
        ...
        def run_data_tool(*args):
            ...
            if "data_visualization_main.py" not in scriptName and "visualization" in data_tools_options_widget.get():
